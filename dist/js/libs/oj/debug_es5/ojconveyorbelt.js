...
(function () {
  ...
  var _RESIZE_LISTENER_COLLAPSE_EVENT_TIMEOUT = 25; // end static members and functions ////////////////////////////////////////////
  ...
  oj.__registerWidget('oj.ojConveyorBelt', $.oj.baseComponent, {
    ...
    _setup: function _setup() {
      ...
      if (!this._cbCommon) {
        ...
        callbackInfo.addResizeListener = function (_elem, listener) {
          oj.DomUtils.addResizeListener(_elem, listener, _RESIZE_LISTENER_COLLAPSE_EVENT_TIMEOUT);
        };
     ...
     * @private
     */
    _createIcon: function _createIcon(iconStyleClass) {
    ...
    scrollElementIntoView: function scrollElementIntoView(element) {
      ...
      if (this._cbCommon._isHorizontal()) {
        // in case of a horizontal conveyorbelt
        var currentScrollLeft = this.widget().find('.oj-conveyorbelt-overflow-container')[0].scrollLeft;

        if (element.offsetLeft + element.offsetWidth >= currentScrollLeft + currentViewportSize) {
          // if element (e.g. tab) is partially hidden to the right side
        ...
        } else if (element.offsetLeft < currentScrollLeft) {
          // if element (e.g. tab) is partially hidden to the left side
          this.widget().find('.oj-conveyorbelt-overflow-container')[0].scrollLeft = element.offsetLeft;
        }
      } else {
        // in case of a vertical conveyorbelt
        var currentScrollTop = this.widget().find('.oj-conveyorbelt-overflow-container')[0].scrollTop;

        if (element.offsetTop + element.offsetHeight >= currentScrollTop + currentViewportSize) {
          // if element (e.g. tab) is partially hidden below
        ...
        } else if (element.offsetTop < currentScrollTop) {
          // if element (e.g. tab) is partially hidden above
