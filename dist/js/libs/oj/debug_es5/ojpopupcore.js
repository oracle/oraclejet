...
oj.PopupServiceImpl.prototype.open = function (options) {
  ...
  if (!(status === oj.ZOrderUtils.STATUS.UNKNOWN || status === oj.ZOrderUtils.STATUS.BEFORE_OPEN || status === oj.ZOrderUtils.STATUS.CLOSE)) {
  ...
  if (!modality || !(oj.PopupService.MODALITY.MODELESS === modality || oj.PopupService.MODALITY.MODAL === modality)) {
  ...
  if (!layerLevel || !(oj.PopupService.LAYER_LEVEL.TOP_LEVEL === layerLevel || oj.PopupService.LAYER_LEVEL.NEAREST_ANCESTOR === layerLevel)) {
  ...
  oj.ZOrderUtils.setStatus(popup, oj.ZOrderUtils.STATUS.OPENING); // set logical parent

  oj.DomUtils.setLogicalParent(popup, launcher);
  oj.ZOrderUtils.addToAncestorLayer(popup, launcher, modality, layerClass, layerLevel, isCustomElement);
  ...
  var _finalize = function _finalize() {
    try {
      ...
      oj.ZOrderUtils.setStatus(popup, oj.ZOrderUtils.STATUS.OPEN); // invoke the after open callback if one is provided.
      ...
      oj.ZOrderUtils.applyEvents(layer, events); // if the originating subtree where the popup was defined is removed during
      ...
      if (!oj.ZOrderUtils._getSurrogate(layer) && $.isFunction(events[oj.PopupService.EVENT.POPUP_REMOVE])) {
...
oj.PopupServiceImpl.prototype.close = function (options) {
  ...
  if (!events) {
    events = oj.ZOrderUtils.getEvents(layer); // eslint-disable-next-line no-param-reassign
  ...
  if (!(status === oj.ZOrderUtils.STATUS.OPEN || status === oj.ZOrderUtils.STATUS.BEFORE_CLOSE) || !events) {
    this._assertEventSink();
  ...
  oj.ZOrderUtils.setStatus(popup, oj.ZOrderUtils.STATUS.CLOSING); // Unregister events during before close callback
  ...
  var _finalize = function _finalize() {
    try {
      ...
      oj.ZOrderUtils.removeFromAncestorLayer(popup); // remove the logical parent
...
oj.PopupServiceImpl.prototype._assertEventSink = function () {
  ...
  if (!hasPopupsOpen && callbackEventFilter) {
    ...
    docElement.removeEventListener('mousewheel', oj.PopupServiceImpl._refreshCallback, {
      passive: true,
      capture: true
    });
    ...
    docElement = document.documentElement;
    docElement.addEventListener('mousewheel', oj.PopupServiceImpl._refreshCallback, {
      passive: true,
      capture: true
    });
...
oj.PopupServiceImpl.prototype._eventFilterCallback = function (event) {
  ...
  if (event.type === 'keydown' && oj.ZOrderUtils.hasModalDialogOpen() && !oj.DomUtils.isAncestor(defaultLayer[0], target[0])) {
...
oj.PopupServiceImpl._COPY_SAFE_EVENT_PROPERTIES = ['altKey', 'bubbles', 'cancelable', 'ctrlKey', 'currentTarget', 'eventPhase', 'metaKey', 'relatedTarget', 'shiftKey', 'target', 'timeStamp', 'view', 'which', 'button', 'buttons', 'clientX', 'clientY', 'offsetX', 'offsetY', 'pageX', 'pageY', 'screenX', 'screenY', 'toElement', 'char', 'charCode', 'key', 'keyCode'];
/**
...
oj.ZOrderUtils.setStatus = function (popup, status) {
  ...
  if (status > oj.ZOrderUtils.STATUS.UNKNOWN && status <= oj.ZOrderUtils.STATUS.CLOSE) {
...
oj.ZOrderUtils.getFirstAncestorLayer = function (launcher) {
  ...
  while (parent && parent.length > 0 && parent.attr(oj.ZOrderUtils._SURROGATE_ATTR) !== oj.ZOrderUtils._DEFAULT_LAYER_ID) {
...
oj.ZOrderUtils.addToAncestorLayer = function (popup, launcher, modality, layerClass, layerLevel, isCustomElement) {
...
oj.ZOrderUtils.removeFromAncestorLayer = function (popup) {
  ...
  var originatingSubtreeExists = oj.ZOrderUtils._removeSurrogate(layer); // if the popup is not orphaned
...
oj.ZOrderUtils._visitTree = function (layer, callback, context) {
  ...
  for (var i = childrenCount - 1; i > -1; i--) {
    ...
    if (oj.ZOrderUtils._hasSurrogate(child[0])) {
      ...
      vrtn = oj.ZOrderUtils._visitTree(child, callback, context); // eslint-disable-next-line no-param-reassign
...
oj.ZOrderUtils.isAboveTopModalLayer = function (element) {
  ...
  return oj.DomUtils.isAncestorOrSelf(topModalLayer, element) || oj.ZOrderUtils.compareStackingContexts($(topModalLayer), $(element)) < 0;
};
/**
...
oj.PopupWhenReadyMediator.prototype._getBusyStateDescription = function (element, operation, widgetName) {
...
oj.PopupWhenReadyMediator.prototype.isOperationPending = function (widgetInstance, operation, methodName, methodArgs) {
  var isPending = false;
...
oj.PositionUtils.addTransformOriginAnimationEffectsOption = function (element, effects) {
  ...
  if (effectsAsString.match(exp)) {
    var alignMnemonic =
    /** @type {string} */
    element.data(oj.PositionUtils._ALIGN_MNEMONIC_DATA);
...
oj.PositionUtils._coerceMyAtToJet = function (type, source, offsetSource, sourceDefault) {
  ...
  if (oj.StringUtils.isString(source)) {
    ...
    if (tokens.length > 0 && !oj.StringUtils.isEmpty(tokens[0])) {
      groups = oj.PositionUtils._parsePositionNmnemonic(tokens[0], oj.PositionUtils._HORIZONTAL_ENUM_TST_REGX); // verify horizontal enum
    ...
    if (tokens.length > 1 && !oj.StringUtils.isEmpty(tokens[1])) {
      groups = oj.PositionUtils._parsePositionNmnemonic(tokens[1], oj.PositionUtils._VERTICAL_ENUM_TST_REGX); // verify vertical enum
    ...
    if ('horizontal' in source) {
      groups = oj.PositionUtils._parsePositionNmnemonic(source.horizontal, oj.PositionUtils._HORIZONTAL_ENUM_TST_REGX);
    ...
    if ('vertical' in source) {
      groups = oj.PositionUtils._parsePositionNmnemonic(source.vertical, oj.PositionUtils._VERTICAL_ENUM_TST_REGX);
...
oj.PositionUtils.coerceToJet = function (source, defaults) {
  ...
  var targetAt = oj.PositionUtils._coerceMyAtToJet('at', source.at, null, atDefault); // sum the "at" and "my" offsets
  ...
  var target = $.extend({}, targetMy, targetAt, targetOffset, oj.PositionUtils._coerceCollisionToJet(source.collision, collisionDefault), oj.PositionUtils._coerceOfToJet(source.of, ofDefault), _coerceUsingToJet(source.using, usingDefault));
  return target;
};
/**
