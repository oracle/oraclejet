...
MessagesViewModel.NAVIGATION_TRACKER = {
  ...
  _indexOfFocusWithin: function _indexOfFocusWithin(target) {
    ...
    for (var i = 0; i < messagesContainerIds.length; i++) {
      ...
      if (messagesContainerDiv && oj.DomUtils.isAncestorOrSelf(messagesContainerDiv, target)) {
        return i;
      }
    }
  ...
  _messageContainerListener: function _messageContainerListener(id, event) {
    ...
    if (event.type === 'focus' || event.type === 'click') {
      ...
      if (prevActiveElement && messagesContainerDiv && !oj.DomUtils.isAncestorOrSelf(messagesContainerDiv, prevActiveElement)) {
