...
function ChildMutationObserver(element, handler) {
  ...
  var _trackOption = oj.BaseCustomElementBridge.getRegistered(element.tagName) ? oj.BaseCustomElementBridge.getTrackChildrenOption(element) : 'none';
  /**
    ...
    for (var i = 0; i < mutations.length; i++) {
      ...
      while (testElement) {
        if (testElement === _element) {
          ...
          testElement = null;
        } else if (_trackOption === 'nearestCustomElement' && !oj.ElementUtils.isValidCustomElementName(testElement.localName)) {
