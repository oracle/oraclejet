 ...
 * @since 2.0.0
 */
oj.Composite = {}; // this is the variable name that the AMD module will return in the require callback (used in a no-require environment)
...
oj.Composite.__BINDING_PROVIDER = '__oj_binding_prvdr'; // TYPEDEFS
...
oj.CollectionUtils.copyInto(oj.CompositeElementBridge.proto, {
      ...
      if (!bridge.SaveEarlyPropertySet(prop, value)) {
        ...
        if (setResult.propertySet) {
          if (setResult.isSubproperty) {
            ...
            var propertyTracker = oj.CompositeElementBridge._getPropertyTracker(bridge, setResult.property);
  ...
  CreateComponent: function CreateComponent(element) {
    ...
    activatedPromise.then(function () {
      ...
      Object.defineProperty(element, oj.Composite.__BINDING_PROVIDER, {
        value: 'knockout'
      ...
      var cache = oj.BaseCustomElementBridge.__GetCache(element.tagName); // Need to clone nodes first


      var view = oj.CompositeElementBridge._getDomNodes(cache.view, element);

      oj.CompositeTemplateRenderer.renderTemplate(params, element, view); // Set flag when we can fire property change events

      bridge.__READY_TO_FIRE = true; // Resolve the component busy state
  ...
  DefinePropertyCallback: function DefinePropertyCallback(proto, property, propertyMeta) {
    ...
    var get = function get(bOuterSet) {
      var propertyTracker = oj.CompositeElementBridge._getPropertyTracker(this._BRIDGE, property); // If the attribute has not been set, return the default value
  ...
  GetTrackChildrenOption: function GetTrackChildrenOption() {
    return 'immediate';
  },
  ...
  InitializeElement: function InitializeElement(element) {
    ...
    if (!cache.view) {
      ...
      var view = descriptor[oj.BaseCustomElementBridge.DESC_KEY_VIEW]; // when multiple instances of the same CCA are on the same page, because of the async
    ...
    if (!cache.css) {
      ...
      var css = descriptor[oj.BaseCustomElementBridge.DESC_KEY_CSS]; // CSS is optional so we need to check if it was provided
  ...
  _getViewModel: function _getViewModel(element) {
    if (!this._VIEW_MODEL) {
...
oj.CompositeElementBridge.register = function (tagName, descriptor) {
  var descrip = {};
  descrip[oj.BaseCustomElementBridge.DESC_KEY_META] = oj.CompositeElementBridge._getResource(tagName, descriptor, oj.BaseCustomElementBridge.DESC_KEY_META);
  descrip[oj.BaseCustomElementBridge.DESC_KEY_VIEW] = oj.CompositeElementBridge._getResource(tagName, descriptor, oj.BaseCustomElementBridge.DESC_KEY_VIEW);
  descrip[oj.BaseCustomElementBridge.DESC_KEY_CSS] = oj.CompositeElementBridge._getResource(tagName, descriptor, oj.BaseCustomElementBridge.DESC_KEY_CSS);
  descrip[oj.BaseCustomElementBridge.DESC_KEY_VIEW_MODEL] = oj.CompositeElementBridge._getResource(tagName, descriptor, oj.BaseCustomElementBridge.DESC_KEY_VIEW_MODEL);
...
oj.CompositeElementBridge._getDomNodes = function (content, element) {
  ...
  var bridge = oj.BaseCustomElementBridge.getInstance(element); // TODO update this error message once we remove support for Array of DOM nodes and DocumentFragment

  bridge.throwError(element, 'The composite View is not one of the following supported types: string, Array of DOM nodes, DocumentFragment');
...
oj.CompositeElementBridge._walkSubtree = function (subIdMap, nodeMap, node) {
  if (!node.hasAttribute('slot')) {
    oj.CompositeElementBridge._addNodeToSubIdMap(subIdMap, nodeMap, node); // For upstream or indirect dependency we will still rely components being registered on the oj namespace.
