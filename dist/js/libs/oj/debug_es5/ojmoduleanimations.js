 ...
 * @export
 */
oj.ModuleAnimations = {}; // mapping variable definition, used in a no-require environment. Maps the oj.ModuleAnimations object to the name used in the require callback.
...
oj.ModuleAnimations._getModuleEffect = function (animateName) {
  if (oj.ModuleAnimations._moduleEffects == null) {
    oj.ModuleAnimations._moduleEffects = ThemeUtils.parseJSONFromFontFamily('oj-animation-module-effects');
  }
...
oj.ModuleAnimations._getImplementation = function (animateName) {
  ...
  if (descriptor) {
    return oj.ModuleAnimations.createAnimation(descriptor.oldViewEffect, descriptor.newViewEffect, descriptor.newViewOnTop);
  }
...
oj.ModuleAnimations._getNavigateMethod = function (context, navigationType) {
  if (oj.ModuleAnimations._navigateMethods == null) {
    oj.ModuleAnimations._navigateMethods = ThemeUtils.parseJSONFromFontFamily('oj-animation-navigate-methods');
  }
...
oj.ModuleAnimations._navigateCanAnimate = function (context, navigationType) {
  ...
  if (oj.ModuleAnimations[animateName]) {
    return oj.ModuleAnimations[animateName].canAnimate == null || oj.ModuleAnimations[animateName].canAnimate(context);
  }
...
oj.ModuleAnimations.coverStart = oj.DomUtils.getReadingDirection() === 'rtl' ? oj.ModuleAnimations.coverRight : oj.ModuleAnimations.coverLeft;
/**
 * ModuleAnimation implementation for changing views by
...
oj.ModuleAnimations.revealEnd = oj.DomUtils.getReadingDirection() === 'rtl' ? oj.ModuleAnimations.revealLeft : oj.ModuleAnimations.revealRight;
/**
 * ModuleAnimation implementation for changing views by
