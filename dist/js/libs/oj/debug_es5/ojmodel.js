  ...
  for (e = 0; e < eventsToFire.length; e += 1) {
    ...
    for (i = 0; i < (handlers ? handlers.length : 0); i += 1) {
      if (handlers[i].attribute === eventsToFire[e].attribute && handlers[i].callback) {
        ...
        if (handlers && handlers[i] && handlers[i].once) {
          ...
          this._removeHandler(oj.Events._getHandlers(this.eventHandlers, eventsToFire[e].event, true), handlers[i]); // Now take it out of the other object's "listen to" list, if relevant
    ...
    for (i = 0; i < (allHandlers ? allHandlers.length : 0); i += 1) {
      ...
      if (allHandlers && allHandlers[i] && allHandlers[i].once) {
        ...
        this._removeHandler(this._getHandlers(this.eventHandlers, oj.Events.EventType.ALL, true), allHandlers[i]); // Now take it out of the other object's "listen to" list, if relevant
...
oj.Collection.extend = function (properties, classProperties) {
  ...
  oj.Collection._justExtending = false; // Add regular properties from this "parent"
  ...
  Collection.prototype = obj; // Allow extending resulting obj
...
oj.Collection._init = function (collection, models, options, properties) {
  ...
  oj.Events.Mixin(collection); // First, copy all properties passed in
  ...
  function resortAndFireEvents(collection, existingModel, modelFoundInCollection, newModel, resortDeferred) {
    ...
    if (added) {
      ...
      if (newModel) {
        ...
        modelFoundInCollection.TriggerInternal(silent, oj.Events.EventType.ADD, modelFoundInCollection, collection, resortOpt);
    ...
    return new Promise(function (allResolve, allReject) {
      ...
      currentStep.then(function () {
        if (addedModels.length > 0) {
          self.TriggerInternal(opt.silent, oj.Events.EventType.ALLADDED, self, addedModels, opt);
        }
...
oj.Collection.prototype._setRangeLocalFetch = function (start, count, placement, original, resolve, reject, fill, options) {
  var self = this;
...
oj.Collection.prototype._fetchInternal = function (options, placement, fillIn) {
  ...
  opt.success = function (response) {
    ...
    var silent = !!opt.silent;
    self.TriggerInternal(silent, oj.Events.EventType.SYNC, self, response, opt); // Call the caller's success callback, if specified
...
oj.Collection.prototype._fillInCollectionWithParsedData = function (data, placement, manageLRU, options) {
  var opt = options || {};
  var parse = opt.parse;
...
oj.Collection.prototype._setPagingReturnValues = function (response, options, data, fillIn) {
  ...
  this.hasMore = this._getHasMore(oj.Collection._getProp(customObj, response, 'hasMore'), this.offset, this.lastFetchSize, this.totalResults); // Adjust total results to account for the case where the server tells us there's no more data, and
    ...
    return new Promise(function (resolve, reject) {
      ...
      var opts = {
        ...
        error: function error(xhr, status, _error) {
          reject(oj.Collection._createRejectionError(xhr, status, _error, self, opt, true));
        }
      };
...
oj.Collection.prototype.include = oj.Collection.prototype.contains; // Only look on models already fetched
...
oj.Collection.prototype.GetCollectionFetchUrl = function (opt) {
  var url = oj.Model.GetPropValue(this, 'url'); // Adorn it with options, if any
...
oj.Model.extend = function (properties, classProperties) {
  ...
  var obj;
  obj = new oj.Model();
  oj.Model._justExtending = false; // Add regular properties from this "parent"
  // oj.Events.Mixin(obj, this.prototype);
...
oj.Model.prototype.clone = function () {
  ...
  c.attributes = oj.Model._cloneAttributes(this.attributes, null); // Remove the cid--this should be unique
...
oj.Model.prototype.hasChanged = function (attribute) {
  if (attribute !== undefined) {
    return oj.Model._hasProperties(this.changed) && Object.prototype.hasOwnProperty.call(this.changed, attribute);
  }
...
oj.Model.prototype.destroy = function (options) {
  ...
  callback = oj.Model._getSuccess(opts); // Grab the collection off the model in case we need to update
...
oj.Model._internalSync = function (method, model, opt) {
  var options = opt || {}; // If Model/Collection has OAuth object, then create Authorization header (see oj.RestImpl.addOptions)
...
oj.sync = function (method, model, options) {
  ...
  if (method.valueOf() === 'read') {
    if (model instanceof oj.Model) {
      ...
      restService = new oj.RestImpl(url, model);
      return _fireAndReturn(restService.getRecord(success, error, model.GetId(), tempOpt, oj.Model.GetContext(tempOpt, model)));
    } // Collection fetch
...
oj.OAuth.prototype.clientCredentialGrant = function () {
  var headers = {};
  var self = this;
  headers[self.accessTokenRequest.auth_header] = 'Basic ' + oj.OAuth._base64_encode(self.accessTokenRequest.client_id + ':' + self.accessTokenRequest.client_secret);
  $.ajax({
    type: 'POST',
    async: false,
...
oj.RestImpl._HEADER_PROP = 'headers'; // Add the properties in options to starter, if not already there
...
oj.RestImpl.prototype._getURL = function (operation, rootURL, customURL, recordID, context, options) {
  ...
  var retObj = {
    url: oj.Model.GetPropValue(null, rootURL),
    type: httpMethod.method
  };
