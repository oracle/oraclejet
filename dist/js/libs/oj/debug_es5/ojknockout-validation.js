...
oj.Object.createSubclass(oj.InvalidComponentTracker, oj.Object, 'oj.InvalidComponentTracker'); // DOCLETS for public properties
...
oj.InvalidComponentTracker.prototype.Init = function () {
  oj.InvalidComponentTracker.superclass.Init.call(this); // INTERNAL PROPERTIES
...
oj.InvalidComponentTracker.prototype._update = function (component, element, option, value) {
  ...
  switch (option) {
        ...
        if (!compValid) {
          if (oj.InvalidComponentTracker._hasInvalidMessages(value)) {
            ...
            if (option === oj.InvalidComponentTracker._OPTION_MESSAGES_SHOWN) {
              ...
              // disabled.
              var isDisabled = component.call(component, 'option', oj.InvalidComponentTracker._OPTION_DISABLED);
              var isReadOnly = component.call(component, 'option', oj.InvalidComponentTracker._OPTION_READONLY);
    ...
    case oj.InvalidComponentTracker._OPTION_READONLY:
      ...
      if (value) {
        mutated = this._updateInvalidTracker(oj.InvalidComponentTracker._OPTION_MESSAGES_SHOWN, trackedIndex || 0, false);
        mutated = this._updateInvalidTracker(oj.InvalidComponentTracker._OPTION_MESSAGES_HIDDEN, trackedIndex || 0, false) || mutated;
...
oj.ValueBinding._ATTRIBUTE_INVALID_COMPONENT_TRACKER = 'invalidComponentTracker'; // A listener is added for this event to listen to changes to the 'messagesHidden' or
// 'messagesShown' options. The listener updates the InvalidComponentTracker.

oj.ValueBinding._EVENT_OPTIONCHANGE = 'ojoptionchange'; // Options we are interested in listening to changes for.

oj.ValueBinding._OPTION_MESSAGES_SHOWN = 'messagesShown';
oj.ValueBinding._OPTION_MESSAGES_HIDDEN = 'messagesHidden'; // options that are managed primarily to detect changes for tracker to be notified.

oj.ValueBinding._OPTION_DISABLED = 'disabled';
oj.ValueBinding._OPTION_READONLY = 'readOnly'; // callback called when managed attribute is being updated
...
oj.ValueBinding._updateInvalidComponentTracker = function (event) {
  ...
  if (option === oj.ValueBinding._OPTION_MESSAGES_SHOWN || option === oj.ValueBinding._OPTION_MESSAGES_HIDDEN) {
    if (ictObs && ko.isWriteableObservable(ictObs)) {
...
oj.ValueBinding._registerInvalidComponentTrackerWriteback = function (property, options, element, component) {
  ...
  if (ko.isObservable(ictObs)) {
    ...
    throw new Error('Binding attribute ' + oj.ValueBinding._ATTRIBUTE_INVALID_COMPONENT_TRACKER + ' should be bound to a ko observable.');
  ...
  if (ko.isWriteableObservable(ictObs)) {
    ...
    var messagesShown = component.call(component, 'option', oj.ValueBinding._OPTION_MESSAGES_SHOWN); // EditableValue's messagesHidden option is an Array of messages currently hidden on component

    var messagesHidden = component.call(component, 'option', oj.ValueBinding._OPTION_MESSAGES_HIDDEN);

    icTracker._update.call(icTracker, component, element, oj.ValueBinding._OPTION_MESSAGES_SHOWN, messagesShown);

    icTracker._update.call(icTracker, component, element, oj.ValueBinding._OPTION_MESSAGES_HIDDEN, messagesHidden);
  ...
  jElem.on(oj.ValueBinding._EVENT_OPTIONCHANGE, eventData, oj.ValueBinding._updateInvalidComponentTracker);
};
/**
...
oj.ComponentBinding.getDefaultInstance().setupManagedAttributes({
  for: 'editableValue',
  attributes: [oj.ValueBinding._ATTRIBUTE_INVALID_COMPONENT_TRACKER, oj.ValueBinding._OPTION_DISABLED, oj.ValueBinding._OPTION_READONLY],
