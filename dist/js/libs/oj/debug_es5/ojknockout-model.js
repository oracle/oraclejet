 ...
 * @ojtsimport knockout
 */
oj.KnockoutUtils = function () {}; // mapping variable definition, used in a no-require environment. Maps the oj.KnockoutUtils object to the name used in the require callback.
...
oj.KnockoutUtils.map = function (m, callback, array) {
  ...
  if (m instanceof Model.Collection) {
    ...
    var updateCollection = function updateCollection(changes) {
      try {
        if (!koObject[oj.KnockoutUtils.underUpdateProp]) {
          koObject[oj.KnockoutUtils.collUpdatingProp] = true; // Process change information
          ...
          if (m.comparator) {
            koObject[oj.KnockoutUtils.underUpdateProp] = true; // These could have been resorted--need to alter the observable array
          ...
          if (_index !== undefined && _index > -1) {
            ...
            if (options.fillIn) {
              ...
              for (var ii = currLen; ii < _index; ii++) {
                koObject.splice(ii, 0, oj.KnockoutUtils.map(collection._atInternal(ii, null, true, false), callback));
    ...
    var updateObservableArrayReset = function updateObservableArrayReset(collection) {
      try {
        ...
        if (collection instanceof Model.Collection) {
          koObject[oj.KnockoutUtils.underUpdateProp] = true; // koObject.splice(0, koObject().length);
    ...
    var updateObservableArraySort = function updateObservableArraySort(collection) {
      try {
        ...
        if (collection instanceof Model.Collection) {
          koObject[oj.KnockoutUtils.underUpdateProp] = true; // Redo the knockout observable array
    ...
    oj.KnockoutUtils._storeOriginalObject(koObject, m); // Activate user callback if specified, to allow user computed observables, etc.
...
oj.KnockoutUtils._callSort = function (a, b, comparator, collection, caller) {
  return Model.Collection.SortFunc(oj.KnockoutUtils._getModel(a), oj.KnockoutUtils._getModel(b), comparator, collection, caller);
