...
(function (dvt) {
  ...
  dvt.PictoChart.prototype.Init = function (context, callback, callbackObj) {
    dvt.PictoChart.superclass.Init.call(this, context, callback, callbackObj); // Create the event handler and add event listeners
  ...
  DvtPictoChartItem.prototype.getDatatip = function (target) {
    ...
    if (name) tds.push(dvt.HtmlTooltipManager.createElement('td', options['_tooltipLabelStyle'], name));
  ...
  DvtPictoChartRenderer.render = function (picto, container, availSpace, info) {
    ...
    var isOriginBottom = DvtPictoChartRenderer.isOriginBottom(picto);
    var isOriginRight = DvtPictoChartRenderer.isOriginRight(picto); // Use a 2D cell map to keep track of which cells are occupied
  ...
  DvtPictoChartRenderer.renderEmptyText = function (picto, container, availSpace) {
    ...
    var emptyText = dvt.TextUtils.renderEmptyText(container, emptyTextStr, availSpace.clone(), picto.getEventManager(), options['_statusMessageStyle']);
