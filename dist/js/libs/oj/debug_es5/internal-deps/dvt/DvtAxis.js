...
(function (dvt) {
  ...
  dvt.DataAxisInfoMixin = function (context, options, availSpace) {
    ...
    this._calcAxisExtents = function () {
      ...
      var scaleUnit = this._calcAxisScale(this.LinearGlobalMin != null ? this.LinearGlobalMin : this._dataMin, this.LinearGlobalMax != null ? this.LinearGlobalMax : this._dataMax);

      scaleUnit = Math.max(scaleUnit, this._minMajorIncrement); // If there's only a single value on the axis, we need to adjust the
      ...
      if (this.LinearGlobalMin == null) {
        ...
        if (this._dataMin >= 0) this.LinearGlobalMin = Math.max(this.LinearGlobalMin, 0);
      ...
      if (this.LinearMaxValue == null) this.LinearMaxValue = this.LinearGlobalMax; // Recalc the scale unit if the axis viewport is limited
  ...
  dvt.Obj.createSubclass(dvt.LinearScaleAxisValueFormatter, dvt.Obj); // Allowed scales that can be used as formatter scale param values
  ...
  dvt.LinearScaleAxisValueFormatter.prototype.InitScales = function () {
    ...
    createScale.call(this, dvt.LinearScaleAxisValueFormatter.SCALE_QUADRILLION, 5 * diff, 'labelScalingSuffixQuadrillion'); // sort _scalesOrder array
  ...
  dvt.LinearScaleAxisValueFormatter.prototype._getScaleFactor = function (scaleName) {
    ...
    if (!scaleName) scaleName = dvt.LinearScaleAxisValueFormatter.SCALE_AUTO;
    var scaleFactor,
  ...
  dvt.LinearScaleAxisValueFormatter.prototype.format = function (value, converter) {
    ...
    if (typeof parsed == 'number') {
      ...
      } else if (converter && converter['format']) parsed = converter['format'](parsed);else {
        var defaultConverter = this._context.getNumberConverter({
          'minimumFractionDigits': this._decimalPlaces,
          'maximumFractionDigits': this._decimalPlaces
        });

        if (defaultConverter && defaultConverter['format']) parsed = defaultConverter['format'](parsed);else if (this._useAutoPrecision) {
