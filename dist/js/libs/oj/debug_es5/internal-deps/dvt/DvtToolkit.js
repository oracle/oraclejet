...
dvt = function () {
  ...
  dvt.Obj.createSubclass = function (extendingClass, baseClass) {
  ...
  dvt.Context = function (container, id, referenceDiv) {
    ...
    this._parentDiv = container; // Save any application set aria role so we don't override.
    ...
    this._role = this._appAriaRole; // Store a reference to the div used for calculating the absolute position of the stage.

    this._referenceDiv = referenceDiv; // Create the defs, and stage

    this._defs = dvt.SvgShapeUtils.createElement('defs');
    ...
    this.Init(this._root, id); // Add the stage element

    dvt.ToolkitUtils.appendChildElem(this._root, this._stage.getElem()); // Since we only support JET in this toolkit version, we're providing a public oj reference
  ...
  dvt.Context.DEFAULT_FONT_SIZE = '12px';
  dvt.Context.ARIA_LABEL_STATE_DELIMITER = ', ';
  dvt.Context.ARIA_LABEL_DESC_DELIMITER = '. ';
  /**
  ...
  dvt.Context.prototype.Init = function (root, id) {
    ...
    this._scheduler = null; //: map of dvt.ClipPath objects stored by id
  ...
  dvt.Context.prototype.isReadyToRender = function () {
    ...
    if (this._parentDiv.offsetParent != null) return true;else {
      var computedStyle = window.getComputedStyle(this._parentDiv);
      if (computedStyle && computedStyle.position == 'fixed') return computedStyle.display != 'none';else return false;
    }
  };
  /**
  ...
  dvt.Context.prototype.increaseGlobalDefReference = function (id) {
    ...
    if (id == null) return;
    if (this._globalDefsMap[id]) this._globalDefsMap[id]++;else this._globalDefsMap[id] = 1;
  };
  /**
  ...
  dvt.Context.prototype.decreaseGlobalDefReference = function (id) {
    // The id of the element must be valid to continue
    if (id == null) return; // Update the reference count for this clip path id

    var refCount = this._globalDefsMap[id];

    if (refCount) {
      if (refCount == 1) delete this._globalDefsMap[id];else this._globalDefsMap[id] = refCount - 1;
  ...
  dvt.Context.prototype.getTooltipManager = function (id) {
    ...
    if (!manager) {
      this._tooltipManagers[id] = new dvt.HtmlTooltipManager(this, id);
    }
  ...
  dvt.Context.prototype.isOffscreen = function (noCache) {
    ...
    try {
      ...
      bOffscreen = rect.bottom < 0 || rect.right < 0 || rect.top > (window.innerHeight || document.documentElement.clientHeight) || rect.left > (window.innerWidth || document.documentElement.clientWidth);
    } catch (e) {
  ...
  dvt.Context.prototype.setActiveElement = function (displayable) {
    ...
    displayable.applyAriaProperties(); // Update the active descendant to point to the displayable
  ...
  dvt.Context._generateActiveElementId = function (displayable) {
    ...
    var dispId = displayable.getId(); // Use the displayable ID as a suffix if it's a string or a number
  ...
  dvt.Context.prototype.setAriaLabel = function (ariaLabel) {
    ...
    if (!this._parentDiv.getAttribute('aria-label')) {
      if (ariaLabel) this._parentDiv.setAttribute('aria-label', dvt.TextUtils.processAriaLabel(ariaLabel));else this._parentDiv.removeAttribute('aria-label');
    }
  };
  /**
  ...
  dvt.Context.prototype.setAriaRole = function (role) {
    if (!this._appAriaRole) {
      if (role) this._parentDiv.setAttribute('role', role);else this._parentDiv.removeAttribute('role');
      this._role = role;
    }
  };
  /**
  ...
  dvt.Context.requestAnimationFrame = function (callback) {
    ...
    if (dvt.Agent.isTouchDevice() && dvt.Agent.browser === 'chrome') return window.setTimeout(callback, 1000 / 60);else return window.requestAnimationFrame(callback);
  };
  /**
  ...
  dvt.Context.cancelAnimationFrame = function (requestId) {
    if (dvt.Agent.isTouchDevice() && dvt.Agent.browser === 'chrome') clearTimeout(requestId);else window.cancelAnimationFrame(requestId);
  };
  /**
  ...
  var DvtScheduler = function DvtScheduler(context) {
    this.Init(context);
  };
  ...
  DvtScheduler.prototype.HandleTimer = function () {
    ...
    } else this._animationRequestId = dvt.Context.requestAnimationFrame(dvt.Obj.createCallback(this, this.HandleTimer)); // Private flag for animation testing. Do not remove.
  ...
  var _STATE_BEGIN = 1;
  var _STATE_RUN = 2; // Type Constants
  ...
  dvt.Animator.TYPE_POLYLINE = 'typePolyline'; // PUBLIC API
  ...
  dvt.Animator.prototype.setEasing = function (easing) {
  ...
  dvt.Animator.prototype.getOnEnd = function () {
  ...
  dvt.Animator.prototype.setOnEnd = function (onEnd, onEndObj) {
    this._onEnd = onEnd;
  ...
  dvt.Animator.prototype.getOnInit = function () {
  ...
  dvt.Animator.prototype.setOnInit = function (onInit, onInitObj) {
    this._onInit = onInit;
  ...
  dvt.Animator.prototype.isRunning = function () {
  ...
  dvt.Animator.prototype.play = function (bImmediate) {
    if (bImmediate) this._play();else // Play after a quick timeout allowing the browser to render the bulk of the DOM and any subsequent components.
  ...
  dvt.Animator.prototype.processTime = function (time) {
    if (!this._bRunning) {
      return;
    }
  ...
  dvt.Animator.prototype.stop = function (bJumpToEnd) {
    if (this._animationRequestId) {
  ...
  dvt.Animator.prototype._getPropItem = function (obj, getter) {
  ...
  dvt.Animator.prototype._setPropItem = function (obj, getter, item) {
    var objMap = this._props.get(obj);
  ...
  dvt.Animator.prototype._getTotalDuration = function () {
  ...
  dvt.Animator.prototype._play = function () {
    if (!this._bRunning) {
      this._bRunning = true;
  ...
  dvt.Animator.prototype._processEnd = function () {
  ...
  dvt.Animator.prototype._processInitialize = function () {
    if (this._onInit) {
  ...
  dvt.Animator.prototype._processPlay = function () {
  ...
  dvt.Animator.prototype._processStep = function (progress) {
    ...
    if (prog >= 0) {
      ...
      this._props.forEach(function (objMap, obj) {
        objMap.forEach(function (item, getter) {
          var interpVal = DvtInterpolator.interpolate(ctx, item.type, item.startVal, item.destVal, prog);
  ...
  var DvtAnimatorPropItem = function DvtAnimatorPropItem(type, setter, destVal) {
    this.type = type;
    this.setter = setter;
  ...
  dvt.Playable.appendOnEnd = function (playable, onEnd, onEndObj) {
    ...
    if (!arOnEnd || !arOnEnd[0]) {
      ...
      var newOnEnd = function newOnEnd() {
  ...
  dvt.Playable.prototype.OnEndUpdated = function () {//subclasses must override
  };
  /**
  ...
  dvt.Playable.prototype.initialize = function () {//subclasses must override if they are supporting the bImmediate flag in play()
  };
  /**
    * Play this item.
    * @param {boolean} bImmediate true to begin the animation immediately.  This should generally be false when used by
  ...
  dvt.Playable.prototype.play = function (bImmediate) {//subclasses must override
  };
  /**
    * Stop this playable.
    */
  ...
  dvt.Playable.prototype.stop = function (bJumpToEnd) {//subclasses must override
  };
  /**
   * @license
   * Copyright (c) 2011 %CURRENT_YEAR%, Oracle and/or its affiliates.
  ...
  dvt.ParallelPlayable = function (context) {
    ...
    if (arguments && arguments.length > 1 && arguments[1] instanceof Array) {
      ...
      arPlayables = [].slice.call(arguments); //remove the context from the arPlayables array
  ...
  dvt.ParallelPlayable.prototype.getDuration = function () {
    ...
    for (var i = 0; i < this._arPlayables.length; i++) {
      ...
      if (playable instanceof dvt.Playable) duration = Math.max(duration, playable.getDuration());
    }
  ...
  dvt.ParallelPlayable.prototype.initialize = function () {
    for (var i = 0; i < this._arPlayables.length; i++) {
      if (this._arPlayables[i] instanceof dvt.Playable) this._arPlayables[i].initialize();
    }
  };
  /**
    * @override
    ...
    if (!this._bStarted) {
      for (var i = 0; i < this._arPlayables.length; i++) {
        ...
        if (playable instanceof dvt.Playable) {
          this._runningCounter++; //call internal onEnd function when each Playable ends
  ...
  dvt.ParallelPlayable.prototype.OnPlayableEnd = function () {
    ...
    this._runningCounter--; //if no more running, call the onEnd function for this ParallelPlayable
  ...
  dvt.SequentialPlayable.prototype.getDuration = function () {
    ...
    for (var i = 0; i < this._arPlayables.length; i++) {
      var playable = this._arPlayables[i];
      if (playable instanceof dvt.Playable) duration += playable.getDuration();
    }
  ...
  dvt.SequentialPlayable.prototype.initialize = function () {
    if (this._arPlayables.length > 0 && this._arPlayables[0] instanceof dvt.Playable) this._arPlayables[0].initialize();
  };
  /**
    * @override
    */
  ...
  dvt.SequentialPlayable.prototype._play = function () {
    ...
    var firstPlayable;
    if (this._arPlayables[0] instanceof dvt.Playable) firstPlayable = this._arPlayables[0];
    var lastPlayable;
    if (this._arPlayables[this._arPlayables.length - 1] instanceof dvt.Playable) lastPlayable = this._arPlayables[this._arPlayables.length - 1];
    var playable;
    ...
    if (lastPlayable) dvt.Playable.appendOnEnd(lastPlayable, this.DoEnd, this);
    this._bStarted = true;
    this._currIndex = 0; // Perform the animation immediately since this is not part of a slow component render operation.
  ...
  dvt.Obj.createSubclass(dvt.BlackBoxAnimationHandler, dvt.Obj); // Black Box Animation Types
  ...
  dvt.BlackBoxAnimationHandler.isSupported = function (type) {
    return type == dvt.BlackBoxAnimationHandler.ALPHA_FADE || type == dvt.BlackBoxAnimationHandler.SLIDE_TO_RIGHT || type == dvt.BlackBoxAnimationHandler.SLIDE_TO_LEFT || type == dvt.BlackBoxAnimationHandler.ZOOM;
  };
  /**
  ...
  dvt.BlackBoxAnimationHandler.getInAnimation = function (context, type, objs, bounds, duration) {
    if (type == dvt.BlackBoxAnimationHandler.ALPHA_FADE) return new dvt.AnimFadeIn(context, objs, duration);else if (type == dvt.BlackBoxAnimationHandler.SLIDE_TO_RIGHT) {
      ...
      return new dvt.AnimMoveBy(context, objs, new dvt.Point(-bounds.w, 0), duration);
    } else if (type == dvt.BlackBoxAnimationHandler.ZOOM) return new dvt.AnimScaleFadeIn(context, objs, bounds, dvt.BaseAnimation.DIR_C, 0.5, duration);else return null;
  };
  /**
  ...
  dvt.BlackBoxAnimationHandler.getCombinedAnimation = function (context, type, outObjs, inObjs, bounds, duration) {
    if (type == dvt.BlackBoxAnimationHandler.ALPHA_FADE) return new DvtCombinedAnimFade(context, outObjs, inObjs, duration);else if (type == dvt.BlackBoxAnimationHandler.SLIDE_TO_RIGHT) {
      ...
      return new dvt.CombinedAnimMoveBy(context, outObjs, inObjs, new dvt.Point(-bounds.w, 0), new dvt.Point(-bounds.w, 0), duration);
    } else if (type == dvt.BlackBoxAnimationHandler.ZOOM) return new DvtCombinedAnimScaleFade(context, outObjs, inObjs, bounds, dvt.BaseAnimation.DIR_C, dvt.BaseAnimation.DIR_C, 0.5, duration);else return null;
  };
  /**
  ...
  dvt.Obj.createSubclass(dvt.DataAnimationHandler, dvt.Obj); // TODO Document the expected interface for animatable objects:
  // getId()
  ...
  dvt.DataAnimationHandler.prototype.constructAnimation = function (oldList, newList) {
    ...
    newList = newList.slice(0); // Loop through the two lists and diff the changes.
    ...
    if (oldList) {
      ...
      for (var newIndex = 0; newIndex < newList.length; newIndex++) {
        ...
        if (!newItem || !newItem.getId()) //newItem and newItem ID must exist for animateMap update
          continue;
    ...
    var updates = new this._context.KeySetImpl(updatesArray); // All remaining objects in newList are inserts
  ...
  dvt.DataAnimationHandler.prototype.add = function (playable, index) {
    if (!playable) return;
    if (!index) index = 0; // Make sure the playables array is large enough
  ...
  dvt.DataAnimationHandler.prototype.getAnimation = function (bOverlap) {
    ...
    if (bOverlap) {
      ...
      for (var phaseIndex = 0; phaseIndex < playables.length; phaseIndex++) {
        ...
        } else phases.push(playables[phaseIndex]); // Update the startTime for the next phase
    ...
    } else return new dvt.SequentialPlayable(this._context, playables);
  };
  /**
  ...
  var DvtInterpolator = function DvtInterpolator() {
    this.Init();
  };
    ...
    for (var i = 0; i < destLength; i++) {
      ...
      if (accumLength + currSegLength > partialLength) {
        ...
        switch (cmd) {
          ...
          case 'C':
            array.push(prevX + ratio * (cpx1 - prevX), prevY + ratio * (cpy1 - prevY), prevX + ratio * (cpx2 - prevX), prevY + ratio * (cpy2 - prevY));
            break;
        }
  ...
  DvtInterpolator.InterpolatePath = function (context, origVal, destVal, progress) {
    var origLength = origVal.length;
    var destLength = destVal.length; //TO DO:
  ...
  DvtInterpolator.InterpolateStroke = function (context, origVal, destVal, progress) {
    if (origVal instanceof dvt.Stroke && destVal instanceof dvt.Stroke) {
      var color = DvtInterpolator.InterpolateColor(context, origVal.getColor(), destVal.getColor(), progress);
      var alpha = DvtInterpolator.InterpolateNumber(context, origVal.getAlpha(), destVal.getAlpha(), progress);
      var width = DvtInterpolator.InterpolateNumber(context, origVal.getWidth(), destVal.getWidth(), progress); //clone the destVal because it may have other properties set that
      //we can't interpolate between

      return new dvt.Stroke(color, alpha, width, destVal.isFixedWidth(), destVal.getDashProps(), destVal.getLineProps());
    }
  ...
  DvtInterpolator.prototype.Init = function () {}; //need to define the map after the functions are defined above
  ...
  dvt.BaseAnimation.DIR_NW = 9; //axis constants
  ...
  dvt.BaseAnimation.AXIS_Y = 2; //rotation directions
  ...
  dvt.BaseAnimation.prototype.initialize = function () {
    if (!this._bInitialized) {
      ...
      this._initializeEndStates(this._arObjects); //initialize the display objects appropriately for the start of the animation
  ...
  dvt.BaseAnimation.prototype.setEasing = function (easing) {
    this._easing = easing;
    if (this._animator) this._animator.setEasing(dvt.BaseAnimation.GetEasingFunction(this._easing));
  };
  /**
    * @protected
  ...
  dvt.BaseAnimation.prototype.InitializePlay = function () {//for subclasses to implement
  };
  /**
  ...
  dvt.BaseAnimation.prototype.InitStartState = function (obj) {//subclasses must implement
  };
  /**
    * @protected
  ...
  dvt.BaseAnimation.prototype.InitEndState = function (obj) {//subclasses must implement
  };
  /**
   * @protected
  ...
  dvt.AnimScaleFadeIn.prototype.Init = function (context, objs, boundsRect, direction, minScale, duration, delay) {
    ...
    dvt.AnimScaleFadeIn.superclass.Init.call(this, context, objs, duration, delay); //need to do this AFTER calling superclass Init because member will
  ...
  dvt.AnimScaleFadeIn.prototype.InitStartState = function (obj) {
    ...
    var newScaleY = this._minScale * obj.getScaleY(); //move the dispObj to the edge of the bounding rect and scale it down
  ...
  dvt.AnimPopIn.prototype.Init = function (context, objs, bCenter, duration, delay) {
    ...
    if (this._bCenter) {
      ...
      this._tempContainers = []; //current index in array of temp containers
  ...
  dvt.AnimPopIn.prototype.setEasing = function (easing) {//do nothing because we want to use our own easing
  };
  /**
    * @protected
    * @override
  ...
  dvt.AnimPopIn.prototype.InitStartState = function (obj) {
    if (this._bCenter) {
      ...
      var dims = obj.getDimensions(); //store the original translate on the temp container

      tempContainer._dvtAnimPopInOrigTx = obj.getTranslateX();
      tempContainer._dvtAnimPopInOrigTy = obj.getTranslateY(); //calculate the center point of the obj

      var dx = dims.x + .5 * dims.w;
      var dy = dims.y + .5 * dims.h; //position the origin of the temp container at the center point of the obj
      //so that the obj appears to scale from its center

      tempContainer.setTranslate(tempContainer._dvtAnimPopInOrigTx + dx, tempContainer._dvtAnimPopInOrigTy + dy); //translate the obj so that its center point is at the origin of the temp container

      obj.setTranslate(-dx, -dy); //reparent the obj to the temp container, and insert the temp container into
      ...
      tempContainer.addChild(obj); //scale the temp container instead of the obj
  ...
  dvt.AnimPopIn.prototype.InitEndState = function (obj) {
    if (this._bCenter) {
      ...
      this._tempContainers.push(tempContainer); //scale the temp container to full size
  ...
  var DvtCombinedAnimFade = function DvtCombinedAnimFade(context, outObjs, inObjs, duration, delay) {
  ...
  DvtCombinedAnimFade.prototype.Init = function (context, outObjs, inObjs, duration, delay) {
    ...
    if (!delay) delay = 0;
    var fadeOut = new dvt.AnimFadeOut(context, outObjs, duration, delay); //delay the fade out by a little bit to make both animations easier to see
  ...
  var DvtCombinedAnimScaleFade = function DvtCombinedAnimScaleFade(context, outObjs, inObjs, boundsRect, outDirection, inDirection, minScale, duration, delay) {
  ...
  DvtCombinedAnimScaleFade.prototype.Init = function (context, outObjs, inObjs, boundsRect, outDirection, inDirection, minScale, duration, delay) {
    if (!outDirection) outDirection = dvt.BaseAnimation.DIR_NE;
    if (!inDirection) inDirection = dvt.BaseAnimation.DIR_NW;
  ...
  dvt.ColorUtils = {};
  dvt.Obj.createSubclass(dvt.ColorUtils, dvt.Obj); //  Channel definition constants
  ...
  dvt.ColorUtils._ALPHA = 3; //  private constants
  ...
  dvt.ColorUtils._getChannel = function (chan, c) {
    var clr = c;
    ...
    if (namedColor) clr = namedColor; // Alpha support

    if (chan === dvt.ColorUtils._ALPHA) {
      if (clr.charAt(0) === '#') return clr.length > 7 ? parseInt(clr.substring(1, 3), 16) / 255 : 1; //check for MT extended format of #aarrggbb
    ...
    if (x1 < 0) {
      ar = []; //: parse channels for different # formats
  ...
  dvt.ColorUtils.getDarker = function (color, factor) {
    ...
    var a = dvt.ColorUtils.getAlpha(color);
  ...
  dvt.ColorUtils.getRGB = function (s) {
    var ret; // If clr is a named color, then convert into usable format
    ...
    if (s.charAt(0) === '#') {
      ...
      if (len > 7) {
        // alpha specified (#aarrggbb) - middle-tier extended format
        ret += parseInt(s.substring(3, 5), 16) + ',' + parseInt(s.substring(5, 7), 16) + ',' + parseInt(s.substring(7, 9), 16);
      } else if (len === 7) {
        // alpha not specified (#rrggbb)
        ret += parseInt(s.substring(1, 3), 16) + ',' + parseInt(s.substring(3, 5), 16) + ',' + parseInt(s.substr(5), 16);
      } else if (len === 4) {
        // #rgb
  ...
  dvt.ColorUtils.getRGBA = function (s) {
    ...
    if (s.charAt(0) === '#') {
      ...
      if (len > 7) {
        ...
        ret += parseInt(s.substring(3, 5), 16) + ',' + parseInt(s.substring(5, 7), 16) + ',' + parseInt(s.substring(7, 9), 16) + ',' + parseInt(s.substring(1, 3), 16) / 255;
      } else if (len === 7) {
        // alpha not specified (#rrggbb)  - alpha of 1 is assumed
        ret += parseInt(s.substring(1, 3), 16) + ',' + parseInt(s.substring(3, 5), 16) + ',' + parseInt(s.substr(5), 16) + ',1';
      } else if (len === 5) {
        // #rrgg
        ret += parseInt(s.substring(1, 3), 16) + ',' + parseInt(s.substring(3, 5), 16) + ',0,1';
      } else if (len === 4) {
  ...
  dvt.ColorUtils._setChannel = function (chan, s, chval) {
    ...
    if (bPound) {
      ...
      ret = bRGBA ? dvt.ColorUtils.makeRGBA(ar[0], ar[1], ar[2], ar[3]) : dvt.ColorUtils.makeRGB(ar[0], ar[1], ar[2]);
    }
  ...
  props.forEach(function (prop) {
    dvt.ColorUtils['get' + prop] = dvt.ColorUtils._getChannel.bind(null, dvt.ColorUtils['_' + prop.toUpperCase()]);
    dvt.ColorUtils['set' + prop] = dvt.ColorUtils._setChannel.bind(null, dvt.ColorUtils['_' + prop.toUpperCase()]);
  });
  /**
  ...
  dvt.ColorUtils.rgb2hsl = function (r, g, b) {
    ...
    if (max == min) {
      ...
      if (max == r) h = (g - b) / diff + (g < b ? 6 : 0);else if (max == g) h = (b - r) / diff + 2;else if (max == b) h = (r - g) / diff + 4;
      h /= 6;
    }
  ...
  dvt.ColorUtils.adjustHSL = function (color, dh, ds, dl) {
    ...
    if (!dvt.ColorUtils._hslCache) dvt.ColorUtils._hslCache = new dvt.Cache(100); // Create the key and look for the cache entry.

    var key = color + ':' + dh + ':' + ds + ':' + dl;

    var ret = dvt.ColorUtils._hslCache.get(key);

    if (ret != null) return ret; // Break down in rgba

    var r = dvt.ColorUtils.getRed(color);
    var g = dvt.ColorUtils.getGreen(color);
    var b = dvt.ColorUtils.getBlue(color);
    var a = dvt.ColorUtils.getAlpha(color); // Get the color as HSL

    var hslColor = dvt.ColorUtils.rgb2hsl(r, g, b); // Add the changes, bounded to the min and max for each value

    var h = Math.min(Math.max(0, hslColor.h + dh), 360);
    var s = Math.min(Math.max(0, hslColor.s + ds), 1);
    var l = Math.min(Math.max(0, hslColor.l + dl), 1); // Convert back to rgba and return

    var rgbColor = dvt.ColorUtils.hsl2rgb(h, s, l);
    if (a == 1) ret = dvt.ColorUtils.makePound(rgbColor.r, rgbColor.g, rgbColor.b);else ret = dvt.ColorUtils.makeRGBA(rgbColor.r, rgbColor.g, rgbColor.b, a); // Store in the cache and return
  ...
  dvt.ClipPath._uniqueSeed = 0; //  Clipping path outlines
  ...
  dvt.Obj.createSubclass(dvt.SolidFill, dvt.Obj);
  /**
  ...
  dvt.SolidFill.invisibleFill = function () {
    if (!dvt.SolidFill._INVISIBLE_FILL) dvt.SolidFill._INVISIBLE_FILL = new dvt.SolidFill('rgba(0,0,0,0)');
  ...
  dvt.SolidFill.prototype.equals = function (fill) {
    ...
    return fill instanceof dvt.SolidFill && fill.getColor() == this.getColor() && fill.getAlpha() == this.getAlpha();
  };
  /**
   * @license
   * Copyright (c) %FIRST_YEAR% %CURRENT_YEAR%, Oracle and/or its affiliates.
  ...
  dvt.Stroke = function (color, alpha, width, fixedWidth, dashProps, lineProps) {
    this._color = color;
  ...
  dvt.Obj.createSubclass(dvt.Stroke, dvt.Obj);

  dvt.Stroke.prototype.getLineProps = function () {
    return this._lineProps;
  };

  dvt.Stroke.prototype.getDashProps = function () {
  ...
  dvt.Stroke.prototype.getAlpha = function () {
    return this._alpha;
  };
  /**
  ...
  dvt.Stroke.prototype.getColor = function () {
    return this._color;
  };
  /**
  ...
  dvt.Stroke.getDefaultDashProps = function (type, width) {
    if (type === 'solid') return null;
    width = Math.ceil(width);
    if (type == 'dashed') return {
      dashArray: '' + Math.max(6, width * 2)
    };
    if (type == 'dotted') return {
      dashArray: '' + Math.max(2, width)
    };
    return null;
  };
  /**
  ...
  dvt.Obj.createSubclass(dvt.Shadow, dvt.Obj);
  ...
  dvt.Obj.createSubclass(dvt.GradientFill, dvt.Obj);
  /**
  ...
  dvt.GradientFill.prototype.Init = function (arColors, arAlphas, arStops, arBounds) {
    if (!arAlphas) {
      arAlphas = [];
  ...
  dvt.GradientFill.prototype.equals = function (fill) {
    return fill instanceof dvt.GradientFill && dvt.ArrayUtils.equals(this.getColors(), fill.getColors()) && dvt.ArrayUtils.equals(this.getStops(), fill.getStops()) && dvt.ArrayUtils.equals(this.getAlphas(), fill.getAlphas()) && dvt.ArrayUtils.equals(this.getBounds(), fill.getBounds());
  };
  /**
   * @license
   * Copyright (c) %FIRST_YEAR% %CURRENT_YEAR%, Oracle and/or its affiliates.
  ...
  dvt.LinearGradientFill.prototype.equals = function (fill) {
    if (fill instanceof dvt.LinearGradientFill && fill.getAngle() == this.getAngle()) return dvt.LinearGradientFill.superclass.equals.call(this, fill);else return false;
  };
  /**
   * @license
   * Copyright (c) %FIRST_YEAR% %CURRENT_YEAR%, Oracle and/or its affiliates.
  ...
  dvt.PatternFill = function (pattern, fillColor, backgroundColor, matrix) {
  ...
  dvt.Obj.createSubclass(dvt.PatternFill, dvt.Obj);
  /**   @final @type {String}  */
  ...
  dvt.PatternFill.prototype.equals = function (fill) {
    return fill instanceof dvt.PatternFill && fill.getPattern() == this.getPattern() && fill.getColor() == this.getColor() && fill.getBackgroundColor() == this.getBackgroundColor();
  };
  /**
  ...
  dvt.PatternFill.prototype.getMatrix = function () {
    return this._matrix || new dvt.Matrix();
  };
  /**
  ...
  dvt.PatternFill._convertPatternValue = function (patternStr) {
    if (patternStr == 'smallDiagonalLeft') return dvt.PatternFill.SM_DIAG_UP_LT;else if (patternStr == 'largeDiagonalLeft') return dvt.PatternFill.LG_DIAG_UP_LT;else if (patternStr == 'smallDiagonalRight') return dvt.PatternFill.SM_DIAG_UP_RT;else if (patternStr == 'largeDiagonalRight') return dvt.PatternFill.LG_DIAG_UP_RT;else if (patternStr == 'smallCrosshatch') return dvt.PatternFill.SM_CROSSHATCH;else if (patternStr == 'largeCrosshatch') return dvt.PatternFill.LG_CROSSHATCH;else if (patternStr == 'smallChecker') return dvt.PatternFill.SM_CHECK;else if (patternStr == 'largeChecker') return dvt.PatternFill.LG_CHECK;else if (patternStr == 'smallTriangle') return dvt.PatternFill.SM_TRIANGLE_CHECK;else if (patternStr == 'largeTriangle') return dvt.PatternFill.LG_TRIANGLE_CHECK;else if (patternStr == 'smallDiamond') return dvt.PatternFill.SM_DIAMOND_CHECK;else if (patternStr == 'largeDiamond') return dvt.PatternFill.LG_DIAMOND_CHECK;else return patternStr;
  };
  /**
   * @license
   * Copyright (c) 2011 %CURRENT_YEAR%, Oracle and/or its affiliates.
  ...
  dvt.SelectionEffectUtils.createSelectingStroke = function (selColor) {
    ...
    return new dvt.Stroke(dvt.SelectionEffectUtils.getHoverBorderColor(selColor), 1, 2, false, null, {
      miterLimit: 1
    });
  };
  /**
   ...
   * @return {dvt.Stroke} a stroke for the focus rectangle
   * @protected
   */
  ...
  dvt.KeyboardFocusEffect.prototype.show = function () {
    if (this._focusEffect instanceof dvt.Shape) {
      if (!this._appendLast) this._container.addChildAt(this._focusEffect, 0);else this._container.addChild(this._focusEffect);
    }
  };

  dvt.KeyboardFocusEffect.prototype.hide = function () {
    if (this._focusEffect instanceof dvt.Shape) this._container.removeChild(this._focusEffect);
  };
  /**
   * @license
   * Copyright (c) 2011 %CURRENT_YEAR%, Oracle and/or its affiliates.
  ...
  var DvtSvgFilterContext = function DvtSvgFilterContext() {
    this.Init();
  };
  ...
  var DvtSvgFilterUtils = function DvtSvgFilterUtils() {};
  ...
  DvtSvgFilterUtils.createFilter = function (effects, svgDisplayable) {
    ...
    if (svgDisplayable) {
      ...
      if (boundsRect) {
        ...
        if (height == 0 || width == 0) {
          var stroke; //if displayable is not a shape, for example a container, then it doesn't have a stroke
  ...
  var DvtSvgGradientUtils = function DvtSvgGradientUtils() {};
  ...
  DvtSvgGradientUtils.createElem = function (grad, id) {
    var elemGrad = dvt.SvgShapeUtils.createElement('linearGradient', id);
    var i;
    ...
    for (i = 0; i < len; i++) {
      ...
      if (color) {
        ...
        if (alpha != null) dvt.ToolkitUtils.setAttrNullNS(elem, 'stop-opacity', alpha, 1);
      }
    ...
    var bUseObjBBox = !arBounds || arBounds[0] == 0 && arBounds[1] == 0 && arBounds[2] == 0 && arBounds[3] == 0; //  The angle of rotation for SVG is clockwise, so must convert from the standard
    ...
    if (bUseObjBBox) {
      ...
      dvt.ToolkitUtils.setAttrNullNS(elemGrad, 'gradientUnits', 'userSpaceOnUse'); // set gradient vector to span the middle of the unit square
  ...
  var DvtSvgPatternFillUtils = function DvtSvgPatternFillUtils() {};
  ...
  DvtSvgPatternFillUtils.CreatePatternElems = function (patternFill, parentElem) {
    ...
    if (pattern === dvt.PatternFill.SM_DIAG_UP_LT || pattern === dvt.PatternFill.LG_DIAG_UP_LT) {
      ...
      dvt.ToolkitUtils.setAttrNullNS(elem, 'd', dvt.PathUtils.moveTo(0, -h / 2) + dvt.PathUtils.lineTo(3 * w / 2, h) + dvt.PathUtils.moveTo(-w / 2, 0) + dvt.PathUtils.lineTo(w / 2, h));
      dvt.ToolkitUtils.appendChildElem(parentElem, elem);
    } else if (pattern === dvt.PatternFill.SM_DIAG_UP_RT || pattern === dvt.PatternFill.LG_DIAG_UP_RT) {
      elem = dvt.SvgShapeUtils.createElement('path');
      dvt.ToolkitUtils.setAttrNullNS(elem, 'stroke', color);
      dvt.ToolkitUtils.setAttrNullNS(elem, 'stroke-opacity', alpha);
      dvt.ToolkitUtils.setAttrNullNS(elem, 'stroke-width', 1);
      dvt.ToolkitUtils.setAttrNullNS(elem, 'stroke-linecap', 'square');
      dvt.ToolkitUtils.setAttrNullNS(elem, 'd', dvt.PathUtils.moveTo(-w / 2, h) + dvt.PathUtils.lineTo(w, -h / 2) + dvt.PathUtils.moveTo(0, 3 * h / 2) + dvt.PathUtils.lineTo(3 * w / 2, 0));
      dvt.ToolkitUtils.appendChildElem(parentElem, elem);
    } else if (pattern === dvt.PatternFill.SM_CROSSHATCH || pattern === dvt.PatternFill.LG_CROSSHATCH) {
      //use path instead of two lines because it's more compact (uses one
      //DOM element instead of two)
      elem = dvt.SvgShapeUtils.createElement('path'); //  g.lineStyle((bSmall? 1 : 2), color) ;

      dvt.ToolkitUtils.setAttrNullNS(elem, 'stroke', color);
      dvt.ToolkitUtils.setAttrNullNS(elem, 'stroke-opacity', alpha);
      dvt.ToolkitUtils.setAttrNullNS(elem, 'stroke-width', 1);
      dvt.ToolkitUtils.setAttrNullNS(elem, 'd', 'M0,0' + 'L' + rightX + ',' + bottomY + 'M' + rightX + ',0' + 'L0,' + bottomY);
      dvt.ToolkitUtils.appendChildElem(parentElem, elem);
    } else if (pattern === dvt.PatternFill.SM_CHECK || pattern === dvt.PatternFill.LG_CHECK) {
      halfW = w / 2;
      halfH = h / 2; //use path instead of two rects because it's more compact (uses one
      //DOM element instead of two)
      ...
      dvt.ToolkitUtils.setAttrNullNS(elem, 'd', 'M' + halfW + ',0' + 'L' + w + ',0' + 'L' + w + ',' + halfH + 'L' + halfW + ',' + halfH + 'Z' + 'M0,' + halfH + 'L' + halfW + ',' + halfH + 'L' + halfW + ',' + h + 'L0,' + h + 'Z');
      dvt.ToolkitUtils.appendChildElem(parentElem, elem);
    } else if (pattern === dvt.PatternFill.SM_TRIANGLE_CHECK || pattern === dvt.PatternFill.LG_TRIANGLE_CHECK) {
      ...
      dvt.ToolkitUtils.setAttrNullNS(elem, 'fill', color);
      dvt.ToolkitUtils.setAttrNullNS(elem, 'fill-opacity', alpha);
      dvt.ToolkitUtils.setAttrNullNS(elem, 'points', '0,' + bottomY + ' ' + rightX + ',0 ' + rightX + ',' + bottomY);
      dvt.ToolkitUtils.appendChildElem(parentElem, elem);
    } else if (pattern === dvt.PatternFill.SM_DIAMOND_CHECK || pattern === dvt.PatternFill.LG_DIAMOND_CHECK) {
      halfW = w / 2;
      halfH = h / 2;
      elem = dvt.SvgShapeUtils.createElement('polygon');
      dvt.ToolkitUtils.setAttrNullNS(elem, 'stroke', color);
      ...
      dvt.ToolkitUtils.setAttrNullNS(elem, 'fill', color);
      dvt.ToolkitUtils.setAttrNullNS(elem, 'fill-opacity', alpha);
      dvt.ToolkitUtils.setAttrNullNS(elem, 'points', '0,' + halfH + ' ' + halfW + ',0 ' + rightX + ',' + halfW + ' ' + halfW + ',' + bottomY);
      dvt.ToolkitUtils.appendChildElem(parentElem, elem);
    }
  };
  /**
  ...
  var DvtSvgShadowUtils = function DvtSvgShadowUtils() {};
  ...
  DvtSvgShadowUtils.createFilterPrimitives = function (filt, shadow, svgDisplayable, filtContext) {
    ...
    var origBlurY = shadow._blurY; //adjust blur values so that SVG behaves more like Flash
    ...
    if (boundsRect) {
      ...
      var absDistance = Math.abs(distance); //use two times the blur by default, because using it directly
    ...
    if (svgDisplayable instanceof dvt.Polyline) {
      var strokeWidth = svgDisplayable.getStroke().getWidth(); // If stroke is wider than 1 pixel, adjust y coordinate and height of shadow bounding box
    ...
    dvt.ToolkitUtils.setAttrNullNS(elemCM, 'values', '0 0 0 ' + red + ' 0 ' + '0 0 0 ' + green + ' 0 ' + '0 0 0 ' + blue + ' 0 ' + '0 0 0 ' + alpha * strength + ' 0'); //var elemCMResult = filtContext.createResultId('colorMatrix');
    //dvt.ToolkitUtils.setAttrNullNS(elemCM, 'result', elemCMResult) ;

    var elemGB = dvt.SvgShapeUtils.createElement('feGaussianBlur'); //include both numbers for stdDev when necessary
   ...
   *  construction, e.g. translate, skew, rotate, etc will return a new instance of a dvt.Matrix
  ...
  dvt.Obj.createSubclass(dvt.Matrix, dvt.Obj); //   [ a  b  tx ]
  //   [ c  d  ty ]
  //   [ 0  0  1  ]
  ...
  /**
   *  Returns a new dvt.Matrix which is a translation of this matrix to the given dx and dy values.
  ...
  dvt.Matrix.prototype.translate = function (dx, dy) {
    return this.concat(new dvt.Matrix(1, 0, 0, 1, dx, dy));
  };
  /**
   *  Returns a new dvt.Matrix which is a scale of this matrix, optionally around a specified point
  ...
  dvt.Matrix.prototype.scale = function (sx, sy, px, py) {
    ...
    var tMat = new dvt.Matrix(sx, 0, 0, sy);
  ...
  /**
   *  Returns a new dvt.Matrix which is a rotation of this matrix.
  ...
  dvt.Matrix.prototype.rotate = function (angleRads) {
    ...
    var tMat = new dvt.Matrix(cos, -sin, sin, cos);
    return this.concat(tMat);
  };
  /**
   *  Returns a new dvt.Matrix that is a skew of this matrix.
  ...
  dvt.Matrix.prototype.skew = function (sxRads, syRads) {
    var tMat = new dvt.Matrix(1, Math.tan(sxRads), Math.tan(syRads), 1);
    return this.concat(tMat);
  };
  /**
  ...
  dvt.Matrix.prototype._determinant = function () {
    var determinant = this._a * (this._d * this._w - this._ty * this._v) - this._b * (this._c * this._w - this._ty * this._u) + this._tx * (this._c * this._v - this._d * this._u);
    return determinant;
  };
  /**
   *  Returns a new dvt.Matrix that is an inversion of this matrix.
   *  @return {dvt.Matrix}
   */
  ...
  dvt.Matrix.prototype.invert = function () {
    ...
    return new dvt.Matrix(A / determinant, B / determinant, C / determinant, D / determinant, TX / determinant, TY / determinant);
  };
  /**
  ...
  dvt.Matrix.prototype._decompose = function () {
    ...
    var Tx = this._tx;
    var Ty = this._ty;
    if (A * D - B * C === 0) return null; //x scale factor
    ...
    var R = Math.atan2(C, A); //skew

    var skewX = Math.atan(K); //to create new matrix with same transforms, use order:
    //1) scale(Sx, Sy)
    //2) skew(skewX, 0);
    //3) rotate(R);
    //4) translate(Tx, Ty);
  ...
  dvt.Matrix.prototype._recompose = function (arDecomposition) {
    var mat = new dvt.Matrix(); // the identity matrix
  ...
  dvt.Matrix.prototype.equals = function (mat) {
    if (!this && mat) return false;else if (this && !mat) return false;else if (!this && !mat) return true;else return this._a === mat.getA() && this._b === mat.getB() && this._c === mat.getC() && this._d === mat.getD() && this._tx === mat.getTx() && this._ty === mat.getTy();
  };
  /**
  ...
  dvt.Point.prototype.equals = function (p) {
    if (p instanceof dvt.Point && this.x === p.x && this.y === p.y) return true;
    return false;
  };
  /**
   * @license
  ...
  dvt.Rectangle.prototype.getIntersection = function (rect) {
    ...
    if (rect && rect.w !== 0 && rect.h !== 0) {
      ...
      if (this.w !== 0 && this.h !== 0) {
        ...
        if (maxX >= thisRight || maxX >= rectRight || maxY >= thisBottom || maxY >= rectBottom) return null;
  ...
  dvt.Cache.prototype.Init = function (targetSize) {
    ...
    this._maxSize = Math.ceil(this._targetSize * (1 + dvt.Cache._BUFFER_SIZE)); // Initialize the cache and array of keys, where the first key is the least recently used.

    this._cache = {};
    this._lruArray = []; // Initialize debug params for cache tuning
  ...
  dvt.Cache.prototype.put = function (key, value) {
    ...
    var bKeyExists = this._cache[key] != null; // Update the cache first

    this._cache[key] = value; // Update the array tracking recently used items
    ...
    if (bKeyExists) {
      ...
      this._lruArray.push(key); // If the cache size exceeds the max length, then remove the least recently used items.
  ...
  dvt.DragAndDropUtils.getDragFeedbackBounds = function (displayables, targetCoordinateSpace) {
    ...
    if (!(displayables instanceof Array)) {
      ...
      bounds = displayables[0].getDimensions(targetCoordinateSpace ? targetCoordinateSpace : displayables[0].getParent()); // ... and adjust it to acommodate other objects in the collection
  ...
  */
  ...
  dvt.LayoutUtils.position = function (availSpace, position, displayable, width, height, gap) {
    if (!displayable) return; // Adjust the gap if the displayable has no area

    gap = width * height > 0 ? gap : 0;
    if (position == 'start') position = dvt.Agent.isRightToLeft(displayable.getCtx()) ? 'right' : 'left';else if (position == 'end') position = dvt.Agent.isRightToLeft(displayable.getCtx()) ? 'left' : 'right';
  ...
  dvt.LayoutUtils.align = function (availSpace, align, displayable, width) {
    if (!displayable) return; // Account for the locale and find the position

    var position = align;
    if (position == 'start') position = dvt.Agent.isRightToLeft(displayable.getCtx()) ? 'right' : 'left';else if (position == 'end') position = dvt.Agent.isRightToLeft(displayable.getCtx()) ? 'left' : 'right'; // Align the text

    if (position == 'left') displayable.setX(availSpace.x);else if (position == 'center') displayable.setX(availSpace.x + availSpace.w / 2 - width / 2);else if (position == 'right') displayable.setX(availSpace.x + availSpace.w - width);
  };
  /**
  ...
  dvt.LayoutUtils.getBubbleSize = function (z, minZ, maxZ, minSize, maxSize) {
    if (z <= 0) return 0; // Marker shape doesn't matter, so assume square markers

    var minArea = minSize * minSize;
    var maxArea = maxSize * maxSize; // If the z value range is less than the area range, reduce the are range to match the z value range by
    ...
    if (maxZ / minZ < maxArea / minArea) {
      ...
      var avgZ = Math.sqrt(minZ * maxZ); // Handle the case where z == minZ == maxZ
  ...
  dvt.PixelMap.prototype.Init = function (scale, innerMap) {
    dvt.PixelMap.superclass.Init.call(this); // Store the inner map and add the outer map reference back to this.
    ...
    this._scale = scale || 1;
    this._capacity = this._scale * this._scale; // Cache the presence of inner and outer maps, since they are checked frequently.
  ...
  dvt.PixelMap.prototype._increment = function (x, y, incr) {
    ...
    var scaledY = this._adjustForScale(y); // Increment the value. If value is greater than capacity, store as true (occupied).


    var value = this.get(scaledX, scaledY);
    value = value != null ? value + incr : incr;
    if (value >= this._capacity) value = true; // Update the map.

    this.put(scaledX, scaledY, value); // If at capacity, update the outer map.
  ...
  dvt.ImageLoader.loadImage = function (src, onComplete) {
    //first look for a cached copy of the image
    var entry = this._cache[src]; //if cached image found, use it

    if (entry) {
      // if image is loading, add listener to queue
      if (entry._image) {
        dvt.ImageLoader._addListenerToQueue(entry._listeners, onComplete);
  ...
  dvt.ImageLoader.loadNewImage = function (src, onComplete) {
    // create img element
    var image = document.createElement('img'); // add a new entry to the image cached
    ...
    if (onComplete) {
      ...
      dvt.ImageLoader._addListenerToQueue(newEntry._listeners, onComplete);
    }

    this._cache[src] = newEntry;
    dvt.ImageLoader.addBubbleEventListener(image, 'load', function (e) {
      // copy width and height to entry and delete image element
      newEntry.width = image.width;
      newEntry.height = image.height;
      delete newEntry._image; // notify all listeners image loaded and delete all listeners
  ...
  dvt.ImageLoader._addListenerToQueue = function (queue, listener) {
    if (listener) {
  ...
  dvt.JsonUtils.clone = function (obj, keyFunc, noClone) {
    ...
    if (obj instanceof Array) {
      ret = []; // Loop through and copy the Array

      for (var i = 0; i < obj.length; i++) {
        if (dvt.JsonUtils._isDeepClonable(obj[i])) // deep clone objects
          ret[i] = dvt.JsonUtils.clone(obj[i], keyFunc, noClone);else // copy values
          ret[i] = obj[i];
      }
      ...
      ret = {}; // Loop through all properties of the object

      for (var key in obj) {
        if (!keyFunc || keyFunc(key)) {
          var value = obj[key];

          if (!(noClone && noClone[key] == true) && dvt.JsonUtils._isDeepClonable(value) && key != '_widgetConstructor') {
            // deep clone objects
  ...
  dvt.JsonUtils.merge = function (a, b, noClone) {
    ...
    var one = a && a._dvtNoClone ? a : dvt.JsonUtils.clone(a, null, noClone);
    ...
    } else if (two == null) return one;else {
  ...
  dvt.JsonUtils._copy = function (a, b) {
    for (var key in a) {
      var value = a[key]; // Treat an 'undefined' value as if the key were not set and ignore
      ...
      if (value && value instanceof Array || key == '_widgetConstructor') {
        ...
        if (value instanceof dvt.CSSStyle) b[key].merge(value);else // value is String
        ...
        if (b[key]) dvt.JsonUtils._copy(value, b[key]);else b[key] = value;
      } else b[key] = value;
    }
  };
  /**
  ...
  dvt.JsonUtils._isDeepClonable = function (obj) {
    if (obj instanceof Object) {
      if (obj['has']) // key set
        return false;else if (obj._dvtNoClone) return false;
  ...
  dvt.JsonUtils._isClonableObject = function (obj) {
    if (obj instanceof Object) {
      if (obj.then) // Promise
        return false;else if (obj.jquery) // JQuery Object
        return false;else if (obj['has']) // key set
        return false;else if (typeof NodeList != 'undefined' && obj instanceof NodeList) // DOM NodeList
        return false;else if (typeof Node != 'undefined' && obj instanceof Node) // DOM objects
        return false;else if (obj._dvtNoClone) return false;else return true;
    }
  ...
  dvt.ToolkitUtils.createSvgDocument = function (id) {
    ...
    if (id != null) dvt.ToolkitUtils.setAttrNullNS(svg, 'id', id);
    dvt.ToolkitUtils.setAttrNullNS(svg, 'width', '100%');
    dvt.ToolkitUtils.setAttrNullNS(svg, 'height', '100%'); //  - IE allows tabbing into svg element

    if (dvt.Agent.browser === 'ie' || dvt.Agent.browser === 'edge') dvt.ToolkitUtils.setAttrNullNS(svg, 'focusable', 'false');
    return svg;
  };
  /**
  ...
  dvt.ToolkitUtils._getDragFeedbackSVG = function (displayables, bounds) {
    ...
    dvt.ToolkitUtils.setAttrNullNS(svgElem, 'class', 'oj-drag'); // Create a group element to apply the top level translate to show the drag feedback at (0, 0)
    ...
    dvt.ToolkitUtils.appendChildElem(svgElem, container); // Loop through and add the displayables

    for (var i = 0; i < displayables.length; i++) {
      var src = displayables[i]; // Clone the selected node and add

      var elem = src.getElem();
      var clone = elem.cloneNode(true); // Remove filters if present, since these effects overflow the bounds and look broken

      if (dvt.ToolkitUtils.getAttrNullNS(clone, 'filter')) dvt.ToolkitUtils.setAttrNullNS(clone, 'filter', null);
      ...
      for (var j = 1; j < pathToStage.length; j++) {
        if (!mat) mat = pathToStage[j].getMatrix();else {
  ...
  dvt.ToolkitUtils.removeAttrNullNS = function (elem, name) {
    ...
    if (dvt.ToolkitUtils.hasAttrNullNS(elem, name)) elem.removeAttribute(name);
  };
  /**
  ...
  dvt.ToolkitUtils.setHref = function (elem, src) {
    if (dvt.Agent.browser === 'safari' || dvt.Agent.browser === 'firefox' && dvt.Agent.version < 51) elem.setAttributeNS('http://www.w3.org/1999/xlink', 'xlink:href', src);else dvt.ToolkitUtils.setAttrNullNS(elem, 'href', src);
  };
  /**
  ...
  dvt.ToolkitUtils.getUrlPathById = function (id) {
    ...
    var hasBase = dvt.Agent.browser === 'ie' || dvt.Agent.browser === 'edge' ? document.querySelector('base') != null : document.URL != document.baseURI;
  ...
  dvt.ToolkitUtils.getImageUrl = function (context, className) {
    ...
    if (typeof $ == 'undefined' || className.indexOf('.') != -1) return className;
    var url = dvt.ToolkitUtils._IMAGE_URL_CACHE[className];
    if (url != null) return url; // Create a dummy CSS div, apply the class name, extract the background-image CSS, and then remove the dummy div.
    ...
    if (url && url.indexOf('url(') !== -1) url = url.slice(url.indexOf('url(') + 4, url.length - 1).replace(/["']/g, '');else url = className;
    cssDiv.remove();
  ...
  dvt.Obj.createSubclass(dvt.SvgDocumentUtils, dvt.Obj); // TODO JSDoc
  // Return displayable
  ...
  dvt.SvgDocumentUtils.addDragListeners = function (displayable, dragStartCallback, dragMoveCallback, dragEndCallback, callbackObj, bPreventClick) {
    ...
    var clickStaticCallback = function clickStaticCallback(event) {
      // Prevent click from occurring (e.g. to prevent clearing selection)
      event.stopPropagation();
    };

    var dragStartStaticCallback = function dragStartStaticCallback(dvtEvent) {
      if (dragStartCallback.call(callbackObj, dvtEvent)) {
        ...
        dvt.SvgDocumentUtils._mozUserSelect = bodyStyle.MozUserSelect; // Disable selection during drag
    ...
    var dragEndStaticCallback = function dragEndStaticCallback(event) {
      ...
      if (isTouch) {
        ...
        if (!isTouch) // Use timeout because the click event is fired after mouseup. We can't remove the listener in the
      ...
      bodyStyle.MozUserSelect = dvt.SvgDocumentUtils._mozUserSelect; // Convert the native event and call the callback
  ...
  dvt.SvgDocumentUtils.disableMouseEvents = function (elem) {
    if (elem.style) elem.style.pointerEvents = 'none'; // Recursively disable mouse events on the children
  ...
  dvt.ArrayUtils.equals = function (a, b) {
    if (!a && b) return false;else if (a && !b) return false;else if (!a && !b) return true;else // a && b
      {
        if (a.length != b.length) return false; // Compare the individual items
  ...
  dvt.ArrayUtils.hasAnyMapItem = function (map, items) {
    ...
    if (!map || !items) return false; // Look for each of the items
  ...
  dvt.ResourceUtils = {
     ...
     */
    format: function format(pattern, replacements) {
      return pattern ? pattern.replace(/\{(\d+)\}/g, function () {
  ...
  dvt.BaseEvent.prototype.Init = function (event) {
    this.type = event.type; // Find the dvt.Obj corresponding to the event target
  ...
  dvt.BaseEvent.FindDisplayable = function (target) {
    while (target) {
      ...
      if (target._obj) return target._obj;else // Otherwise look at the parent
  ...
  var DvtFocusEvent = function DvtFocusEvent(event) {
    this.Init(event);
  };
  ...
  DvtFocusEvent.prototype.Init = function (event) {
    ...
    this.view = event.view; // Find the dvt.Obj corresponding to the event target

    if (event.relatedTarget != null) this.relatedTarget = dvt.BaseEvent.FindDisplayable(event.relatedTarget);
  };
  /**
   * @license
   * Copyright (c) %FIRST_YEAR% %CURRENT_YEAR%, Oracle and/or its affiliates.
  ...
  dvt.Obj.createSubclass(dvt.KeyboardEvent, dvt.BaseEvent); // Constants for keyboard event types
  //: indirectly assign the value so the closure compiler will abbreviate references to the constant

  /** Key down event type **/

  dvt.KeyboardEvent.KEYDOWN = 'keydown';
  /** Key up event type **/

  dvt.KeyboardEvent.KEYUP = 'keyup';
  /** Key press event type **/

  dvt.KeyboardEvent.KEYPRESS = 'keypress';
  /** Tab key **/
  ...
  dvt.KeyboardEvent.isPlus = function (event) {
    ...
    if (keyCode == dvt.KeyboardEvent.NUMPAD_PLUS) return true;

    if (dvt.Agent.browser === 'firefox') {
      // special case for Gecko/Firefox
      if (keyCode == dvt.KeyboardEvent.GECKO_PLUS && event.shiftKey) return true;else return false;
    } else {
      if (keyCode == dvt.KeyboardEvent.WEBKIT_PLUS && event.shiftKey) return true;else return false;
    }
  ...
  dvt.KeyboardEvent.isMinus = function (event) {
    var keyCode = event.keyCode;
    if (keyCode == dvt.KeyboardEvent.NUMPAD_MINUS) return true;

    if (dvt.Agent.browser === 'firefox') {
      // special case for Gecko/Firefox
      if (keyCode == dvt.KeyboardEvent.GECKO_MINUS && !event.shiftKey) return true;else return false;
    } else {
      if (keyCode == dvt.KeyboardEvent.WEBKIT_MINUS && !event.shiftKey) return true;else return false;
    }
  ...
  dvt.Obj.createSubclass(dvt.MouseEvent, dvt.BaseEvent); // Constants for mouse event types
  //: indirectly assign the value so the closure compiler will abbreviate references to the constant

  /** @const **/

  dvt.MouseEvent.CLICK = 'click';
  /** @const **/

  dvt.MouseEvent.DBLCLICK = 'dblclick';
  /** @const **/

  dvt.MouseEvent.MOUSEOVER = 'mouseover';
  /** @const **/

  dvt.MouseEvent.MOUSEOUT = 'mouseout';
  /** @const **/

  dvt.MouseEvent.MOUSEDOWN = 'mousedown';
  /** @const **/

  dvt.MouseEvent.MOUSEUP = 'mouseup';
  /** @const **/

  dvt.MouseEvent.MOUSEMOVE = 'mousemove';
  /** @const **/

  dvt.MouseEvent.MOUSEWHEEL = 'wheel';
  /** @const **/
  ...
  dvt.MouseEvent.prototype.Init = function (event) {
    dvt.MouseEvent.superclass.Init.call(this, event); // Find the dvt.Obj corresponding to the event target

    if (event.relatedTarget != null) this.relatedTarget = dvt.BaseEvent.FindDisplayable(event.relatedTarget); // Copy the remaining information

    this.button = event.button;
    this.ctrlKey = event.ctrlKey || event.metaKey;
    this.shiftKey = event.shiftKey;
    this.pageX = event.pageX;
    this.pageY = event.pageY; //: Flag indicates if the event is modified for Internet Explorer

    this._isEventModifiedForIE = false;
    if (event.wheelDeltaY != null) this.wheelDelta = event.wheelDeltaY / 40;else if (event.deltaY != null) {
      this.deltaMode = event.deltaMode;
      this.deltaY = event.deltaY; // Approximate the wheel delta from the deltaY and deltaMode

      if (event.deltaMode == event.DOM_DELTA_LINE) this.wheelDelta = -event.deltaY;else if (event.deltaMode == event.DOM_DELTA_PIXEL) this.wheelDelta = -event.deltaY / 15;
    } else if (event.wheelDelta != null) this.wheelDelta = event.wheelDelta / 40;else this.wheelDelta = event.detail;
  };
  /**
  ...
  dvt.Obj.createSubclass(dvt.TouchEvent, dvt.BaseEvent); // Constants for touch event types
  //: indirectly assign the value so the closure compiler will abbreviate references to the constant

  /** @const **/

  dvt.TouchEvent.TOUCHSTART = 'touchstart';
  /** @const **/

  dvt.TouchEvent.TOUCHMOVE = 'touchmove';
  /** @const **/

  dvt.TouchEvent.TOUCHEND = 'touchend';
  /** @const **/

  dvt.TouchEvent.TOUCHCANCEL = 'touchcancel';
  /**
   * @param {TouchEvent} event The DOM TouchEvent
   * @protected
   */
  ...
  dvt.TouchEvent.prototype.Init = function (event) {
    dvt.TouchEvent.superclass.Init.call(this, event); // Convert touchcancel to touchend
  ...
  dvt.TouchManager.prototype.Init = function (id, context, eventManager) {
    this._context = context;
    this._id = id;
    this._eventManager = eventManager; // Total number of touches on the screen

    this._touchCount = 0; // Single timer for touch hold

    this._touchHoldTimer = new dvt.Timer(this._context, 200, this._handleTouchHoldStartTimer, this);
    this._blockTouchHold = false; // Stored mapping for history information on touches

    this._touchMap = new Object(); // Save information on touches which are already associated with listeners

    this._savedTouchInfo = new Array(); // internal listener to translate to hover

    this._addTouchHoldMoveEventListener(this._onTouchHoldHover, this); // Single timer for double tap
    ...
    this._doubleTapTimer = new dvt.Timer(this._context, 600, this._handleDoubleTapTimer, this, 1); // dvt.Touch event
  ...
  dvt.TouchManager.prototype._onTouchHoldHover = function (evt) {
    ...
    if (type == dvt.ComponentTouchEvent.TOUCH_HOLD_END_TYPE) {
      ...
      if (obj != null) this.FireListener(new dvt.ComponentTouchEvent(dvt.ComponentTouchEvent.TOUCH_HOVER_OUT_TYPE, touch, obj, null, nativeEvt));
      ...
      info[dvt.TouchManager.PREV_HOVER_OBJ] = null;
      if (obj != null) this.FireListener(new dvt.ComponentTouchEvent(dvt.ComponentTouchEvent.TOUCH_HOVER_OVER_TYPE, touch, obj, null, nativeEvt));
  ...
  dvt.TouchManager.prototype._getObjFromTouch = function (touch) {
    return touch ? dvt.SvgDocumentUtils.elementFromTouch(touch) : null;
  };
  /**
   * @private
  ...
  dvt.TouchManager.prototype._fireHoverOverOutEvents = function (currentObj, prevHoverObj, touch, nativeEvt) {
    if (prevHoverObj != currentObj) {
      if (prevHoverObj != null) this.FireListener(new dvt.ComponentTouchEvent(dvt.ComponentTouchEvent.TOUCH_HOVER_OUT_TYPE, touch, prevHoverObj, currentObj, nativeEvt));
      if (currentObj != null) this.FireListener(new dvt.ComponentTouchEvent(dvt.ComponentTouchEvent.TOUCH_HOVER_OVER_TYPE, touch, currentObj, prevHoverObj, nativeEvt));
    }
  };
  /**
  ...
  dvt.TouchManager.prototype.startTouchHold = function (evt) {
    ...
    if (touchid != null && !isNaN(touchid)) {
      ...
      if (info) {
        ...
        var nativeEvt = evt.getNativeEvent(); // If there are any immediate touches for this id, end it since touch hold has started
  ...
  dvt.TouchManager.prototype.postEventBubble = function (touchEvent) {
    ...
    if (type == dvt.TouchEvent.TOUCHSTART) {} else if (type == dvt.TouchEvent.TOUCHMOVE) {} else if (type == dvt.TouchEvent.TOUCHEND) {
      var changedTouches = this._getStoredTouches(touchEvent.changedTouches);
  ...
  dvt.TouchManager.prototype.performAssociatedTouchEnd = function (touch, uniqueKey, event) {
    ...
    for (var i = 0; i < savedInfoObjs.length; i++) {
      ...
      var endListener = savedInfo['endListener']; // Remove tooltip info first
  ...
  dvt.TouchManager.prototype.getTooltipInfo = function () {
    ...
    for (var i = 0; i < matches.length; i++) {
      ...
      var tooltipObj;
      var touchInfo = this._touchMap[touchId]; // By default, show tooltip for item under finger

      if (touchInfo) tooltipObj = touchInfo['currentObj']; // If an override is present, use this instead
  ...
  dvt.TouchManager.prototype.processTouchMove = function (touchEvent) {
    ...
    for (var i = 0; i < changedTouches.length; i++) {
      ...
      info['touchMoved'] = true; // If the single finger touch id moves, don't attempt a touch hold

      if (info[dvt.TouchManager.TOUCH_MODE] != dvt.TouchManager.TOUCH_MODE_LONG_PRESS) {
        var pageDx = Math.abs(info['pageX'] - touch.pageX);
        var pageDy = Math.abs(info['pageY'] - touch.pageY); // Reset touch hold

        if (this._startSingleFingerTouchId == touchid && (pageDx > 0 || pageDy > 0)) this.resetTouchHold(); // Ignore any touch move less than 3px, decreasing sensitivity for recognizing tap
      ...
      if (info['touchMoved'] || info['fireClick'] && info[dvt.TouchManager.TOUCH_MODE] != dvt.TouchManager.TOUCH_MODE_LONG_PRESS && info['currentObj'] != info['startTarget'] && info['currentObj'] != this._eventManager.getMarqueeGlassPane()) {
  ...
  dvt.TouchManager.prototype.getMultiTouchData = function (touchIds) {
    if (touchIds.length == 2) {
      ...
      if (touch1Data['dx'] == 0 && touch2Data['dy'] == 0) return null; //: use page coords instead of screen coords, which the component can then turn into
      ...
      var dist = Math.sqrt(dx * dx + dy * dy); // previous distance 2 fingers are apart
      ...
      var dz = dist - prevdist; // calculate the distance the center btwn the two fingers has moved
  ...
  dvt.ComponentTouchEvent.TOUCH_DOUBLE_CLICK_TYPE = 'touchdblclick'; // Stub methods so that mouse click handlers don't break
  ...
  dvt.Obj.createSubclass(dvt.DomEventFactory, dvt.Obj); // Note: this doesn't need to live in the factory because it will always be called
  ...
  dvt.EventFactory.newRenderEvent = function () {
    return dvt.EventFactory.newEvent('dvtRender');
  }; //***************************** Common Events ********************************/
  ...
  dvt.EventFactory.newCategoryHighlightEvent = function (categories, bOver) {
    var ret = dvt.EventFactory.newEvent('categoryHighlight'); // Private property for use within the toolkit.

    ret['_highlightType'] = bOver ? 'over' : 'out'; // Ensure that the categories is an array for easy uptake

    categories = categories || [];
    ret['categories'] = !(categories instanceof Array) ? [categories] : categories; // Create the option change event
  ...
  dvt.EventFactory.newActionTooltipEvent = function (type, target) {
    var ret = dvt.EventFactory.newEvent(type);
    ret['target'] = target;
    return ret;
  };
  /**
  ...
  dvt.EventFactory.newPanZoomEvent = function (subtype, dxMin, dxMax, dyMin, dyMax, dxMinTotal, dxMaxTotal, dyMinTotal, dyMaxTotal) {
    var ret = dvt.EventFactory.newEvent('dvtPanZoom');
    ret.subtype = subtype;
    ret.dxMin = dxMin;
    ret.dxMax = dxMax;
    ret.dyMin = dyMin;
  ...
  dvt.EventFactory.newSimpleScrollbarEvent = function (subtype, newMin, newMax) {
    var ret = dvt.EventFactory.newEvent('dvtSimpleScrollbar');
    ret.subtype = subtype;
    ret.newMin = newMin;
    ret.newMax = newMax;
    return ret;
  ...
  dvt.EventFactory.newOverviewEvent = function (subtype, newX1, newX2, newY1, newY2, oldX1, oldX2, oldY1, oldY2) {
    var ret = dvt.EventFactory.newEvent('overview');
    ret.subtype = subtype;
    ret.newX1 = newX1;
    ret.newX2 = newX2;
    ret.newY1 = newY1;
  ...
  dvt.EventFactory.newMarqueeEvent = function (subtype, x, y, w, h, ctrlKey) {
    var ret = dvt.EventFactory.newEvent('dvtMarquee');
    ret.subtype = subtype;
    ret.x = x;
    ret.y = y;
    ret.w = w;
  ...
  dvt.EventFactory.newBreadcrumbsDrillEvent = function (id) {
    var ret = dvt.EventFactory.newEvent('breadcrumbsDrill', id);
    return ret;
  };
  /**
  ...
  dvt.EventFactory.newSunburstRotationEvent = function (startAngle, bComplete) {
    ...
    ret['complete'] = bComplete; // Adjust the angle so that it's always between 0 and 360
  ...
  dvt.EventFactory.newGanttResizeEvent = function (taskContexts, value, start, end, type) {
    var ret = dvt.EventFactory.newEvent('resize');
    ret['taskContexts'] = taskContexts;
    ret['value'] = value;
    ret['start'] = start;
    ret['end'] = end;
  ...
  dvt.EventFactory.newGanttExpandCollapseEvent = function (type, id, rowData, itemData, expanded) {
    var ret = dvt.EventFactory.newEvent(type, id);
    ret['rowData'] = rowData;
  ...
  dvt.EventFactory.newPanEvent = function (subtype, newX, newY, oldX, oldY, animator) {
    var ret = dvt.EventFactory.newEvent('dvtPan');
    ret.subtype = subtype;
    ret.newX = newX;
    ret.newY = newY;
    ret.oldX = oldX;
  ...
  dvt.EventFactory.newZoomEvent = function (subtype, newZoom, oldZoom, animator, centerPoint, tx, ty) {
    var ret = dvt.EventFactory.newEvent('dvtZoom');
    ret.subtype = subtype;
    ret.newZoom = newZoom;
    ret.oldZoom = oldZoom;
    ret.animator = animator;
  ...
  dvt.EventFactory.newTimelineOverviewEvent = function (subtype, itemId, isMultiSelect) {
    var ret = dvt.EventFactory.newEvent('timeline');
    ret.subtype = subtype;
    ret.itemId = itemId;
  ...
  Dvt3rdPartyPathUtils.lineCardinalClosed = function (points, tension) {
    return points.length < 3 ? Dvt3rdPartyPathUtils.lineLinear(points) : points[0] + Dvt3rdPartyPathUtils.lineHermite((points.push(points[0]), points), Dvt3rdPartyPathUtils.lineCardinalTangents([points[points.length - 2]].concat(points, [points[1]]), tension));
  };
  /**
  ...
  dvt.DisplayableUtils.getDimensionsForced = function (context, obj) {
    ...
    stage.removeChild(obj); //restore original parent
  ...
  dvt.DisplayableUtils.getDimForced = function (context, obj) {
    ...
    var dim = dvt.DisplayableUtils.getDimensionsForced(context, obj); //cached the dimensions
  ...
  dvt.PathUtils.rectangleWithBorderRadius = function (x, y, w, h, radius, multiplier, defaultValue) {
    ...
    if (radius) {
      if (radius.indexOf('/') != -1) {
        ...
        if (horiz.length == 1) topLeftX = topRightX = bottomRightX = bottomLeftX = horiz[0];else if (horiz.length == 2) {
        ...
        if (vert.length == 1) topLeftY = topRightY = bottomRightY = bottomLeftY = vert[0];else if (vert.length == 2) {
        ...
        if (split.length == 1) {
          topLeftX = topRightX = bottomRightX = bottomLeftX = topLeftY = topRightY = bottomRightY = bottomLeftY = split[0];
    ...
    if (!radius || radius == '0') return dvt.PathUtils._rectangle(x, y, w, h);
    return dvt.PathUtils._roundedRectangle(x, y, w, h, dvt.PathUtils._parseBorderRadiusItem(topLeftX, multiplier), dvt.PathUtils._parseBorderRadiusItem(topLeftY, multiplier), dvt.PathUtils._parseBorderRadiusItem(topRightX, multiplier), dvt.PathUtils._parseBorderRadiusItem(topRightY, multiplier), dvt.PathUtils._parseBorderRadiusItem(bottomRightX, multiplier), dvt.PathUtils._parseBorderRadiusItem(bottomRightY, multiplier), dvt.PathUtils._parseBorderRadiusItem(bottomLeftX, multiplier), dvt.PathUtils._parseBorderRadiusItem(bottomLeftY, multiplier));
  };
  /**
  ...
  dvt.PathUtils._roundedRectangle = function (x, y, w, h, tlcrX, tlcrY, trcrX, trcrY, brcrX, brcrY, blcrX, blcrY) {
    ...
    var cmd = dvt.PathUtils.moveTo(x + tlcrX, y) + dvt.PathUtils.lineTo(x + w - trcrX, y) + dvt.PathUtils.arcTo(trcrX, trcrY, Math.PI / 2, 1, x + w, y + trcrY) + dvt.PathUtils.lineTo(x + w, y + h - brcrY) + dvt.PathUtils.arcTo(brcrX, brcrY, Math.PI / 2, 1, x + w - brcrX, y + h) + dvt.PathUtils.lineTo(x + blcrX, y + h) + dvt.PathUtils.arcTo(blcrX, blcrY, Math.PI / 2, 1, x, y + h - blcrY) + dvt.PathUtils.lineTo(x, y + tlcrY) + dvt.PathUtils.arcTo(tlcrX, tlcrY, Math.PI / 2, 1, x + tlcrX, y) + dvt.PathUtils.closePath();
    return cmd;
  };
  /**
  ...
  dvt.PathUtils.curveThroughPoints = function (points, connectWithLine, splineType) {
    ...
    for (var i = 0; i < points.length; i += 2) {
      // convert to D3 points format
      pts.push([points[i], points[i + 1]]);
    }

    var prefix = connectWithLine ? 'L' : 'M';
    if (splineType == dvt.PathUtils.SPLINE_TYPE_MONOTONE_VERTICAL) return prefix + Dvt3rdPartyPathUtils.lineMonotone(pts, false);else if (splineType == dvt.PathUtils.SPLINE_TYPE_MONOTONE_HORIZONTAL) return prefix + Dvt3rdPartyPathUtils.lineMonotone(pts, true);else if (splineType == dvt.PathUtils.SPLINE_TYPE_CARDINAL_CLOSED) return prefix + Dvt3rdPartyPathUtils.lineCardinalClosed(pts, 0.7);else return prefix + Dvt3rdPartyPathUtils.lineCardinal(pts, 0.7);
  };
  /**
  ...
  dvt.PathUtils.createPathArray = function (sCmds) {
    if (!sCmds) return null; //  Unpack into an array of commands and coords.
    ...
    var ar = cmds.split(/[ ,]/g);
    var len = ar.length;
    var i; //  Convert coordinates in command array to floats.
    ...
    for (var i = 0; i < arCmds.length; i++) {
      ...
      if (cmdType.toUpperCase() === 'A') {
        ...
        if (cmdType.toUpperCase() === 'H') {
          ...
          scales.push(sx);
          scales.push(sy); // For absolute commands, even indices should be translated by x, odd indices by y
  ...
  dvt.PathUtils.simplifyPath = function (cmdAr, scale) {
    ...
    var simplifiedCmd;
    var mx = 0;
    var my = 0; // we expect all path commands to start with an m and end with a z
    ...
    for (var i = 0; i < cmdAr.length; i++) {
      if (isNaN(cmdAr[i])) {
        ...
        if (cmd == 'Z' || cmd == 'z') {
          simplifiedCmd = cmd;
          tempSimplifiedStr += cmd; // if a command only contains move commands i.e. m[x] [y]z, do not add it but keep track of the relative position
      ...
      switch (cmd) {
        ...
        case 'h':
        case 'v':
          if (cmd == 'h') partialX += cmdAr[i];else partialY += cmdAr[i];
  ...
  dvt.PathUtils._rectangle = function (x, y, w, h) {
    var cmd = dvt.PathUtils.moveTo(x, y) + dvt.PathUtils.horizontalLineTo(x + w) + dvt.PathUtils.verticalLineTo(y + h) + dvt.PathUtils.horizontalLineTo(x) + dvt.PathUtils.closePath();
    return cmd;
  };
  /**
   * @license
  ...
  dvt.TextUtils.getTextStringWidth = function (context, textString, cssStyle) {
    if (textString == null || textString.length == 0) return 0; // Look for the value in the cache and return it if found.
    ...
    if (cachedDims != null) return cachedDims;else {
  ...
  dvt.TextUtils.fitText = function (text, maxWidth, maxHeight, container, minChars, noRemove) {
    var untruncatedTextString = text.getTextString(); // Initial checks for OutputText

    if (text instanceof dvt.OutputText) {
      var textDims = dvt.TextUtils.getTextStringDimensions(text.getCtx(), untruncatedTextString, text.getCSSStyle()); // Safe to quit if height doesn't fit.
    ...
    } else if (text instanceof dvt.OutputText || text instanceof dvt.BackgroundOutputText) dvt.TextUtils._truncateOutputText(text, maxWidth, minChars);else text.setMaxWidth(maxWidth, minChars); // Check if the truncated text can fit


    if (text.getTextString() == '') {
      if (!noRemove) container.removeChild(text); // Reset the original text

      text.setTextString(untruncatedTextString);
      return false;
    } else {
      if (text.getTextString() != untruncatedTextString) text.setUntruncatedTextString(untruncatedTextString);
      return true;
    }
  };
  /**
  ...
  dvt.TextUtils.renderEmptyText = function (container, textStr, space, eventManager, style) {
    ...
    text.alignCenter();
    text.alignMiddle(); // Truncate the text to fit horizontally.  Note, we do not account for vertical size, because displaying a
    ...
    if (dvt.TextUtils.fitText(text, maxWidth, Infinity, container, 0)) {
      ...
      if (text.isTruncated()) eventManager.associate(text, new dvt.SimpleObjPeer(text.getUntruncatedTextString())); // WAI-ARIA
  ...
  dvt.TextUtils._truncateOutputText = function (text, maxWidth, minChars) {
    ...
    if (dims.w <= maxWidth) return; // Determine avg pixels per char and make an initial guess at the truncation
    ...
    dims = text.getDimensions(); // Add characters if initial guess is too short.  Keep track of the previous string and dims in case we overshoot

    var prevTextString = truncatedTextString;
    var prevDims = dims;

    while (dims.w < maxWidth) {
      if (maxNumChars >= textString.length) break; // Calculate the availWidth and use that to estimate the number of characters to increment

      var availWidth = maxWidth - dims.w; // estimatedIncrement: The smaller of the chars remaining and the estimate of characters than can fit

      var estimatedIncrement = Math.min(textLength - maxNumChars, Math.max(Math.floor(availWidth / avgPixelsPerChar), 1)); // Add chars and measure again

      maxNumChars += estimatedIncrement;
      truncatedTextString = textString.substring(0, maxNumChars) + dvt.OutputText.ELLIPSIS;
      text.setTextString(truncatedTextString);
      dims = text.getDimensions(); // If we overshot and the increment was only 1, then restore the old string
  ...
  dvt.TextUtils._getRepresentativeDimensions = function (context, cssStyle) {
    ...
    if (cachedDims == null) {
      ...
      text.alignAuto();
      text.setCSSStyle(cssStyle); // Add to the stage to obtain correct measurements

      var stage = text.getCtx().getStage();
      stage.addChild(text);
      var dims = text.GetSvgDimensions();
      stage.removeChild(text); // Cache the dims of a single character. Conservative because real strings are not solely longest characters.
  ...
  dvt.TextUtils._getCanvasTextWidth = function (context, text, cssStyle) {
    ...
    var fontSize = (cssStyle ? cssStyle.getFontSize() : null) || context.getDefaultFontSize() || '12px'; // include 'px' if fontSize is just a number
  ...
  dvt.TextUtils._configureCanvas = function (context, font) {
    ...
    if (dvt.Agent.browser === 'ie' || dvt.Agent.browser === 'edge') {
      font = font.replace(/-apple-system(-\w+)*,?/g, ''); // Also, it's relative font units only work if canvas is attached to DOM
  ...
  dvt.TextUtils.processAriaLabel = function (label) {
    var ret = label;

    if (ret) {
      // Make all brackets consistent to simplify later searches
      ret = ret.replace(/(<|&#60;)/g, '&lt;');
      ret = ret.replace(/(>|&#62;)/g, '&gt;'); // Strip out bold and italic tags

      ret = ret.replace(/&lt;b&gt;/g, '');
      ret = ret.replace(/&lt;\/b&gt;/g, '');
      ret = ret.replace(/&lt;i&gt;/g, '');
      ret = ret.replace(/&lt;\/i&gt;/g, ''); // Replace logical newlines sequences with semicolons
    ...
    for (var i = 0; i < regions.length; i++) {
      ...
      if (region) {
        ...
        if (region.type === dvt.ClipPath.RECT) {
          ...
          if (region.rx) dvt.ToolkitUtils.setAttrNullNS(elem, 'rx', region.rx);
          if (region.ry) dvt.ToolkitUtils.setAttrNullNS(elem, 'ry', region.ry);
        ...
        if (elem) dvt.ToolkitUtils.appendChildElem(elemClip, elem);
      }
    }
  ...
  dvt.SvgShapeUtils.addMask = function (mask, context) {
    ...
    var gradientId = dvt.SvgShapeUtils.addSpecialFill(gradient, context);
    ...
    dvt.ToolkitUtils.setAttrNullNS(rectElem, 'fill', dvt.ToolkitUtils.getUrlById(gradientId));
  ...
  dvt.Displayable = function () {// This class should never be instantiated directly
  };
  ...
  dvt.Displayable._ATTRS_TRANSFERABLE_TO_OUTER = [
  /*'filter',*/
  'clip-path'];
  /**
  ...
  dvt.Displayable.prototype.Init = function (context, type, id) {
    ...
    this.setId(id); // TODO  Removal of null initialization routines
  ...
  dvt.Displayable.prototype.setId = function (id, bForce) {
    if (this._id !== id) {
      ...
      if (dvt.Displayable.SET_ID_ON_DOM || bForce) {
        if (id) dvt.ToolkitUtils.setAttrNullNS(this.getOuterElem(), 'id', id);else dvt.ToolkitUtils.removeAttrNullNS(this.getOuterElem(), 'id');
      }
    }
  };
  /**
  ...
  dvt.Displayable.prototype.isDescendantOf = function (obj) {
    if (!obj || !this.getParent()) return false;else if (this.getParent() == obj) return true;else return this.getParent().isDescendantOf(obj);
  };
  /**
  ...
  dvt.Displayable.prototype.setPixelHinting = function (bHint) {
    if (bHint) dvt.ToolkitUtils.setAttrNullNS(this._elem, 'shape-rendering', 'crispEdges');else dvt.ToolkitUtils.removeAttrNullNS(this._elem, 'shape-rendering');
  };
  /**
  ...
  dvt.Displayable.prototype.setVisible = function (bVis) {
    if (this._bVisible !== bVis) {
      this._bVisible = bVis;
      var elem = this.getElem(); // - HTML5: VISIBILITY="VISIBLE" LEFT ON ELEMENT AFTER ANIMATION
  ...
  dvt.Displayable.prototype.setClipPath = function (cp) {
    ...
    if (cp) {
      if (this.ClipPathId == cp.getId()) {
        ...
        this._context.increaseGlobalDefReference(this.ClipPathId); // Create an outer group if there is a matrix defined, since SVG applies transforms before clip paths.
  ...
  dvt.Displayable.prototype.setMask = function (mask) {
    ...
    if (mask) {
      if (this.MaskId == mask.getId()) {
        ...
        this._context.increaseGlobalDefReference(this.MaskId); // Create an outer group if there is a matrix defined, since SVG applies transforms before clip paths.
  ...
  dvt.Displayable.prototype.ConvertCoordSpaceRect = function (rect, targetCoordinateSpace) {
    if (!targetCoordinateSpace || targetCoordinateSpace === this) return rect; // First calculate relative to the stage

    var stageP1 = this.localToStage(new dvt.Point(rect.x, rect.y));
    var stageP2 = this.localToStage(new dvt.Point(rect.x + rect.w, rect.y + rect.h)); // Then convert relative to target

    var targetP1 = targetCoordinateSpace.stageToLocal(stageP1);
    var targetP2 = targetCoordinateSpace.stageToLocal(stageP2);
    return new dvt.Rectangle(Math.min(targetP1.x, targetP2.x), Math.min(targetP1.y, targetP2.y), Math.abs(targetP2.x - targetP1.x), Math.abs(targetP2.y - targetP1.y));
  };
  /**
  ...
  dvt.Displayable.prototype.GetDimensionsWithStroke = function (targetCoordinateSpace) {
    ...
    if (!targetCoordinateSpace || targetCoordinateSpace === this) return dims;else {
  ...
  dvt.Displayable.prototype._createOuterGroupElem = function () {
    ...
    this._outerElem = dvt.SvgShapeUtils.createElement('g', outerId); // Reparent the DOM elements
    ...
    dvt.ToolkitUtils.appendChildElem(this._outerElem, this.getElem()); // Transfer attributes from the old outermost SVG element to the new outer element
  ...
  dvt.Displayable.prototype.setCursor = function (cursorType) {
    this._cursor = cursorType; // Update the DOM

    if (cursorType) dvt.ToolkitUtils.setAttrNullNS(this.getElem(), 'cursor', cursorType);else dvt.ToolkitUtils.removeAttrNullNS(this.getElem(), 'cursor');
  };
  /**
  ...
  dvt.Displayable.prototype.applyAriaProperties = function () {
    if (dvt.Agent.deferAriaCreation()) {
      // Apply Aria Role
      if (this._ariaRole) this._setAriaRole(this._ariaRole); // Apply Aria Properties
  ...
  dvt.Displayable.prototype.setAriaRole = function (role, bForce) {
    if (!dvt.Agent.deferAriaCreation() || bForce) this._setAriaRole(role);
    this._ariaRole = role;
  };
  /**
   * Sets the WAI-ARIA role.
  ...
  dvt.Displayable.prototype._setAriaRole = function (role) {
    ...
    if (role) dvt.ToolkitUtils.setAttrNullNS(elem, 'role', role);else dvt.ToolkitUtils.removeAttrNullNS(elem, 'role');
  };
  /**
  ...
  dvt.Displayable.prototype.setAriaProperty = function (property, value, bForce) {
    if (!property) return;
    if (!dvt.Agent.deferAriaCreation() || bForce) this._setAriaProperty(property, value);
  ...
  dvt.Displayable.prototype._setAriaProperty = function (property, value) {
    ...
    if (value != null && value !== '') {
      ...
      if (property == 'label') value = dvt.TextUtils.processAriaLabel(value);
      dvt.ToolkitUtils.setAttrNullNS(elem, 'aria-' + property, value);
    } else dvt.ToolkitUtils.removeAttrNullNS(elem, 'aria-' + property);
  };
  /**
  ...
  dvt.Displayable.defineProps = function (obj, propertyInfo) {
  ...
  dvt.Displayable.prototype.SvgPropertyChanged = function (name) {// subclasses can override
  };
  /**
  ...
  dvt.Displayable.generateAriaLabel = function (shortDesc, states) {
    ...
    if (hasStates) {
      desc.push(states.join(dvt.Context.ARIA_LABEL_STATE_DELIMITER));
    }
  ...
  dvt.Displayable.prototype._applyDrawEffects = function () {
    var effects = this._drawEffects; // remove current filter
    ...
    if (!this._outerElem && this._matrix) this._createOuterGroupElem(); //add new filter if necessary
  ...
  dvt.Displayable.prototype.addEvtListener = function (type, listener, options, obj) {
    ...
    var useCapture = dvt.ToolkitUtils.getUseCaptureFromOptions(options); // Store a reference to the listener

    var listenersArray = this._getListeners(type, useCapture, true);

    listenersArray.push(listener);
    listenersArray.push(obj); // Call the impl so that it can add the actual listener
  ...
  dvt.Displayable.prototype.removeEvtListener = function (type, listener, options, obj) {
    // Extract useCapture from options
    var useCapture = dvt.ToolkitUtils.getUseCaptureFromOptions(options); // Remove the listener
  ...
  dvt.Displayable.prototype._addListener = function (type, options) {
    ...
    var listener = this._getListener(useCapture); // on keyboard events, add the listener to the component's wrapping div, since SVG
    // DOM elements don't support keystrokes.


    if (type == dvt.KeyboardEvent.KEYUP || type == dvt.KeyboardEvent.KEYDOWN || type == dvt.KeyboardEvent.KEYPRESS || type == DvtFocusEvent.FOCUS || type == DvtFocusEvent.FOCUSIN || type == DvtFocusEvent.FOCUSOUT || type == DvtFocusEvent.BLUR) {
      var context = this.getCtx();
      var stage = context.getStage();
      var wrappingDiv = stage.getSVGRoot().parentNode;
      if (!wrappingDiv) return; // allow support for multiple displayables to receive keyboard events
  ...
  dvt.Displayable.prototype._removeListener = function (type, options) {
    ...
    } else dvt.ToolkitUtils.removeDomEventListener(this.getElem(), type, listener, options);
  };
  /**
    ...
    if (event.type === dvt.MouseEvent.MOUSEOVER || dvt.MouseEvent.MOUSEOUT) {
      ...
      if (hoverItem) {
        if (hoverItem !== event.target) {
          var dvtEvent = dvt.DomEventFactory.newEvent(event, this.getCtx()); //: Set the event as MOUSEOUT event, target as hoverItem, relatedTarget as event.target
  ...
  dvt.Displayable.prototype.setFill = function (fill) {
    ...
    if (fill instanceof dvt.GradientFill || fill instanceof dvt.PatternFill) {
      var fillId = dvt.SvgShapeUtils.addSpecialFill(fill, this.getCtx());
      dvt.ToolkitUtils.setAttrNullNS(this._elem, 'fill', dvt.ToolkitUtils.getUrlById(fillId));
    } else {
      // Basic fill
      ...
      if (fillObj && fillObj['color']) {
        ...
        if (fillObj['alpha'] != null) dvt.ToolkitUtils.setAttrNullNS(this._elem, 'fill-opacity', fillObj['alpha'], 1);
      }
    }
  ...
  dvt.Displayable.prototype.setClassName = function (className) {
    if (this._className && !className) dvt.ToolkitUtils.removeAttrNullNS(this._elem, 'class');else if (className) dvt.ToolkitUtils.setAttrNullNS(this._elem, 'class', className);
  ...
  dvt.Displayable.prototype.setStyle = function (style) {
    if (this._style) dvt.ToolkitUtils.removeAttrNullNS(this._elem, 'style');
    if (style) dvt.CSSStyle.applyCssObjectToElement(style, this._elem);
    this._style = style;
    return this;
  };
  /**
  ...
  dvt.Displayable.prototype.setStroke = function (stroke) {
    ...
    if (strokeObj && strokeObj['color']) {
      ...
      if (strokeObj['alpha'] != null) dvt.ToolkitUtils.setAttrNullNS(this._elem, 'stroke-opacity', strokeObj['alpha'], 1);
    } else {
      dvt.ToolkitUtils.removeAttrNullNS(this._elem, 'stroke');
      dvt.ToolkitUtils.removeAttrNullNS(this._elem, 'stroke-opacity');
    ...
    if (stroke.isFixedWidth()) dvt.ToolkitUtils.setAttrNullNS(this._elem, 'vector-effect', 'non-scaling-stroke');
    var dashProps = stroke.getDashProps();

    if (dashProps && dashProps.dashArray) {
      dvt.ToolkitUtils.setAttrNullNS(this._elem, 'stroke-dasharray', dashProps.dashArray);
    } else {
      dvt.ToolkitUtils.removeAttrNullNS(this._elem, 'stroke-dasharray');
    }

    if (dashProps && dashProps.dashOffset) {
      dvt.ToolkitUtils.setAttrNullNS(this._elem, 'stroke-dashoffset', dashProps.dashOffset);
    } else {
      dvt.ToolkitUtils.removeAttrNullNS(this._elem, 'stroke-dashoffset');
    }
    ...
    if (lineProps && lineProps.lineJoin) {
      dvt.ToolkitUtils.setAttrNullNS(this._elem, 'stroke-linejoin', lineProps.lineJoin);
    } else {
      dvt.ToolkitUtils.removeAttrNullNS(this._elem, 'stroke-linejoin');
    }

    if (lineProps && lineProps.lineCap) {
      dvt.ToolkitUtils.setAttrNullNS(this._elem, 'stroke-linecap', lineProps.lineCap);
    } else {
      dvt.ToolkitUtils.removeAttrNullNS(this._elem, 'stroke-linecap');
    }

    if (lineProps && lineProps.miterLimit) {
      dvt.ToolkitUtils.setAttrNullNS(this._elem, 'stroke-miterlimit', lineProps.miterLimit);
    } else {
      dvt.ToolkitUtils.removeAttrNullNS(this._elem, 'stroke-miterlimit');
    }
  };
  ...
  dvt.Displayable.prototype.setSolidStroke = function (color, alpha, strokeWidth) {
    this.setStroke(new dvt.Stroke(color, alpha, strokeWidth));
  };
  /**
  ...
  dvt.Displayable.prototype.setMatrix = function (mat) {
    ...
    if (this._matrix != mat) {
      this._matrix = mat; // Create an outer elem if needed

      if (!this._outerElem && dvt.Displayable.HasAttributes(this.getElem(), dvt.Displayable._ATTRS_TRANSFERABLE_TO_OUTER)) this._createOuterGroupElem();
      this._matrix = mat; //apply the new matrix if it's non-null

      if (mat) {
        var sMat = 'matrix(' + mat.getA() + ',' + mat.getC() + ',' + mat.getB() + ',' + mat.getD() + ',' + mat.getTx() + ',' + mat.getTy() + ')'; //set the transform attribute on the outer SVG element of this shape

        dvt.ToolkitUtils.setAttrNullNS(this.getElem(), 'transform', sMat, 'matrix(1,0,0,1,0,0)'); //set the vector-effect attribute to prevent the stroke-width
  ...
  dvt.Container.prototype.addChild = function (obj) {
    if (obj) {
      ...
      if (oldParent) oldParent.removeChild(obj); //initialize _arList after removing child from parent in case
    ...
    if (index >= 0 && index <= this.getNumChildren() && obj) // validate index
      {
        ...
        if (this._bShapeContainer) {
          ...
          //  (parent) object and add the new child to the child group DOM element.
          //  else will add the object directly to the shape group container element.
          this.CreateChildGroupElem(false); //need to increment the index to account for the shape dom element,
        ...
        var containerElem = this.getContainerElem();
        var existingNode = containerElem.childNodes[idx]; // : IE9 cannot handle a value of undefined in insertBefore.  Set to null in such a case.
  ...
  dvt.Container.prototype.CreateChildGroupElem = function (rmChildren, forced) {
    ...
    if (!this._childGroupElem && (forced || this.getNumChildren() === 0)) {
      ...
      this._childGroupElem = dvt.SvgShapeUtils.createElement('g', childGroupId); //  Remove this's DOM element from the parent, and append to child group DOM element.
      ...
      if (!rmChildren) dvt.ToolkitUtils.appendChildElem(this._childGroupElem, this._elem); // add shape to new group
      //transfer attributes from the old outermost SVG element to the
      //new outer group element

      dvt.Displayable.TransferAttributes(this._elem, this._childGroupElem, this.GetAttributesTransferableToGroup()); //Note need to copy _obj reference to the new group element so that events can be propagated.
  ...
  dvt.Container.prototype.getDimensionsWithStroke = function () {
    ...
    while (ar.length > 0) {
      ...
      var child = ar.shift(); //get dims in coordinate space of this container, because we're
  ...
  dvt.Container.prototype.UpdateSelectionEffect = function () {// Needed for setFill function
  };
  /**
  ...
  var DvtStage = function DvtStage(context, svgRoot, id) {
    // Note: Initialization code in constructor to prevent subclassing.
    this.Init(context, 'g', id);
    this._SVGRoot = svgRoot; // containing SVG DOM element

    this.disableSelection(this._elem); //set cursor to default to avoid text cursors on text objects
  ...
  dvt.Shape = function () {// This class should never be instantiated directly
  ...
  dvt.Shape.prototype.Init = function (context, type, id) {
    dvt.Shape.superclass.Init.call(this, context, type, id); // TODO : For all properties that default to null or false, remove from constructor
  ...
  dvt.Shape.prototype.setHollow = function (fc, strokeWidth) {
    if (!this._bHollow) {
      ...
      this._origStroke = this.getStroke(); // Set hollow shape fill and stroke based on shape fill

      var hollowFill = dvt.SolidFill.invisibleFill();
      var hollowStroke;

      if (fc) {
        hollowStroke = new dvt.Stroke(dvt.ColorUtils.getRGB(fc), dvt.ColorUtils.getAlpha(fc), strokeWidth);
      } else {
        hollowStroke = new dvt.Stroke(this.GetProperty('fill').getColor(), this.GetProperty('fill').getAlpha(), strokeWidth);
      }
  ...
  dvt.Shape.prototype.getDimensionsWithStroke = function () {
    ...
    var dims = this.GetElemDimensionsWithStroke(); //get the dims for any children of this container

    var containerDims = dvt.Shape.superclass.getDimensionsWithStroke.call(this); //create union of shape and container dims
  ...
  dvt.Shape.prototype.GetElemDimensionsWithStroke = function () {
    //get the dims for this shape by itself
    var dims = this.getDimensionsSelf && this.getNumChildren() - this._getInnerShapeCount() == 0 ? this.getDimensionsSelf() : dvt.Shape.superclass.getDimensions.call(this); //assume that stroke touches every edge of bounding box, so push out
  ...
  dvt.Shape.prototype.UpdateSelectionEffect = function () {
    if (this.isSelected() || this.isHoverEffectShown()) {
      ...
      if (stroke) this.InnerShape.setStroke(stroke);
      this.InnerShape.setMouseEnabled(false);
  ...
  dvt.Shape.prototype.GetHoverSelectionOuterStroke = function () {
    ...
    if (this._outerShape) {
      if (this._outerShape.isSelected()) {
        outerStroke = this._outerShape.isHoverEffectShown() ? this._outerShape.SelectedHoverOuterStroke : this._outerShape.SelectedOuterStroke;
  ...
  dvt.Shape.prototype.setSelectable = function (bSelectable) {
    ...
    if (this._bSelectable) this.setCursor(dvt.SelectionEffectUtils.getSelectingCursor());else this.setCursor(null);
  };
  /**
   * Implemented for DvtSelectable
   */
  ...
  dvt.Shape.prototype.CreateSelectedHoverStrokes = function () {
    ...
    if (!this.SelectedHoverOuterStroke) {
      this.SelectedHoverOuterStroke = new dvt.Stroke(this.SelectedOuterStroke.getColor(), this.SelectedOuterStroke.getAlpha(), this.SelectedOuterStroke.getWidth() + (this.HoverInnerStroke.getWidth() - this.SelectedInnerStroke.getWidth()));
    }
  };
  /**
   * Implemented for DvtSelectable
  ...
  dvt.Shape.prototype.getDimensions = function (targetCoordinateSpace) {
    ...
    if (this.getDimensionsSelf && this.getNumChildren() - this._getInnerShapeCount() == 0) return this.getDimensionsSelf(targetCoordinateSpace);else return dvt.Shape.superclass.getDimensions.call(this, targetCoordinateSpace);
  };
  /**
  ...
  var DvtPolygonalShape = function DvtPolygonalShape() {// This class should never be instantiated directly
  };
  ...
  dvt.Obj.createSubclass(dvt.Circle, dvt.Shape);
  /**
   * @param {dvt.Context} context
   * @param {number} cx The x coordinate of the center of the circle.
   * @param {number} cy The y coordinate of the center of the circle.
  ...
  dvt.Circle.prototype.Init = function (context, cx, cy, r, id) {
    dvt.Circle.superclass.Init.call(this, context, 'circle', id);
    this.setCx(cx).setCy(cy);
    this.setRadius(r);
  }; // Create SVG property getters and setters


  dvt.Displayable.defineProps(dvt.Circle, {
    r: {
      name: 'radius'
    },
    cx: {
  ...
  dvt.Image.prototype.Init = function (context, src, x, y, w, h, id) {
    dvt.Image.superclass.Init.call(this, context, 'image', id); // If we're in JET where the src may be a CSS class name, then obtain the URL from the background-image property.

    src = dvt.ToolkitUtils.getImageUrl(context, src); // IE doesn't allow interactivity unless there's a fill
    ...
    this.setX(x).setY(y);
    this.setWidth(w);
    this.setHeight(h); //TODO: set preserveAspectRatio="none" for now

    dvt.ToolkitUtils.setAttrNullNS(this._elem, 'preserveAspectRatio', 'none'); // By default, hide images from VoiceOver

    this.setAriaProperty('hidden', 'true');
  }; // Create SVG property getters and setters


  dvt.Displayable.defineProps(dvt.Image, {
    x: {},
    y: {},
    width: {},
    height: {}
  ...
  dvt.Image.prototype.getDimensions = function (targetCoordinateSpace) {
    // Optimized implementation that allows container geometry to be taken into account to avoid costly DOM calls
    if (this.getDimensionsSelf && this.getNumChildren() - this._getInnerShapeCount() == 0) return this.getDimensionsSelf(targetCoordinateSpace);else {
      var bbox = this.getElem().getBBox();
      if (bbox.width && bbox.height) bbox = new dvt.Rectangle(bbox.x, bbox.y, bbox.width, bbox.height);else bbox = new dvt.Rectangle(bbox.x, bbox.y, this.getWidth(), this.getHeight());
  ...
  dvt.Image.prototype.UpdateSelectionEffect = function () {// noop: Does not participate in selection effects
  };
  /**
   * @override
   */
  ...
  dvt.Image.prototype.setAriaProperty = function (property, value) {
    ...
    if (property != 'hidden') dvt.Image.superclass.setAriaProperty.call(this, 'hidden', null);
  };
  /**
   * @license
   * Copyright (c) 2008 %CURRENT_YEAR%, Oracle and/or its affiliates.
  ...
  dvt.Displayable.defineProps(dvt.Line, {
    x1: {
      value: 0
    },
    x2: {
  ...
  dvt.Line.prototype.setHollow = function () {
    ...
    if (!this._bHollow) {
      ...
      if (color) {
        var alpha = stroke.getAlpha(); // Workaround for Safari where versions < 5.1 draw rgba values as black

        if (dvt.Agent.browser === 'safari' && color.indexOf('rgba') !== -1) {
          dvt.ToolkitUtils.setAttrNullNS(hollowMarker, 'stroke', dvt.ColorUtils.getRGB(color)); // Use alpa in rgba value as a multiplier to the alpha set on the object as this is what svg does.

          if (alpha != null) dvt.ToolkitUtils.setAttrNullNS(hollowMarker, 'stroke-opacity', dvt.ColorUtils.getAlpha(color) * alpha);else dvt.ToolkitUtils.setAttrNullNS(hollowMarker, 'stroke-opacity', dvt.ColorUtils.getAlpha(color));
        } else {
          dvt.ToolkitUtils.setAttrNullNS(hollowMarker, 'stroke', color);
          if (alpha != null) dvt.ToolkitUtils.setAttrNullNS(hollowMarker, 'stroke-opacity', alpha);
        }
      }
  ...
  dvt.Line.prototype.getDimensions = function (targetCoordinateSpace) {
    ...
    var bounds = dvt.Line.superclass.getDimensions.call(this, targetCoordinateSpace); // : Vertical/horizontal lines in svg are ignored when group containers wrap them

    if (this._childGroupElem && this._elem) {
      var groupBox = this._childGroupElem.getBBox(); // Empty bounding box is an indication of only vertical/horizontal lines present in this group
  ...
  dvt.Line.prototype._adjustDimensionsForStyle = function () {
    if (this.getX1() == this.getX2()) this.setX2(this.getX2() + .001);else if (this.getY1() == this.getY2()) this.setY2(this.getY2() + .001);
  };
  /**
   * @override
  ...
  dvt.SimpleMarker = function (context, shape, cx, cy, width, height, borderRadius, bMaintainAspectRatio, bUseIntegerCoords, id) {
    this.Init(context, shape, cx, cy, width, height, borderRadius, bMaintainAspectRatio, bUseIntegerCoords, id);
  };

  dvt.Obj.createSubclass(dvt.SimpleMarker, dvt.Shape); // MARKER SHAPES
  ...
  dvt.SimpleMarker._HUMAN_CMDS = 'M 306.40625 386.78125 C 304.19988 386.78125 302.40625 389.07579 302.40625 391.90625 ' + 'C 302.40625 394.73671 304.19988 397.03125 306.40625 397.03125 C 308.61263 397.03125 310.40625 394.73671 310.40625 ' + '391.90625 C 310.40625 389.07579 308.61263 386.78125 306.40625 386.78125 z M 301.78125 396.0625 C 300.43025 397.2945 ' + '298.28125 400.28125 298.90625 403.15625 C 302.41725 405.79925 309.20225 406.154 314.03125 403 C 314.21825 399.828 ' + '312.68325 397.5635 310.90625 396.0625 C 308.65625 400.7185 304.28125 399.7815 301.78125 396.0625 z ';
  /** @const @private **/

  dvt.SimpleMarker._SHAPE_STAR_CMDS = [-50, -11.22, -16.69, -17.94, 0, -47.55, 16.69, -17.94, 50, -11.22, 26.69, 13.8, 30.9, 47.56, 0, 33.42, -30.9, 47.56, -26.69, 13.8];
  /**
   * Object initializer.
   * @param {dvt.Context} context
  ...
  dvt.SimpleMarker.prototype.Init = function (context, shape, cx, cy, width, height, borderRadius, bMaintainAspectRatio, bUseIntegerCoords, id) {
    if (bUseIntegerCoords) {
      cx = Math.round(cx);
      cy = Math.round(cy);
      width = Math.round(width);
      height = Math.round(height); // Dimensions have to be even to prevent floating numbers when calculating x and y
    ...
    if (this._shape == dvt.SimpleMarker.SQUARE || this._shape == dvt.SimpleMarker.RECTANGLE) this._path = this._getBorderRadiusPath(context, cx, cy, width, height, borderRadius);
    if (this._shape == dvt.SimpleMarker.HUMAN) this._path = new dvt.Path(context, dvt.SimpleMarker._HUMAN_CMDS);
  ...
  dvt.SimpleMarker.prototype._updateSvgProperties = function () {
    ...
    if ((this._shape == dvt.SimpleMarker.RECTANGLE || this._shape == dvt.SimpleMarker.SQUARE) && !this._borderRadius) {
    ...
    } else if (this._shape == dvt.SimpleMarker.CIRCLE || this._shape == dvt.SimpleMarker.ELLIPSE) {
      if (this._propertyChange.cx) dvt.ToolkitUtils.setAttrNullNS(this._elem, 'cx', this._cx, 0);
      if (this._propertyChange.cy) dvt.ToolkitUtils.setAttrNullNS(this._elem, 'cy', this._cy, 0);

      if (this._shape == dvt.SimpleMarker.CIRCLE) {
        if (this._propertyChange.s) dvt.ToolkitUtils.setAttrNullNS(this._elem, 'r', this._s / 2, 0);
      } else {
        if (this._propertyChange.width) dvt.ToolkitUtils.setAttrNullNS(this._elem, 'rx', this._width / 2, 0);
        if (this._propertyChange.height) dvt.ToolkitUtils.setAttrNullNS(this._elem, 'ry', this._height / 2, 0);
      }
    } else if (this._isPolygon()) {
      if (this._propertyChange.s || this._propertyChange.cx || this._propertyChange.cy) this._setPolygon(this._getPolygonArray());
    } else if (this._isPath()) {
      if (this._propertyChange.width || this._propertyChange.height || this._propertyChange.cx || this._propertyChange.cy) this._setCmds(this._getCmds());
    }
  ...
  dvt.SimpleMarker.prototype._getPolygonArray = function () {
    ...
    if (this._shape == dvt.SimpleMarker.TRIANGLE_UP) {
      ...
      ar = [cx - halfx, cy - sixthy, cx - sixthx, cy - sixthy, cx - sixthx, cy - halfy, cx + sixthx, cy - halfy, cx + sixthx, cy - sixthy, cx + halfx, cy - sixthy, cx + halfx, cy + sixthy, cx + sixthx, cy + sixthy, cx + sixthx, cy + halfy, cx - sixthx, cy + halfy, cx - sixthx, cy + sixthy, cx - halfx, cy + sixthy];
    } else if (this._shape == dvt.SimpleMarker.STAR) {
      ar = dvt.SimpleMarker._SHAPE_STAR_CMDS; // Scale and translate from center of (0,0)

      ar = dvt.PolygonUtils.scale(ar, this.getMaintainAspectRatio() ? this._s / 100 : this._width / 100, this.getMaintainAspectRatio() ? this._s / 100 : this._height / 100);
  ...
  dvt.SimpleMarker.prototype._getCmds = function () {
    ...
    if (this._shape == dvt.SimpleMarker.RECTANGLE) maintain = false;
  ...
  dvt.SimpleMarker.prototype._getBorderRadiusPath = function (context, cx, cy, width, height, borderRadius) {
    if (this._shape == dvt.SimpleMarker.SQUARE || this._shape == dvt.SimpleMarker.RECTANGLE) {
      ...
      var x = cx - w / 2;
      var y = cy - h / 2;
      var cmds = dvt.PathUtils.rectangleWithBorderRadius(x, y, w, h, borderRadius, s, '0');
  ...
  dvt.SimpleMarker.prototype.setDataColor = function (dataColor, bSkipStroke) {
    ...
    if (!bSkipStroke) {
      ...
      this.setHoverStroke(new dvt.Stroke(innerColor, 1, 1, true), new dvt.Stroke(hoverColor, 1, 3.5, true));
      this.setSelectedStroke(new dvt.Stroke(innerColor, 1, 1.5, true), new dvt.Stroke(outerColor, 1, 4.5, true));
      this.setSelectedHoverStroke(new dvt.Stroke(innerColor, 1, 1.5, true), new dvt.Stroke(hoverColor, 1, 4.5, true));
    }
  };
  /**
  ...
  dvt.SimpleMarker.prototype.copyShape = function () {
    return new dvt.SimpleMarker(this.getCtx(), this._shape, this.getCx(), this.getCy(), this.getWidth(), this.getHeight(), this._getBorderRadius(), this.getMaintainAspectRatio());
  };
  /**
   * @override
   */
  ...
  dvt.SimpleMarker.prototype.getDimensionsSelf = function (targetCoordinateSpace) {
    ...
    var bounds = new dvt.Rectangle(x, y, this.getWidth(), this.getHeight()); // Calculate the bounds relative to the target space
  ...
  dvt.ImageMarker.prototype.Init = function (context, cx, cy, width, height, borderRadius, source, sourceSelected, sourceHover, sourceHoverSelected, id) {
    ...
    this._updateBorder(borderRadius); // IE doesn't allow interactivity unless there's a fill
  ...
  dvt.ImageMarker.prototype._updateSvgProperties = function () {
    if (this._propertyChange.cx || this._propertyChange.width) dvt.ToolkitUtils.setAttrNullNS(this._elem, 'x', this._cx - this._width / 2, 0);
    if (this._propertyChange.cy || this._propertyChange.height) dvt.ToolkitUtils.setAttrNullNS(this._elem, 'y', this._cy - this._height / 2, 0);
    if (this._propertyChange.width) dvt.ToolkitUtils.setAttrNullNS(this._elem, 'width', this._width, 0);
    if (this._propertyChange.height) dvt.ToolkitUtils.setAttrNullNS(this._elem, 'height', this._height, 0); // redraws border radius clipPath and _borderPath element
  ...
  dvt.ImageMarker.prototype.showHoverEffect = function () {
    this.IsShowingHoverEffect = true;
    if (this.isSelected()) this._setSource(this._getImageHoverSelected());else this._setSource(this._getImageHover());
  };
  /**
   * @override
   */
  ...
  dvt.ImageMarker.prototype.hideHoverEffect = function () {
    this.IsShowingHoverEffect = false;
    if (this.isSelected()) this._setSource(this._getImageSelected());else this._setSource(this._getImage());
  };
  /**
   * @override
   */
  ...
  dvt.ImageMarker.prototype.setSelected = function (selected) {
    ...
    if (selected) {
      if (this.isHoverEffectShown()) this._setSource(this._getImageHoverSelected());else this._setSource(this._getImageSelected());
    } else {
  ...
  dvt.ImageMarker.prototype._updateBorder = function (borderRadius) {
    if (borderRadius || this._borderPath) {
      ...
      if (borderRadius && borderRadius != '0') {
        var clipCmds = dvt.PathUtils.rectangleWithBorderRadius(this._cx - this._width / 2, this._cy - this._height / 2, this._width, this._height, borderRadius, Math.min(this._width, this._height), '0');
      ...
      if (this._borderPath) {
        var borderCmds = dvt.PathUtils.rectangleWithBorderRadius(this._cx - this._width / 2 + this._borderWidth / 2, this._cy - this._height / 2 + this._borderWidth / 2, this._width - this._borderWidth, this._height - this._borderWidth, borderRadius, Math.min(this._width, this._height), '0');
  ...
  dvt.ImageMarker.prototype.setStroke = function (stroke) {
    if (!stroke || stroke.getWidth() <= 0) {
      this._borderWidth = 0; // Clear old border path if no new stroke.
    ...
    if (!this._borderPath || width != this._borderWidth) {
      ...
      var cmds = dvt.PathUtils.rectangleWithBorderRadius(this._cx - this._width / 2 + width / 2, this._cy - this._height / 2 + width / 2, this._width - width, this._height - width, this._borderRadius, Math.min(this._width, this._height), '0'); // Create and add _borderPath element

      if (this._borderPath) this._borderPath.setCmds(cmds);else {
  ...
  dvt.Path.prototype.Init = function (context, cmds, id) {
    ...
    if (Array.isArray(cmds)) this.setCmds(dvt.PathUtils.getPathString(cmds));else this.setCmds(cmds);
  };
  /**
  ...
  dvt.Path.prototype.setCmds = function (cmds) {
    if (cmds) this.SetSvgProperty('d', cmds); // Clear the cached array commands, which will be populated when needed.

    this.SetProperty('arCmds', null); // Return self for linking setters
  ...
  dvt.Path.prototype.setCommandsArray = function (arCmds) {
    ...
    this.setCmds(cmds); // Cache the array

    this.SetProperty('arCmds', arCmds); // Return self for linking setters
  ...
  dvt.Path.prototype.GetAriaElem = function () {
    if (dvt.Agent.isTouchDevice()) this.CreateChildGroupElem(false, true);
  ...
  dvt.Displayable.defineProps(dvt.Rect, {
    x: {
      value: 0
    },
    y: {
  ...
  dvt.Rect.prototype.setCSSStyle = function (style) {
    dvt.Rect.superclass.setCSSStyle.call(this, style); // TODO  CLEANUP: This code seems to apply to all shapes.  It's unclear why we only support it here.
  ...
  dvt.Rect.prototype._getStrokeOffset = function () {
    ...
    if (stroke) {
      ...
      if (this._strokeAlignment == 'outer') {
        strokeOffset = stroke.getWidth() / 2; // Get the inner rect stroke

        var innerStroke = this.GetHoverSelectionInnerStroke();
        if (innerStroke != null) strokeOffset += innerStroke.getWidth(); // When stroke alignment is INNER, the strokeOffset needs to account for outer stroke width
      } else if (this._strokeAlignment == 'inner') {
        strokeOffset = -stroke.getWidth() / 2; // Get the outer rect stroke
  ...
  dvt.Rect.prototype.setStroke = function (stroke) {
    dvt.Rect.superclass.setStroke.call(this, stroke); // Update element stroke dimensions if the stroke alignment is not CENTER
  ...
  dvt.Rect.prototype.GetElemDimensionsWithStroke = function () {
    var dims = dvt.Rect.superclass.GetElemDimensionsWithStroke.call(this); //Update the element dimesions if the stroke alignment is not CENTER
  ...
  dvt.OutputText.BIDI_ZERO_WIDTH = "\u200B"; // Horizontal Alignments
  ...
  dvt.OutputText.H_ALIGN_RIGHT = 'right'; // Vertical Alignments
  ...
  dvt.OutputText.prototype.Init = function (context, textStr, x, y, id) {
    dvt.OutputText.superclass.Init.call(this, context, 'text', id); // Attribute for IE alignment. This stores the component x position of the text.
    ...
    this._x = x != null ? x : 0; // - NODE RENDERED INCORRECTLY IN IE9+

    this._baseline = false; // Fix for 14297988: BIDI and mixed text in IE

    if (dvt.Agent.isRightToLeft(context) && (dvt.Agent.browser === 'ie' || dvt.Agent.browser === 'edge')) {
    ...
    this.setX(x);
    this.setY(y); // TODO : Remove this workaround and the incorrect none default in DvtSvgShape.
    // Workaround to remove some strange defaulting for the fill, which is set to none in DvtSvgShape.Init.

    dvt.ToolkitUtils.removeAttrNullNS(this.getElem(), 'fill'); // By default, hide text from VoiceOver
  ...
  dvt.OutputText.prototype.setTextString = function (textString) {
    ...
    textString = textString != null ? String(textString).trim() : ''; // Add zero-width char to force correct alignment in RTL for IE.

    if (this._needsTextAnchorAdjustment() && (dvt.Agent.browser === 'ie' || dvt.Agent.browser === 'edge') && textString.charAt(0) != dvt.OutputText.BIDI_ZERO_WIDTH) textString = dvt.OutputText.BIDI_ZERO_WIDTH + textString;
    this._textString = textString; // Update the text node if it is already created
    ...
    if (dvt.Agent.browser === 'ie' || dvt.Agent.browser === 'edge') this.setX(this._x); // readjust x position. IE text-anchor attribute is buggy.
  ...
  dvt.OutputText.prototype.getX = function () {
    var x;
    if (dvt.Agent.browser === 'ie' || dvt.Agent.browser === 'edge') x = this._x; // return the component x
  ...
  dvt.OutputText.prototype.setX = function (x) {
    this._x = x != null ? x : 0;
    if (dvt.Agent.browser === 'ie' || dvt.Agent.browser === 'edge') return this.SetSvgProperty('x', this._x + this._getIEAlignmentOffset(), 0);else return this.SetSvgProperty('x', x, 0);
  };
  /**
  ...
  dvt.OutputText.prototype._getIEAlignmentOffset = function () {
    ...
    if (align == dvt.OutputText.H_ALIGN_LEFT) return isRTL ? dvt.TextUtils.getTextStringWidth(this.getCtx(), this.getTextString(), this.getCSSStyle()) : 0;else if (align == dvt.OutputText.H_ALIGN_CENTER) return isRTL ? dvt.TextUtils.getTextStringWidth(this.getCtx(), this.getTextString(), this.getCSSStyle()) / 2 : -1 * (dvt.TextUtils.getTextStringWidth(this.getCtx(), this.getTextString(), this.getCSSStyle()) / 2);else if (align == dvt.OutputText.H_ALIGN_RIGHT) return isRTL ? 0 : -1 * dvt.TextUtils.getTextStringWidth(this.getCtx(), this.getTextString(), this.getCSSStyle());
  };
  /**
  ...
  dvt.OutputText.prototype.setFontSize = function (size) {
    ...
    style.setFontSize(dvt.CSSStyle.FONT_SIZE, String(size), this.getCtx());
  ...
  dvt.OutputText.prototype.setHorizAlignment = function (align) {
    if (align == dvt.OutputText.H_ALIGN_LEFT) this.alignLeft();else if (align == dvt.OutputText.H_ALIGN_CENTER) this.alignCenter();else if (align == dvt.OutputText.H_ALIGN_RIGHT) this.alignRight();
  };
  /**
  ...
  dvt.OutputText.prototype.setVertAlignment = function (align) {
    if (align == dvt.OutputText.V_ALIGN_TOP) this.alignTop();else if (align == dvt.OutputText.V_ALIGN_MIDDLE) this.alignMiddle();else if (align == dvt.OutputText.V_ALIGN_BOTTOM) this.alignBottom();else if (align == dvt.OutputText.V_ALIGN_AUTO) this.alignAuto();
  };
  /**
  ...
  dvt.OutputText.prototype.alignLeft = function () {
    ...
    if (this._horizAlign == dvt.OutputText.H_ALIGN_LEFT) return;
  ...
  dvt.OutputText.prototype.alignCenter = function () {
    // No change in value, return
    if (this._horizAlign == dvt.OutputText.H_ALIGN_CENTER) return;
    this._horizAlign = dvt.OutputText.H_ALIGN_CENTER;
    if (dvt.Agent.browser === 'ie' || dvt.Agent.browser === 'edge') this.setX(this._x); // readjust x position. IE text-anchor attribute is buggy.
    else dvt.ToolkitUtils.setAttrNullNS(this.getElem(), 'text-anchor', 'middle', 'start');
  };
  /**
  ...
  dvt.OutputText.prototype.alignRight = function () {
    // No change in value, return
    if (this._horizAlign == dvt.OutputText.H_ALIGN_RIGHT) return;
  ...
  dvt.OutputText.prototype.alignTop = function () {
    // No change in value, return
    if (this._vertAlign == dvt.OutputText.V_ALIGN_TOP) return;else if (this._vertAlign == dvt.OutputText.V_ALIGN_BOTTOM && dvt.Agent.browser === 'safari') this._setBaseline(false);
    this._vertAlign = dvt.OutputText.V_ALIGN_TOP; // - NODE RENDERED INCORRECTLY IN IE9+
  ...
  dvt.OutputText.prototype.alignMiddle = function () {
    // No change in value, return
    if (this._vertAlign == dvt.OutputText.V_ALIGN_MIDDLE) return;else if (this._vertAlign == dvt.OutputText.V_ALIGN_BOTTOM && dvt.Agent.browser === 'safari') this._setBaseline(false);
    this._vertAlign = dvt.OutputText.V_ALIGN_MIDDLE; // - NODE RENDERED INCORRECTLY IN IE9+
  ...
  dvt.OutputText.prototype.alignBottom = function () {
    // No change in value, return
    if (this._vertAlign == dvt.OutputText.V_ALIGN_BOTTOM) return;
    this._vertAlign = dvt.OutputText.V_ALIGN_BOTTOM; // - NODE RENDERED INCORRECTLY IN IE9+

    if (dvt.Agent.browser === 'ie' || dvt.Agent.browser === 'edge') {
      this._setBaseline(true);
    } else if (dvt.Agent.browser === 'safari') {
  ...
  dvt.OutputText.prototype.alignAuto = function () {
    // No change in value, return
    if (this._vertAlign == dvt.OutputText.V_ALIGN_AUTO) return;else if (this._vertAlign == dvt.OutputText.V_ALIGN_BOTTOM && dvt.Agent.browser === 'safari') this._setBaseline(false);
    this._vertAlign = dvt.OutputText.V_ALIGN_AUTO;
    if (dvt.Agent.browser === 'ie' || dvt.Agent.browser === 'edge') this._setBaseline(false);else this.SetDominantBaselineAttr(null);
  };
  /**
  ...
  dvt.OutputText.prototype.SetDominantBaselineAttr = function (baseline) {
    if (baseline) dvt.ToolkitUtils.setAttrNullNS(this.getElem(), 'dominant-baseline', baseline);else dvt.ToolkitUtils.removeAttrNullNS(this.getElem(), 'dominant-baseline');
  };
  /**
   * Sets the dvt.CSSStyle of this object.
   * @param {dvt.CSSStyle} style The dvt.CSSStyle of this object.
  ...
  dvt.OutputText.prototype.setCSSStyle = function (style) {
    ...
    if (style) {
      ...
      if (fillObj && fillObj['color']) {
        ...
        if (fillObj['alpha'] != null) dvt.ToolkitUtils.setAttrNullNS(elem, 'fill-opacity', fillObj['alpha'], 1);
      }

      val = style.getStyle('font-family');
      if (val && !this.getCtx().isDefaultFontFamily(val)) dvt.ToolkitUtils.setAttrNullNS(elem, 'font-family', val);else dvt.ToolkitUtils.removeAttrNullNS(elem, 'font-family');
      val = style.getStyle('font-size');

      if (val && val != this.getCtx().getDefaultFontSize()) {
        dvt.ToolkitUtils.setAttrNullNS(elem, 'font-size', val); // - NODE RENDERED INCORRECTLY IN IE9+

        if (dvt.Agent.browser === 'ie' || dvt.Agent.browser === 'edge' || dvt.Agent.browser === 'safari') {
          this.setMatrix(this.getMatrix());
        }
      } else dvt.ToolkitUtils.removeAttrNullNS(elem, 'font-size');
      ...
      if (val && val != this.getCtx().getDefaultFontWeight()) dvt.ToolkitUtils.setAttrNullNS(elem, 'font-weight', val);else dvt.ToolkitUtils.removeAttrNullNS(elem, 'font-weight'); //NOTE: svg does not recognize css "text-align" attribute,
  ...
  dvt.OutputText.prototype._getBaselineTranslation = function () {
    ...
    if (this._baseline) {
      ...
      if (this._textString && !this._isRepresentativeText()) return dvt.TextUtils.getBaselineTranslation(this);else {
        var valign = this.getVertAlignment();

        if (valign != dvt.OutputText.V_ALIGN_AUTO) {
          var size = dvt.ToolkitUtils.getAttrNullNS(this.getElem(), 'font-size');
          if (!size) size = dvt.ToolkitUtils.getAttrNullNS(this.getOuterElem(), 'font-size');
          if (!size || isNaN(size) && size.indexOf('px') == -1) size = this.getCtx().getDefaultFontSize();
  ...
  dvt.OutputText.prototype._isRepresentativeText = function () {
    return this._textString == dvt.OutputText.REPRESENTATIVE_TEXT || this._textString == dvt.OutputText.BIDI_ZERO_WIDTH || this._textString == dvt.OutputText.BIDI_ZERO_WIDTH + dvt.OutputText.REPRESENTATIVE_TEXT;
  };
  /**
   * @override
   */
  ...
  dvt.OutputText.prototype.GetSvgDimensions = function () {
    // TODO  target coord space would be broken here
    var bbox = dvt.OutputText.superclass.GetSvgDimensions.call(this); //  - NODE RENDERED INCORRECTLY IN IE9+

    if (bbox) {
      bbox.y += this._getBaselineTranslation(); //  - Japanese group label doesn't display in IE11

      if (!this._isRepresentativeText()) bbox.h = dvt.TextUtils.getTextStringHeight(this.getCtx(), this.getCSSStyle());
    }
  ...
  dvt.OutputText.prototype.UpdateSelectionEffect = function () {// noop: Does not participate in selection effects
  };
  /**
   * @override
   */
  ...
  dvt.OutputText.prototype.getDimensions = function (targetCoordinateSpace) {
    ...
    localDims.x = this.getX(); // Adjust dimensions for text alignment.  We do this because we don't take alignment into account in the cache, and
    // also because browsers return inconsistently calculated dimensions based on alignment.

    if (hAlign === dvt.OutputText.H_ALIGN_RIGHT) localDims.x -= localDims.w;else if (hAlign === dvt.OutputText.H_ALIGN_CENTER) localDims.x -= localDims.w / 2;
    if (vAlign === dvt.OutputText.V_ALIGN_TOP) localDims.y = this.getY();else if (vAlign === dvt.OutputText.V_ALIGN_BOTTOM) localDims.y = this.getY() - localDims.h;else if (vAlign === dvt.OutputText.V_ALIGN_MIDDLE) localDims.y = this.getY() - localDims.h / 2;else if (vAlign === dvt.OutputText.V_ALIGN_AUTO) localDims.y += this.getY(); // Transform to the target coord space and return
  ...
  dvt.BackgroundOutputText.prototype.setFontSize = function (size) {
    this.TextInstance.setFontSize(size); // Also update the local copy of DvtCssStyle

    this._style.setFontSize(dvt.CSSStyle.FONT_SIZE, String(size), this.getCtx());
  ...
  dvt.BackgroundOutputText.prototype.setCSSStyle = function (style) {
    ...
    if (style.getStyle(dvt.CSSStyle.BACKGROUND_COLOR) && !style.getStyle(dvt.CSSStyle.COLOR)) style.setStyle(dvt.CSSStyle.COLOR, dvt.ColorUtils.getContrastingTextColor(style.getStyle(dvt.CSSStyle.BACKGROUND_COLOR)));
  ...
  dvt.BackgroundOutputText.prototype._realignBackground = function () {
    ...
    var hAlign = this.getHorizAlignment();
    var vAlign = this.getVertAlignment(); // Align x-Coordinate

    if (hAlign == dvt.OutputText.H_ALIGN_LEFT) this._backgroundRect.setX(this.getX() - padding);else if (hAlign == dvt.OutputText.H_ALIGN_CENTER) this._backgroundRect.setX(this.getX() - bboxDims.w * 0.5 - padding);else if (hAlign == dvt.OutputText.H_ALIGN_RIGHT) this._backgroundRect.setX(this.getX() - bboxDims.w - padding); // Align y-Coordinate
    ...
    if (dvt.Agent.engine === 'blink' && vAlign == dvt.OutputText.V_ALIGN_MIDDLE) yAdjustment = fontSize * 0.12; // Chrome, vALign = middle, rect must be shifted up a bit
    else if ((dvt.Agent.browser === 'ie' || dvt.Agent.browser === 'edge') && vAlign == dvt.OutputText.V_ALIGN_BOTTOM) yAdjustment = -fontSize * 0.4; // IE, vAlign = bottom, rect must be shifted down a bit

    var deltaH = this.GetTextDimensionsForRealign(bboxDims);
    if (vAlign == dvt.OutputText.V_ALIGN_TOP) this._backgroundRect.setY(this.getY());else if (vAlign == dvt.OutputText.V_ALIGN_MIDDLE) this._backgroundRect.setY(this.getY() - deltaH * 0.5 - yAdjustment);else if (vAlign == dvt.OutputText.V_ALIGN_BOTTOM) this._backgroundRect.setY(this.getY() - deltaH - yAdjustment);else if (vAlign == dvt.OutputText.V_ALIGN_AUTO) this._backgroundRect.setY(bboxDims.y); // width may need to be reset (rotated labels ended up extra long)
  ...
  dvt.BackgroundOutputText.prototype._setBackgroundCSSStyle = function (rect, style) {
    if (style) {
      ...
      if (bgColor != null) rect.setSolidFill(bgColor);else rect.setInvisibleFill();
  ...
  dvt.MultilineText._LINE_SPACE = 0; // Horizontal Alignments
  ...
  dvt.MultilineText.H_ALIGN_RIGHT = 'right'; // Vertical Alignments
  ...
  dvt.MultilineText.prototype.Init = function (context, textStr, x, y, id, preserveNewLine) {
    ...
    this._preservedLines = []; // Apply properties
  ...
  dvt.MultilineText.prototype.setTextString = function (textString) {
    ...
    if (!this._textInstance) {
      ...
      this._textInstance.setTextString(textString); // Remove any additional text lines
  ...
  dvt.MultilineText.prototype._getTextLines = function () {
    var lines = this._additionalLines.slice(); // Add this._textInstance unless it was removed by a fitText call
  ...
  dvt.MultilineText.prototype.setY = function (y) {
    this._yCoord = y; // If not using vertical align, use default alignment for the multiline text
  ...
  dvt.MultilineText.prototype.alignLeft = function () {
    // No change in value, return
    if (this._horizAlign == dvt.MultilineText.H_ALIGN_LEFT) return;
  ...
  dvt.MultilineText.prototype.alignCenter = function () {
    // No change in value, return
    if (this._horizAlign == dvt.MultilineText.H_ALIGN_CENTER) return;
  ...
  dvt.MultilineText.prototype.alignRight = function () {
    // No change in value, return
    if (this._horizAlign == dvt.MultilineText.H_ALIGN_RIGHT) return;
  ...
  dvt.MultilineText.prototype.alignTop = function () {
    // No change in value, return
    if (this._vertAlign == dvt.MultilineText.V_ALIGN_TOP) return;
  ...
  dvt.MultilineText.prototype.alignMiddle = function () {
    // No change in value, return
    if (this._vertAlign == dvt.MultilineText.V_ALIGN_MIDDLE) return;
  ...
  dvt.MultilineText.prototype.alignBottom = function () {
    // No change in value, return
    if (this._vertAlign == dvt.MultilineText.V_ALIGN_BOTTOM) return;
  ...
  dvt.MultilineText.prototype._alignVerticalText = function () {
    ...
    for (var i = 0; i < textLines.length; i++) {
      ...
      if (this._vertAlign == dvt.MultilineText.V_ALIGN_TOP) adjustment = lineHeight * i;else if (this._vertAlign == dvt.MultilineText.V_ALIGN_MIDDLE) {
        var range = lineHeight * (lineCount - 1);
        adjustment = lineHeight * i - range / 2;
      } else if (this._vertAlign == dvt.MultilineText.V_ALIGN_BOTTOM) adjustment = -lineHeight * (lineCount - i - 1);
  ...
  dvt.MultilineText.prototype.__fitText = function (maxWidth, maxHeight, minChars) {
    if (!this._preserveNewLine) {
      ...
      this._removeAdditionalLines(); // First try to fit the entire string in 1 line
      ...
      if (!dvt.TextUtils.fitText(this._textInstance, maxWidth, maxHeight, this, minChars)) {
        ...
        return true;else if (!this.isWrapEnabled()) // Accept text that fits partially
        return true;
  ...
  dvt.MultilineText.prototype.wrapText = function (maxWidth, maxHeight, minChars, breakOnTruncation) {
    if (this.getLineCount() > 1) // Clear any previous wrapping results
      this._removeAdditionalLines(); // Calculate line height so that we can add additional lines as space allows.

    var lineHeight = this.getLineHeight();
    var lineSpace = lineHeight * dvt.MultilineText._LINE_SPACE;
    var maxLines = this.getMaxLines();
    var availHeight = Math.min(maxHeight, maxLines * (lineHeight + lineSpace) - lineSpace) - lineHeight - lineSpace; // We are currently splitting MultilineText wrapping into two exclusive behaviors
    ...
    if (this._preserveNewLine) {
      ...
      for (var i = 0; i < this._preservedLines.length; i++) {
        var currentLine = this._preservedLines[i]; // Consider text truncated and return if we still have a line, but not enough availHeight
        ...
        if (dvt.TextUtils.fitText(currentLine, maxWidth, Infinity, this, minChars)) {
          ...
          availHeight -= lineSpace + lineHeight; // Consider the text truncated if any line is truncated

          if (currentLine.isTruncated() && !this.isTruncated()) this._bTruncated = true;

          if (i > 0) {
            this._additionalLines.push(currentLine); // Offset the new line from the previousLine, if none fit so far this line will be at the top
      ...
      var lastTruncatedString = null; // Split the string into parts.  Reverse the array so that we can use like a queue.
      ...
      var moveToNewLine = function () {
        ...
        this._additionalLines.push(newLine); // Update the pointers to the line and the availHeight


        currentLine = newLine;
        currentString = null;
        availHeight -= lineSpace + lineHeight;
      }.bind(this); // Loop and add each split into the current line.  Create a new line once the current one is full.
        ...
        if (!dvt.TextUtils.fitText(currentLine, maxWidth, Infinity, this, minChars, true)) {
          ...
          //    it is better to drop the text than have a line with no ellipsis(that would indicate dropped lines).
              ...
              return false;
                ...
                lastTruncatedLine.setTextString(lastTruncatedString); // Roll back to the last successful truncated line
          ...
          lastTruncatedLine = currentLine; // Decide whether to keep this split in the current line or push to the next
  ...
  dvt.MultilineText.prototype.GetSvgDimensions = function (targetCoordinateSpace) {
    ...
    if (textLines.length == 0) return new dvt.Rectangle(0, 0, 0, 0);
  ...
  dvt.MultilineText.prototype.getLineHeight = function () {
    if (!this._lineHeight) this._lineHeight = dvt.TextUtils.getTextStringHeight(this.getCtx(), this.getCSSStyle());
  ...
  dvt.MultilineText.prototype.setUntruncatedTextString = function (textString) {// noop
  };
  /**
   * @override
   */
  ...
  dvt.MultilineText.prototype.UpdateSelectionEffect = function () {// noop: Does not participate in selection effects
  };
  /**
  ...
  dvt.MultilineText.prototype.setHorizAlignment = function (align) {
    if (align == dvt.MultilineText.H_ALIGN_LEFT) this.alignLeft();else if (align == dvt.MultilineText.H_ALIGN_CENTER) this.alignCenter();else if (align == dvt.MultilineText.H_ALIGN_RIGHT) this.alignRight();
  };
  /**
  ...
  dvt.MultilineText.prototype.setVertAlignment = function (align) {
    if (align == dvt.MultilineText.V_ALIGN_TOP) this.alignTop();else if (align == dvt.MultilineText.V_ALIGN_MIDDLE) this.alignMiddle();else if (align == dvt.MultilineText.V_ALIGN_BOTTOM) this.alignBottom();
  };
  /**
   * @license
   * Copyright (c) 2008 %CURRENT_YEAR%, Oracle and/or its affiliates.
  ...
  dvt.BackgroundMultilineText.prototype.UpdateSelectionEffect = function () {// noop: Does not participate in selection effects
  };
  /**
   * @license
   * Copyright (c) 2008 %CURRENT_YEAR%, Oracle and/or its affiliates.
  ...
  dvt.Use.prototype.Init = function (context, x, y, target, id) {
    ...
    if (context.existsGlobalDefReference(targetId)) context.increaseGlobalDefReference(targetId);else {
  ...
  dvt.Displayable.defineProps(dvt.Use, {
    x: {
      value: 0
    },
    y: {
  ...
  var DvtLinearGradient = function DvtLinearGradient() {
    this.Init();
  };
  /*
  ...
  dvt.Obj.createSubclass(DvtLinearGradient, dvt.CSSGradient); //convert sides into an angle (in radian)
  ...
  dvt.Obj.createSubclass(dvt.GradientParser, dvt.Obj); //W3C CSS3 gradient
  ...
  dvt.GradientParser.parseCSSGradient = function (gradient) {
    if (gradient != null) {
      ...
      if (dvt.GradientParser._startsWith(gradient, dvt.GradientParser.GD_LINEAR)) {
  ...
  dvt.GradientParser._parseLinearGradient = function (gradient) {
    ...
    if (sides == null || sides.length == 0) return null; // sides
    // 0: match string
    // 1: angle
    // 2: angle unit
    // 3: to (if present)
  ...
  dvt.GradientParser._removeParenthesis = function (gradient, keyWord) {
    if (gradient.charAt(gradient.length - 1) != ')') return null; // remove keyWord and parethesis around it
  ...
  dvt.CSSStyle.SKIN_ALTA = 'alta'; // Background Properties
  ...
  dvt.CSSStyle.BACKGROUND_POSITION = 'background-position'; // Border Properties
  ...
  dvt.CSSStyle.BORDER_STYLE = 'border-style'; // - border-radius css property not supported when used inside <dvt:node>
  ...
  dvt.CSSStyle.BORDER_BOTTOM_LEFT_RADIUS = 'border-bottom-left-radius'; // Margin Properties
  ...
  dvt.CSSStyle.MARGIN_RIGHT = 'margin-right'; // Padding Properties
  ...
  dvt.CSSStyle.PADDING_RIGHT = 'padding-right'; // Font Properties
  ...
  dvt.CSSStyle.TEXT_ALIGN = 'text-align'; // Size Properties

  dvt.CSSStyle.HEIGHT = 'height';
  dvt.CSSStyle.WIDTH = 'width';
  dvt.CSSStyle.MAX_WIDTH = 'max-width'; //values for background-repeat

  dvt.CSSStyle.NO_REPEAT = 'no-repeat';
  dvt.CSSStyle.REPEAT = 'repeat';
  dvt.CSSStyle.REPEAT_X = 'repeat-x';
  dvt.CSSStyle.REPEAT_Y = 'repeat-y'; //value for margin
  ...
  dvt.CSSStyle._SINGLE_QUOTE = "'"; // Default color ramps
  ...
  dvt.CSSStyle.prototype.setStyle = function (key, val) {
    ...
    if (val == null) this._setStyleAttr(key, val);else {
  ...
  dvt.CSSStyle.prototype._setStyleAttr = function (key, value) {
    if (value != null) this._styleMap[key] = value;else delete this._styleMap[key];
    return this;
  };
  /**
  ...
  dvt.CSSStyle.prototype.setFontSize = function (key, val, context) {
    var fsize = val.trim();
    ...
    if (specSize) {
      ...
      // The context object is not passed from setStyle() method on dvt.CSSStyle.
      // The constant is used as default value in this case.
      var defaultFontSize = context ? context.getDefaultFontSize() : dvt.Context.DEFAULT_FONT_SIZE;
  ...
  dvt.CSSStyle.prototype._setBorderWidthShorthand = function (key, val) {
    ...
    switch (bwArray.length) {
      case 1:
        this._setStyleAttr(key, dvt.CSSStyle._getBorderWidth(bwArray[0])); // remove other border widths
  ...
  dvt.CSSStyle.prototype._setBorderSide = function (key, val) {
    ...
    for (var i = 0; i < borderArray.length; i++) {
      ...
      if (borderVal.indexOf('rgb') == 0) {
        ...
        if (!key.endsWith('-' + dvt.CSSStyle.COLOR)) key1 = key + '-' + dvt.CSSStyle.COLOR;
        ...
        this._setStyleAttr(key1, borderVal);
          ...
          this._setStyleAttr(key1, dvt.CSSStyle._parseHexColor(borderVal));
            ...
            if (!key.endsWith('-' + dvt.CSSStyle.WIDTH)) key1 = key + '-' + dvt.CSSStyle.WIDTH;
  ...
  dvt.CSSStyle.prototype._getMargin = function (val) {
    var margin = val.trim();
    if (dvt.CSSStyle.AUTO == margin) return dvt.CSSStyle.AUTO_MARGIN;else return this._getPercent(margin);
  ...
  dvt.CSSStyle.prototype._setBorderShorthand = function (key, val) {
    ...
    for (var i = 0; i < borderArray.length; i++) {
      borderVal = borderArray[i]; //rgb border color
      ...
      else if (dvt.CSSStyle._isColorValue(borderVal)) this._setBorderColorShorthand(dvt.CSSStyle.BORDER_COLOR, borderVal); //border width
        else if (dvt.CSSStyle._isBorderWidthValue(borderVal)) this._setBorderWidthShorthand(dvt.CSSStyle.BORDER_WIDTH, borderVal); //we don't currently handle border style, but if it's none, set width=0
          else if (val == 'none') noBorder = true;
    }
  ...
  dvt.CSSStyle.prototype._setBorderRadius = function (key, val) {
    ...
    switch (brHorzArr.length) {
      case 1:
      default:
        this._setStyleAttr(key, dvt.CSSStyle._getBorderRadius(brHorzArr[0])); // remove other border radius
    ...
    if (brArray.length == 2 && brArray[1] != null) {
      ...
      switch (brVertArr.length) {
        case 1:
          if (this.getStyle(key)) this._setStyleAttr(key, this.getStyle(key) + ' ' + dvt.CSSStyle._getBorderRadius(brVertArr[0]));else this._setStyleAttr(key, dvt.CSSStyle._getBorderRadius(brVertArr[0]));
          break;
  ...
  dvt.CSSStyle.prototype.getBorderRadius = function () {
    if (this.getStyle(dvt.CSSStyle.BORDER_RADIUS) !== 'undefined' && this.getStyle(dvt.CSSStyle.BORDER_RADIUS) !== null) return dvt.CSSStyle.toNumber(this.getStyle(dvt.CSSStyle.BORDER_RADIUS));else return 0;
  };

  dvt.CSSStyle.prototype._setBackgroundRepeat = function (key, val) {
    var a = val.split(' '); //  - buttons look broken on safari
    // in Safari button -> background-repeat: repeat no-repeat

    if (a.length == 2) {
      // both repeat or no_repeat
      if (a[0] == a[1]) val = a[0];else if (a[0] == dvt.CSSStyle.REPEAT && a[1] == dvt.CSSStyle.NO_REPEAT) val = dvt.CSSStyle.REPEAT_X;
      if (a[0] == dvt.CSSStyle.NO_REPEAT && a[1] == dvt.CSSStyle.REPEAT) val = dvt.CSSStyle.REPEAT_Y;
    }
  ...
  dvt.CSSStyle.prototype._setColorAttr = function (key, val) {
    ...
    var color = dvt.ColorUtils.getColorFromName(val);
  ...
  dvt.CSSStyle.prototype.hasBackgroundStyles = function () {
    if (this.getStyle(dvt.CSSStyle.BORDER_COLOR) || this.getStyle(dvt.CSSStyle.BORDER_WIDTH) || this.getStyle(dvt.CSSStyle.BORDER_RADIUS) || this.getStyle(dvt.CSSStyle.BACKGROUND_COLOR)) return true;
    return false;
  };
  /**
  ...
  dvt.CSSStyle._isColorValue = function (val) {
    val = val.trim();
    if (val.indexOf('rgb') == 0 || val.indexOf('#') == 0) return true; // color name?
  ...
  dvt.CSSStyle._isBorderWidthValue = function (val) {
    var bwidth = val.trim();
    if (dvt.CSSStyle._NAMED_WIDTH_MAP[bwidth]) return true;
  ...
  dvt.CSSStyle.parseBorderColor = function (val, cArray) {
    ...
    if (val.indexOf('rgb') == 0) {
      ...
      colorObj = val.slice(0, endIndex);
        ...
        colorObj = dvt.ColorUtils.getColorFromName(singleColor); // hex color
  ...
  dvt.CSSStyle._parseHexColor = function (val) {
    if (!val) return null; // assume black if we didn't get a valid numeric string or hex format
  ...
  dvt.CSSStyle.getTextMeasurementProperties = function () {
    return [dvt.CSSStyle.FONT_FAMILY, dvt.CSSStyle.FONT_SIZE, dvt.CSSStyle.FONT_STYLE, dvt.CSSStyle.FONT_WEIGHT];
  };
  /**
  ...
  dvt.CSSStyle.cssStringToObjectProperty = function (property) {
    if (property) property = property.replace(/-([a-z])/g, function (g) {
  ...
  dvt.CSSStyle.getTimeMilliseconds = function (value) {
    if (typeof value == 'string') {
      if (value.slice(-2) == 'ms') value = parseInt(value.slice(0, -2));else if (value.slice(-1) == 's') value = parseFloat(value.slice(0, -1)) * 1000;else value = parseInt(value);
    }
  ...
  dvt.CSSStyle.prototype.applyStylesToElement = function (element) {
    dvt.CSSStyle.applyCssObjectToElement(this._styleMap, element);
  };
  /**
   * Applies the contained styles to the specified element in a CSP-safe way
   */
  ...
  dvt.CSSStyle.applyCssObjectToElement = function (style, element) {
    var keys = Object.keys(style);

    for (var i = 0; i < keys.length; i++) {
      var key = keys[i];
      element.style[dvt.CSSStyle.cssStringToObjectProperty(key)] = style[key];
    }
  };
  /**
   * @license
  ...
  dvt.BaseComponent = function () {// Components should implement newInstance factory methods instead of exposing contructors.
  };
  ...
  dvt.BaseComponent.prototype.UpdateAriaAttributes = function () {
    if (this.IsParentRoot()) {
      var translations = this.getOptions().translations; // Set up the aria role and label for this component for accessibility without deferring
  ...
  dvt.BaseComponent.prototype.SetOptions = function (options) {// Subclasses should override
  };
  /**
  ...
  dvt.BaseComponent.prototype.applyOptions = function (options, noClone) {
    if (!this.Options || !options) return; // Merge the new properties with the existing ones and set
  ...
  dvt.BaseComponent.prototype.render = function (options, width, height) {// Subclasses should override
  };
  /**
  ...
  dvt.BaseComponent.prototype.RenderComplete = function () {
    ...
    this.Animation = null;
    this.AnimationStopped = false; // Reset the cached isOffscreen check

    this.getCtx().resetIsOffscreen(); // Finally fire the ready event
  ...
  dvt.BaseComponent.prototype.highlight = function (categories) {// subclasses should override if highlightedCategories is supported. Called from JET to avoid re-render.
  };
  /**
  ...
  dvt.BaseComponent.prototype.select = function (selection) {// subclasses should override if selection is supported. Called from JET to avoid re-render.
  };
  /**
  ...
  dvt.BaseComponent.prototype.appendSharedDefs = function (elem) {
    ...
    if (!sharedDefsDiv) {
      ...
      sharedDefsDiv.appendChild(svg); // Add the div to the end of body (following what DVT tooltips do).
  ...
  dvt.BaseComponent.prototype.destroy = function () {
    ...
    var sharedDefsDiv = document.getElementById(this._SHARED_DEFS_DIV_ID);
    if (isLastDVTComponent && sharedDefsDiv) document.body.removeChild(sharedDefsDiv); // Always call superclass last for destroy
  ...
  dvt.BaseComponentDefaults._SKINS = [dvt.CSSStyle.SKIN_ALTA];
  /**
  ...
  dvt.BaseComponentDefaults.prototype.Init = function (defaultsMap, context) {
    ...
    this._context = context; // Initialize the defaults cache on the class if it doesn't already exist. This is used to prevent excess cloning and
  ...
  dvt.BaseComponentDefaults.prototype.calcOptions = function (userOptions) {
    ...
    var defaults = this.getDefaults(skin); // Use defaults if no overrides specified

    if (!userOptions) return defaults; // Get component noclone options and add data provider properties
    ...
    var options = dvt.JsonUtils.merge(userOptions, defaults, noClone); //  - short circuit animation if animation duration is 0
  ...
  dvt.BaseComponentDefaults.prototype.getDefaults = function (skin) {
    if (!skin) skin = this.getBaseSkinName(); // Return the cached defaults if available.

    if (this.constructor.defaultsCache[skin]) return dvt.JsonUtils.clone(this.constructor.defaultsCache[skin]);else {
      // Note: Subsequent default objects are deltas on top of previous objects
      var skinIndex = Math.max(dvt.BaseComponentDefaults._SKINS.indexOf(skin), 0);
  ...
  dvt.SimpleScrollbar.prototype.render = function (options, width, height) {
    ...
    this.addChild(background); // Create thumb
    ...
    this._thumbMax = this._globalMax; // The renderedThumbMin/Max can be wider than the actual thumbMin/Max if the viewport range is very small

    this._renderedThumbMin = this._thumbMin;
    this._renderedThumbMax = this._thumbMax; // Add hit area to make interaction with scrollbar easier
    ...
    this.addChild(hitArea); // Add event listeners
  ...
  dvt.SimpleScrollbar.prototype.setViewportRange = function (min, max, globalMin, globalMax) {
    ...
    if (globalMax != null) this._globalMax = globalMax; // Store the actual min/max before minRange adjustments

    this._thumbMin = min;
    this._thumbMax = max; // For usability, make sure that the thumb is not shorter than the minimum number of pixels
    ...
    this._renderedThumbMax = max; // Get the coords and modify the thumb
  ...
  dvt.SimpleScrollbar.prototype._getCoord = function (value) {
    ...
    if (this._isHoriz) return ratio * this._width;else return ratio * this._height;
  };
  /**
  ...
  dvt.SimpleScrollbar.prototype._onDragStart = function (event) {
    ...
    } else val = this._getValue(event.pageX, event.pageY); // Bypass thumb checking logic if we're scrolling on an outer container
  ...
  dvt.SimpleScrollbar.prototype._onDragMove = function (event) {
    ...
    var dragOffset = val - this._prevVal; // If the drag target is not the scrollbar, the thumb should move in the opposite direction as the drag target
  ...
  dvt.SimpleScrollbar.prototype._onDragEnd = function (event) {
    if (!this._dragged) return;
    if (event.touches) this._setViewportRange(this._thumbMin, this._thumbMax, 'end');else {
      var val = this._getValue(event.pageX, event.pageY);
  ...
  dvt.SimpleScrollbar.prototype._setViewportRange = function (min, max, eventSubtype) {
    ...
    this.setViewportRange(min, max); // Fire event

    if (this._callback) {
      var evt = dvt.EventFactory.newSimpleScrollbarEvent(eventSubtype, min, max);
  ...
  dvt.SimpleScrollableContainer.prototype.prepareContentPane = function () {
    ...
    if (this._isHorizontalScrolling) this._contentSize = size.w + size.x;else this._contentSize = size.h + size.y; // TODO 
    ...
    if (this.hasScrollingContent()) {
      ...
      if (this._isHorizontalScrolling) this._contentSize += size.x;else this._contentSize += size.y; // create masks and scrollbars
      ...
      this._maskContainer.setMask(this._fadeBottom); // Creates the scrollbar
      ...
      if (this._isHorizontalScrolling) {
        ...
        this._scrollbar.setTranslate(dvt.Agent.isRightToLeft(this.getCtx()) ? dvt.SimpleScrollableContainer._SCROLLBAR_GAP : this._width - (dvt.SimpleScrollableContainer._SCROLLBAR_WIDTH + dvt.SimpleScrollableContainer._SCROLLBAR_GAP), dvt.SimpleScrollableContainer._SCROLLBAR_GAP);
      }

      this._scrollbar.setViewportRange(0, this._getAvailSize()); // make scrollbars visible for tests


      if (!dvt.Agent.isEnvironmentTest()) this._scrollbar.setAlpha(0);
  ...
  dvt.SimpleScrollableContainer.prototype._updateFade = function (translate) {
    if (translate == 0) // scrolled to the top, show bottom mask only
      this._maskContainer.setMask(this._fadeBottom);else if (translate + this._getAvailSize() >= this._contentSize) // scrolled to the bottom, show top mask only
      this._maskContainer.setMask(this._fadeTop);else // scrolled to the middle, show mask at top and bottom
  ...
  dvt.SimpleScrollableContainer.prototype._handleScrollbar = function (event) {
    ...
    if (this._isHorizontalScrolling) this._container.setTranslateX(event.newMin * -1);else this._container.setTranslateY(event.newMin * -1);
  ...
  dvt.Obj.createSubclass(dvt.DragSource, dvt.Obj); // Margin to add to drag feedback bounds to minimize clipping
  ...
  dvt.DragSource.prototype.Init = function (context) {
    this._context = context; // current draggable object under the mouse pointer

    this._dragCandidate = null; // object being dragged
  ...
  dvt.DragSource.prototype.setDragCandidate = function (obj) {
    ...
    if (obj && obj.isDragAvailable && obj.getDragTransferable) this._dragCandidate = obj;else this._dragCandidate = null;
  };
  /**
  ...
  dvt.DragSource.prototype.getDragOverFeedback = function (mouseX, mouseY) {
    ...
    this._context.getTooltipManager().hideTooltip(); // Return the drag feedback


    var dragObj = this.getDragObject();
    if (dragObj && dragObj.getDragFeedback) return dragObj.getDragFeedback(mouseX, mouseY);else return null;
  };
  /**
  ...
  dvt.DragSource.prototype.getDragOffset = function (mouseX, mouseY) {
    ...
    if (feedback) {
      var bounds = dvt.DragAndDropUtils.getDragFeedbackBounds(feedback, this._context.getStage()); //return displayables to their original condition, if necessary,
  ...
  dvt.DragSource.prototype.initiateDrag = function () {
    ...
    if (dvt.Agent.isTouchDevice() && tm) tm.cancelTouchHold();
  };
  /**
  ...
  dvt.Button.prototype.Init = function (context, upState, overState, downState, disabledState, id, callback, callbackObj) {
    ...
    this.setCallback(callback, callbackObj); //: don't do anything special in setUpState() during construction
    ...
    this._bToggled = false; //: don't do anything special in setUpState() during construction
  ...
  dvt.Button.prototype.setEnabled = function (bEnabled) {
    if (this._enabled != bEnabled) {
      ...
      var clickEvent = isTouchDevice ? dvt.TouchEvent.TOUCHSTART : dvt.MouseEvent.CLICK; // Set passive: false for touch start event, so that preventDefault can be called on the event.

      var clickEventOptions = isTouchDevice ? {
        capture: false,
        passive: false
      } : {
        capture: false
  ...
  dvt.Button.prototype._enableMouseEvents = function (container, enabled) {
    ...
    var child = container.firstChild; //loop over all childs
  ...
  dvt.Button.prototype.setTooltip = function (tooltip) {
    this._tooltip = tooltip; // Assume that if using dvt.Button's tooltip getter/setter that we should set wai-aria properties
  ...
  dvt.TransientButton.prototype.Init = function (context, upState, overState, downState, disabledState, callback, callbackObj) {
    ...
    var stage = context.getStage(); // Need to set 'useCapture' to true for focus/blur event listeners since dvt.EventManager calls 'stopPropagation'
  ...
  dvt.TransientButton.prototype.OnMouseDown = function (event) {
    dvt.TransientButton.superclass.OnMouseDown.call(this, event); //Event propagation is stopped because TransientButton should not initiate drag on mouse down event
  ...
  dvt.TransientButton.prototype.OnMouseUp = function (event) {
    dvt.TransientButton.superclass.OnMouseUp.call(this, event); //Event propagation is stopped because TransientButton should not initiate drag on mouse up event
  ...
  dvt.Agent = function () {}; //Note: AgentInfo is set when ojdvt-base module is loaded


  var AgentInfo;

  dvt.Agent.setAgentInfo = function (info) {
    AgentInfo = info;
  };

  dvt.Agent.getAgentInfo = function () {
    return AgentInfo;
  };

  Object.defineProperty(dvt.Agent, 'browser', {
    enumerable: true,
    get: function get() {
      return dvt.Agent.getAgentInfo().browser;
    }
  });
  Object.defineProperty(dvt.Agent, 'engine', {
    enumerable: true,
    get: function get() {
      return dvt.Agent.getAgentInfo().engine;
    }
  });
  Object.defineProperty(dvt.Agent, 'version', {
    enumerable: true,
    get: function get() {
      return dvt.Agent.getAgentInfo().browserVersion;
    }
  });
  Object.defineProperty(dvt.Agent, 'os', {
    enumerable: true,
    get: function get() {
      return dvt.Agent.getAgentInfo().os.toLowerCase();
    }
  });
  ...
  dvt.Agent.getElementPosition = function (element) {
    if (dvt.Agent.browser === 'ie' || dvt.Agent.browser === 'edge') return dvt.Agent._getElementPositionIE(element);else return dvt.Agent._getElementPositionDefault(element);
  };
  /**
  ...
  dvt.Agent.isRightToLeft = function (context) {
    ...
    if (context && context.getReadingDirection() != null) return context.getReadingDirection() == 'rtl';else return dvt.Agent._bRtl;
  };
  /**
  ...
  dvt.Agent._initialize = function () {
    ...
    if (!dvt.Agent._bInitialized) {
      ...
      dvt.Agent._bTouchDevice = (dvt.Agent.browser === 'chrome' || dvt.Agent.browser === 'safari') && (dvt.Agent.os === 'ios' || dvt.Agent.os === 'android');
      dvt.Agent._devicePixelRatio = window && window.devicePixelRatio != null ? window.devicePixelRatio : 1; // Don't initialize again
  ...
  dvt.Agent._getElementPositionDefault = function (element) {
    ...
    var boundingRect = element.getBoundingClientRect(); // top and bottom are not rounded off in Gecko1.9
    ...
    var body = element.ownerDocument.body; // clientLeft and clientTop would be 0 for Gecko1.9
  ...
  dvt.Agent._getElementPositionIE = function (element) {
    ...
    var elemLeft = boundingRect.left;
    var docElement = element.ownerDocument.documentElement;
    var scrollLeft = docElement.scrollLeft; // RTL adjustment for IE scrolled view
  ...
  dvt.Agent.getFocusColor = function () {
    ...
    if (!dvt.Agent._focusColor) {
      ...
      } else if (dvt.Agent.browser === 'ie' || dvt.Agent.browser === 'edge') focusColor = dvt.Agent.FOCUS_COLOR_IE;
  ...
  dvt.Automation.prototype.getSubIdForDomElement = function (domElement) {
    ...
    if (displayable && !(displayable instanceof Array)) return this.GetSubIdForDomElement(displayable);else if (domElement) {
      // tooltip automation
      if (this.IsTooltipElement(domElement)) return dvt.Automation.TOOLTIP_SUBID;
    }
    return null;
  };
  /**
  ...
  dvt.Automation._findDisplayable = function (domElement) {
    while (domElement) {
      // If this object has a displayable, return it
      if (domElement._obj) return domElement._obj;else // Otherwise look at the parent
  ...
  dvt.Automation.prototype.IsTooltipElement = function (domElement) {
    ...
    if (id && id.indexOf(dvt.HtmlTooltipManager._TOOLTIP_DIV_ID) == 0) return true;
    return false;
  };
  /**
   * @license
  ...
  dvt.SelectionHandler.prototype.Init = function (context, type) {
    this._context = context;
    this._selection = [];
    this._type = type ? type : dvt.SelectionHandler.TYPE_SINGLE; // Some selected id may not correspond to a drawn target because it's outside the viewport.
  ...
  dvt.SelectionHandler.prototype.processInitialSelections = function (selectedIds, targets) {
    ...
    this.clearSelection(true); // If nothing selected, we are done

    if (!selectedIds || !targets) return; // Construct a keySet so that the process to iterate though the targets and match the selectedIds takes
  ...
  dvt.SelectionHandler.prototype.processGroupSelection = function (targets, addToExisting) {
    ...
    for (var i = 0; i < targets.length; i++) {
      target = targets[i]; //: if this click is unrelated to selection, then
      //don't change selection at all
  ...
  dvt.SelectionHandler.prototype._removeFromSelection = function (target) {
    if (!target.isSelected()) return false; // First deselect the object, then remove it from the selected array
  ...
  dvt.KeyboardHandler.getNextNavigable = function (currentNavigable, event, navigableItems, ignoreBounds, targetCoordinateSpace) {
    ...
    if (!currentNavigable) {
      if (!navigableItems || navigableItems.length < 1) return null;else return navigableItems[0];
    ...
    for (var i = 0; i < navigableItems.length; i++) {
      ...
      if (ignoreBounds || dvt.KeyboardHandler._isInBounds(currentBounds, candidateBounds, direction)) {
        ...
        if ((direction == dvt.KeyboardEvent.UP_ARROW || direction == dvt.KeyboardEvent.LEFT_ARROW) && delta < 0 && (!nextNavigable || delta > nextNavigableDelta) || (direction == dvt.KeyboardEvent.DOWN_ARROW || direction == dvt.KeyboardEvent.RIGHT_ARROW) && delta > 0 && (!nextNavigable || delta < nextNavigableDelta)) {
  ...
  dvt.KeyboardHandler.prototype.getDefaultNavigable = function (navigableItems) {
    ...
    for (var i = 1; i < navigableItems.length; i++) {
      ...
      navigableLocation = navigable.getKeyboardBoundingBox(); // return the top left-most item in non-bidi and top right-most item in bidi

      if ((navigableLocation.x == defaultLocation.x && navigableLocation.y < defaultLocation.y || navigableLocation.x < defaultLocation.x) && !dvt.Agent.isRightToLeft(this._eventManager.getCtx()) || (navigableLocation.x + navigableLocation.w == defaultLocation.x + defaultLocation.w && navigableLocation.y < defaultLocation.y || navigableLocation.x + navigableLocation.w > defaultLocation.x + defaultLocation.w) && dvt.Agent.isRightToLeft(this._eventManager.getCtx())) {
  ...
  dvt.KeyboardHandler._computeDelta = function (currentBounds, candidateBounds, direction) {
    ...
    if (direction == dvt.KeyboardEvent.UP_ARROW || direction == dvt.KeyboardEvent.DOWN_ARROW) delta = candidateY - currentY;else if (direction == dvt.KeyboardEvent.LEFT_ARROW || direction == dvt.KeyboardEvent.RIGHT_ARROW) delta = candidateX - currentX;
    return delta;
  };
  /**
  ...
  dvt.KeyboardHandler.getNextAdjacentNavigable = function (current, event, listOfObjects) {
    ...
    var nextDistance = Number.MAX_VALUE; // If an object is in contact it overrules all other attributes
    // Only another in contact object with better attributes will have higher precedence

    var nextInContact = false; // Whether or not the for loop has encountered the current object
    ...
    for (var i = 0; i < listOfObjects.length; i++) {
      ...
      if (!dvt.KeyboardHandler._isValidDestination(object, current, keycode)) continue;

      var inContact = dvt.KeyboardHandler._calcInContact(object, current, keycode);

      if (nextInContact && !inContact) continue;

      var distance = dvt.KeyboardHandler._calcDistanceAngleWeighted(object, current, keycode); // : If there is another object with the exact same position as the current one, only navigate to that
      ...
      if (distance == 0 && !hasFoundCurrent) continue; // Make sure incontact flag have highest precedence

      if (!nextInContact && inContact || distance < nextDistance && (nextInContact && inContact || !nextInContact)) {
  ...
  dvt.KeyboardHandler._calcInContact = function (object, current, keycode) {
    ...
    switch (keycode) {
      case dvt.KeyboardEvent.UP_ARROW:
        return dvt.KeyboardHandler._isVerticallyAligned(objRect, curRect) && (curRect.y <= objRect.y + objRect.h || dvt.KeyboardHandler._areEqualWithinTolerance(curRect.y, objRect.y + objRect.h));

      case dvt.KeyboardEvent.DOWN_ARROW:
        return dvt.KeyboardHandler._isVerticallyAligned(objRect, curRect) && (objRect.y <= curRect.y + curRect.h || dvt.KeyboardHandler._areEqualWithinTolerance(objRect.y, curRect.y + curRect.h));

      case dvt.KeyboardEvent.RIGHT_ARROW:
        return dvt.KeyboardHandler._isHorizontallyAligned(objRect, curRect) && (objRect.x <= curRect.x + curRect.w || dvt.KeyboardHandler._areEqualWithinTolerance(objRect.x, curRect.x + curRect.w));

      case dvt.KeyboardEvent.LEFT_ARROW:
        return dvt.KeyboardHandler._isHorizontallyAligned(objRect, curRect) && (curRect.x <= objRect.x + objRect.w || dvt.KeyboardHandler._areEqualWithinTolerance(curRect.x, objRect.x + objRect.w));
  ...
  dvt.KeyboardHandler._isHorizontallyAligned = function (rect1, rect2) {
    return rect1.y >= rect2.y && rect1.y <= rect2.y + rect2.h || rect2.y >= rect1.y && rect2.y <= rect1.y + rect1.h;
  };
  /**
  ...
  dvt.KeyboardHandler._calcDistanceAngleWeighted = function (object, current, keycode) {
    ...
    var distance = Math.sqrt(x_dist * x_dist + y_dist * y_dist); // Angle penalty based on direction

    if (angle > optimalAngle1 && (keycode == dvt.KeyboardEvent.RIGHT_ARROW || keycode == dvt.KeyboardEvent.LEFT_ARROW) || angle < dvt.Math.HALF_PI - optimalAngle1 && (keycode == dvt.KeyboardEvent.UP_ARROW || keycode == dvt.KeyboardEvent.DOWN_ARROW)) {
      if (angle > optimalAngle2 && (keycode == dvt.KeyboardEvent.RIGHT_ARROW || keycode == dvt.KeyboardEvent.LEFT_ARROW) || angle < dvt.Math.HALF_PI - optimalAngle2 && (keycode == dvt.KeyboardEvent.UP_ARROW || keycode == dvt.KeyboardEvent.DOWN_ARROW)) {
  ...
  dvt.MarqueeHandler = function (container, marqueeBounds, glassPaneBounds, fill, stroke, allowHorizResize, allowVertResize, horizResizeBounds, vertResizeBounds) {
    this.Init(container, marqueeBounds, glassPaneBounds, fill, stroke, allowHorizResize, allowVertResize, horizResizeBounds, vertResizeBounds);
  };
  ...
  dvt.MarqueeHandler.prototype.Init = function (container, marqueeBounds, glassPaneBounds, fill, stroke, allowHorizResize, allowVertResize, horizResizeBounds, vertResizeBounds) {
  ...
  dvt.MarqueeHandler.prototype.processDragStart = function (relPos, ctrlKey) {
    if (!this._marqueeOn) {
      ...
      if (!this._bounds.containsPoint(this._origPt.x, this._origPt.y)) {
        if (this._vertBounds && this._vertBounds.containsPoint(this._origPt.x, this._origPt.y)) this._resizeHoriz = false;else if (this._horizBounds && this._horizBounds.containsPoint(this._origPt.x, this._origPt.y)) this._resizeVert = false;else return null;
      }
  ...
  dvt.MarqueeHandler.prototype.processDragMove = function (relPos, ctrlKey) {
    ...
    if (!this._marqueeDrawn) {
      this._container.addChild(this._glassPane); // Initiate the marquee


      this._marquee = new dvt.Rect(this._context, this._bounds.x, this._bounds.y, this._bounds.w, this._bounds.h); // marquee element should not receive pointer events
    ...
    var newPt = this._container.stageToLocal(relPos); // Bound the newPt within the marquee bounds
    ...
    newPt.y = Math.min(newPt.y, this._bounds.y + this._bounds.h); // Update marquee x and w

    if (this._resizeHoriz) {
      this._marquee.setWidth(Math.abs(newPt.x - this._origPt.x));

      if (newPt.x < this._origPt.x) this._marquee.setX(newPt.x);else this._marquee.setX(this._origPt.x);
    ...
    if (this._resizeVert) {
      ...
      if (newPt.y < this._origPt.y) this._marquee.setY(newPt.y);else this._marquee.setY(this._origPt.y);
    }
  ...
  dvt.MarqueeHandler.prototype._createMarqueeEvent = function (subtype, ctrlKey) {
    var x = null;
    var y = null;
    var w = null;
    var h = null;
    ...
    return new dvt.EventFactory.newMarqueeEvent(subtype, x, y, w, h, ctrlKey);
  };
  /**
  ...
  dvt.MarqueeHandler.prototype.getCursor = function (relPos) {
    ...
    if (withinBounds || withinVertBounds || withinHorizBounds) return 'crosshair';else return 'inherit';
  };
  /**
  ...
  dvt.PanZoomHandler.prototype.Init = function (container, panZoomBounds, glassPaneBounds, zoomRate) {
    ...
    this._zoomRate = zoomRate; // Set up the glass pane
  ...
  dvt.PanZoomHandler.prototype.processDragStart = function (relPos) {
    if (!this._panOn) {
      ...
      this._lastPt = this._origPt; // Ignore if the start point is outside the pan/zoom bounds

      if (!this._bounds.containsPoint(this._origPt.x, this._origPt.y)) return null;
      this._panOn = true;
      return dvt.EventFactory.newPanZoomEvent('panStart', 0, 0, 0, 0, 0, 0, 0, 0);
    }
  ...
  dvt.PanZoomHandler.prototype.processDragMove = function (relPos) {
    ...
    this._lastPt = newPt;
    return dvt.EventFactory.newPanZoomEvent('panMove', -deltaX, -deltaX, -deltaY, -deltaY, -totalDeltaX, -totalDeltaX, -totalDeltaY, -totalDeltaY);
  };
  /**
   * Processes drag end.
   * @param {dvt.Point} relPos The event position relative to the stage.
  ...
  dvt.PanZoomHandler.prototype.processDragEnd = function (relPos) {
    if (this._panOn) {
      ...
      if (this._glassPaneDrawn) {
        ...
        var totalDeltaY = newPt.y - this._origPt.y;
        this._lastPt = null;
        return dvt.EventFactory.newPanZoomEvent('panEnd', -deltaX, -deltaX, -deltaY, -deltaY, -totalDeltaX, -totalDeltaX, -totalDeltaY, -totalDeltaY);
      }
    }
  ...
  dvt.PanZoomHandler.prototype.processMouseWheel = function (relPos, delta) {
    ...
    delta *= this._zoomRate; // Compute the deltas. It should maintain the cursor pointing at the same item.
    ...
    return dvt.EventFactory.newPanZoomEvent('zoom', deltaXMin, deltaXMax, deltaYMin, deltaYMax, deltaXMin, deltaXMax, deltaYMin, deltaYMax);
  };
  /**
  ...
  dvt.PanZoomHandler.prototype.processPinchStart = function (relPos1, relPos2) {
    if (!this._pinchOn) {
      ...
      this._lastPt2 = this._origPt2; // Ignore if the start points are outside the pan/zoom bounds

      if (!this._bounds.containsPoint(this._origPt1.x, this._origPt1.y) || !this._bounds.containsPoint(this._origPt2.x, this._origPt2.y)) return null;
      this._pinchOn = true;
      return dvt.EventFactory.newPanZoomEvent('pinchStart', 0, 0, 0, 0, 0, 0, 0, 0);
    }
  ...
  dvt.PanZoomHandler.prototype.processPinchMove = function (relPos1, relPos2) {
    ...
    return dvt.EventFactory.newPanZoomEvent('pinchMove', deltas.dxMin, deltas.dxMax, deltas.dyMin, deltas.dyMax, totalDeltas.dxMin, totalDeltas.dxMax, totalDeltas.dyMin, totalDeltas.dyMax);
  };
  /**
   * Processes pinch end.
   * @return {object}
  ...
  dvt.PanZoomHandler.prototype.processPinchEnd = function () {
    if (this._pinchOn) {
      ...
      this._lastPt1 = null;
      this._lastPt2 = null;
      return dvt.EventFactory.newPanZoomEvent('pinchEnd', 0, 0, 0, 0, totalDeltas.dxMin, totalDeltas.dxMax, totalDeltas.dyMin, totalDeltas.dyMax);
    }
  ...
  dvt.PanZoomHandler.prototype._computePinchDeltas = function (pos1, pos2, prevPos1, prevPos2) {
    ...
    var prevDy = Math.abs(prevPos1.y - prevPos2.y); // The check dx > dy/2 is to make it easier to zoom in y-direction alone. Otherwise, one might pinch in y-direction,
    // but the slight finger movement in the x-direction will cause the x-axis to zoom as well.

    var zoomX = dx > dy / 2 ? prevDx / dx : 1;
    var zoomY = dy > dx / 2 ? prevDy / dy : 1; // Calculate the distance the center of the two fingers has moved
  ...
  dvt.PanZoomHandler.prototype.panBy = function (dx, dy) {
    ...
    return dvt.EventFactory.newPanZoomEvent('panEnd', deltaX, deltaX, deltaY, deltaY, deltaX, deltaX, deltaY, deltaY);
  };
  /**
   * Returns the appropriate cursor type.
   * @param {dvt.Point=} relPos The current cursor position relative to the stage.
  ...
  dvt.CategoryRolloverHandler = function (callback, callbackObj) {
    this._callback = callback;
    this._callbackObj = callbackObj; // Assume the source of the event is also the callbackObj. We can add a param if needed.
  ...
  dvt.CategoryRolloverHandler.prototype.processEvent = function (event, objs, initialHoverDelay, bAnyMatched, customAlpha) {
    if (!event || !objs || objs.length == 0) return; // Cancel any pending highlight operations. We've either moved to a new object or left the current one.
  ...
  dvt.CategoryRolloverHandler.prototype.GetRolloverCallback = function (event, objs, bAnyMatched, customAlpha) {
    var callback = function callback() {
      ...
      dvt.CategoryRolloverHandler.highlight(event['categories'], objs, bAnyMatched, customAlpha); // Fire the event to the component's callback if specified.
  ...
  dvt.CategoryRolloverHandler.prototype.GetRolloutCallback = function (event, objs, bAnyMatched, customAlpha) {
    var callback = function callback() {
      ...
      this.SetHighlightModeTimeout(); // Fire the event to the component's callback if specified.
  ...
  dvt.CategoryRolloverHandler.highlight = function (categories, objs, bAnyMatched, customAlpha) {
    if (!objs) return;
    var dimmedAlpha = !customAlpha ? dvt.CategoryRolloverHandler._FADE_OUT_OPACITY : customAlpha; // Loop through the objects and update objects not belonging to the specified category.
  ...
  dvt.CategoryRolloverHandler._highlightObj = function (obj, bDimmed, dimmedAlpha) {
    var alpha = bDimmed ? dimmedAlpha : 1; // Delegate to the object to perform highlighting if supported. Otherwise adjust the alpha of all displayables.

    if (obj.highlight) obj.highlight(bDimmed, alpha);else {
  ...
  dvt.CategoryRolloverHandler.prototype.SetHighlightModeTimeout = function () {
    ...
    var highlightTimeout = dvt.Obj.createCallback(this, function () {
   ...
   * @param {function} callback A function that responds to component events.
   * @param {object} callbackObj The optional object instance that the callback function is defined on.
   * @param {dvt.BaseComponent} component The dvt.BaseComponent that this EventManager is associated with; omit if the EventManager is being used for a non-BaseComponent object.
   * @class
  ...
  dvt.EventManager = function (context, callback, callbackObj, component) {
  ...
  dvt.EventManager.prototype.Init = function (context, callback, callbackObj, component) {
    this._context = context;
    this._callback = callback;
    this._callbackObj = callbackObj;
    this._component = component; // Used to keep track of the previous and current touch targets

    this._touchMap = {}; // Initialize the higher level event handlers.  These handlers are exposed as protected fields
    // so that subclasses can fully customize the behavior of this event manager.

    this._selectionHandler = null;
    this.KeyboardHandler = null; // Rollover Handler: Used to support the hover delays.

    this.RolloverHandler = this.CreateCategoryRolloverHandler(callback, callbackObj); // Tooltips enabled by default
    ...
    this.CustomTooltipManager.addTooltipEventListener('actionTooltipClosed', this.OnActionTooltipClosed, this);
    this.CustomTooltipManager.addTooltipEventListener('actionTooltipStarted', this.OnActionTooltipStarted, this);
    this.TouchManager = null;
    if (dvt.Agent.isTouchDevice()) this.TouchManager = new dvt.TouchManager('touchmanager', this._context, this); // The DvtKeyboardNavigable item that currently has keyboard focus

    this._focusedObj = null; // flag to indicate if the component should display keyboard focus

    this._shouldDisplayKeyboardFocus = false; // An array of event managers that can process keyboard events sent to this
    ...
    this._keyboardHandlers = [this]; // index to the _keyboardHandlers array indicating the current handler
    // that receives keyboard events

    this._currentKeyboardHandlerIdx = -1; // Map of DnD listeners
  ...
  dvt.EventManager.prototype.associate = function (displayable, obj, bReplace) {
    if (displayable) {
      ...
      if (!displayable._logicalObjects || bReplace) displayable._logicalObjects = []; // Add this logical object and event manager mapping

      displayable._logicalObjects.push({
        logicalObject: obj,
        eventManager: this
      });
    }
  ...
  dvt.EventManager.prototype.addListeners = function (displayable) {
    ...
    if (dvt.Agent.isTouchDevice()) {
      ...
      this.TouchManager.addTouchEventListener(dvt.ComponentTouchEvent.TOUCH_HOVER_START_TYPE, this.OnComponentTouchHoverStart, this);
      this.TouchManager.addTouchEventListener(dvt.ComponentTouchEvent.TOUCH_HOVER_MOVE_TYPE, this.OnComponentTouchHoverMove, this);
      this.TouchManager.addTouchEventListener(dvt.ComponentTouchEvent.TOUCH_HOVER_END_TYPE, this.OnComponentTouchHoverEnd, this);
      this.TouchManager.addTouchEventListener(dvt.ComponentTouchEvent.TOUCH_HOVER_OUT_TYPE, this.OnComponentTouchHoverOut, this);
      this.TouchManager.addTouchEventListener(dvt.ComponentTouchEvent.TOUCH_HOVER_OVER_TYPE, this.OnComponentTouchHoverOver, this);
      this.TouchManager.addTouchEventListener(dvt.ComponentTouchEvent.TOUCH_CLICK_TYPE, this.OnComponentTouchClick, this);
      this.TouchManager.addTouchEventListener(dvt.ComponentTouchEvent.TOUCH_DOUBLE_CLICK_TYPE, this.OnComponentTouchDblClick, this);
    } else {
      displayable.addEvtListener(dvt.MouseEvent.CLICK, this._onClick, false, this);
      displayable.addEvtListener(dvt.MouseEvent.DBLCLICK, this._onDblClick, false, this);
      displayable.addEvtListener('contextmenu', this.OnContextMenu, false, this);
      displayable.addEvtListener(dvt.MouseEvent.MOUSEMOVE, this.OnMouseMove, false, this);
      ...
      displayable.addEvtListener(dvt.MouseEvent.MOUSEUP, this.OnMouseUp, false, this);
    ...
    if (this.isDndSupported()) {
      var context = this.getCtx();
      this.setDragSource(new dvt.DragSource(context)); // Add drag source listeners

      this._addDndListener('dragstart', this.OnDndDragStart);

      this._addDndListener('drag', this.OnDndDrag);

      this._addDndListener('dragend', this.OnDndDragEnd); // Add drop target listeners
  ...
  dvt.EventManager.prototype.removeListeners = function (displayable, keepTooltip) {
    if (!displayable) return; // Hide any tooltips previously shown
  ...
  dvt.EventManager.prototype.RemoveListeners = function (displayable) {
    if (dvt.Agent.isTouchDevice()) {
      ...
      this.TouchManager.removeTouchEventListener(dvt.ComponentTouchEvent.TOUCH_HOVER_START_TYPE, this.OnComponentTouchHoverStart, this);
      this.TouchManager.removeTouchEventListener(dvt.ComponentTouchEvent.TOUCH_HOVER_MOVE_TYPE, this.OnComponentTouchHoverMove, this);
      this.TouchManager.removeTouchEventListener(dvt.ComponentTouchEvent.TOUCH_HOVER_END_TYPE, this.OnComponentTouchHoverEnd, this);
      this.TouchManager.removeTouchEventListener(dvt.ComponentTouchEvent.TOUCH_HOVER_OUT_TYPE, this.OnComponentTouchHoverOut, this);
      this.TouchManager.removeTouchEventListener(dvt.ComponentTouchEvent.TOUCH_HOVER_OVER_TYPE, this.OnComponentTouchHoverOver, this);
      this.TouchManager.removeTouchEventListener(dvt.ComponentTouchEvent.TOUCH_CLICK_TYPE, this.OnComponentTouchClick, this);
      this.TouchManager.removeTouchEventListener(dvt.ComponentTouchEvent.TOUCH_DOUBLE_CLICK_TYPE, this.OnComponentTouchDblClick, this);
    } else {
      displayable.removeEvtListener(dvt.MouseEvent.CLICK, this._onClick, false, this);
      displayable.removeEvtListener(dvt.MouseEvent.DBLCLICK, this._onDblClick, false, this);
      displayable.removeEvtListener('contextmenu', this.OnContextMenu, false, this);
      displayable.removeEvtListener(dvt.MouseEvent.MOUSEMOVE, this.OnMouseMove, false, this);
      ...
      displayable.removeEvtListener(dvt.MouseEvent.MOUSEUP, this.OnMouseUp, false, this);
  ...
  dvt.EventManager.prototype.destroy = function () {
    if (this.CustomTooltipManager) {
      this.CustomTooltipManager.removeTooltipEventListener('actionTooltipClosed', this.OnActionTooltipClosed, this);
      this.CustomTooltipManager.removeTooltipEventListener('actionTooltipStarted', this.OnActionTooltipStarted, this);
    }
  };
  /**
  ...
  dvt.EventManager.prototype.setDragSource = function (dragSource) {
    ...
    if (dvt.Agent.isTouchDevice()) this.DragSource.setTouchManager(this.TouchManager);
  };
  /**
  ...
  dvt.EventManager.prototype.setKeyboardHandler = function (handler) {
    if (!dvt.Agent.isTouchDevice()) this.KeyboardHandler = handler;
  };
  /**
  ...
  dvt.EventManager.prototype.setFocusObj = function (navigable) {
    ...
    if (navigable != curFocus) {
      ...
      if (this._currentKeyboardHandlerIdx > -1 && this._currentKeyboardHandlerIdx < this._keyboardHandlers.length) {
  ...
  dvt.EventManager.prototype.setFocused = function (isFocused) {
    var navigable = this.getFocus(); // don't show keyboard focus effect on touch devices

    if (navigable && this.KeyboardHandler) {
      if (!isFocused) {
        navigable.hideKeyboardFocusEffect();
      } else if (this._shouldDisplayKeyboardFocus && this._currentKeyboardHandlerIdx >= 0 && this._currentKeyboardHandlerIdx < this._keyboardHandlers.length && this._keyboardHandlers[this._currentKeyboardHandlerIdx] === this) {
  ...
  dvt.EventManager.prototype._getTooltipColor = function (obj, x, y) {
  ...
  dvt.EventManager.prototype.fireSelectionEvent = function (logicalObj) {
    ...
    if (!selectionHandler) return; // Get the array of selected ids

    var selectedIds = selectionHandler.getSelectedIds(); // Create and fire the event
  ...
  dvt.EventManager.prototype._onClick = function (event) {
    if (this.IsDoubleClickable(event)) {
      if (this._clickTimer && this._clickTimer.isRunning()) {
        ...
        if (event.pageX == clickEvent.pageX && event.pageY == clickEvent.pageY) {
          this._savedClickCount++; // Same coords, this is a double click, so ignore second click event
  ...
  dvt.EventManager.prototype.OnClick = function (event) {
    ...
    this.ProcessSelectionEventHelper(obj, event.ctrlKey); // Stop displaying keyboard focus after the user has clicked

    this._shouldDisplayKeyboardFocus = false; // Done if there is no object
  ...
  dvt.EventManager.prototype.IsDoubleClickable = function (event) {
    var obj = this.GetLogicalObject(event.target);
    if (obj && obj.isDoubleClickable && obj.isDoubleClickable()) return true;else return false;
  };
  /**
  ...
  dvt.EventManager.prototype.ProcessSelectionEventHelper = function (logicalObj, isMultiSelect) {
    ...
    if (selectionHandler) {
      var bSelectionChanged = selectionHandler.processClick(logicalObj, isMultiSelect); // If the selection has changed, fire an event

      if (bSelectionChanged) this.fireSelectionEvent(logicalObj);
    }
  };

  dvt.EventManager.prototype._onDblClick = function (event) {
  ...
  dvt.EventManager.prototype.OnContextMenu = function (event) {
    var obj = this.GetLogicalObject(event.target); // set keyboard focus on the object we are firing the context menu on
  ...
  dvt.EventManager.prototype._onContextMenuHelper = function (event, obj) {
    var popupLaunched = this._processActionPopup(event.target, new dvt.Point(event.pageX, event.pageY));

    if (popupLaunched) event.preventDefault(); // Process selection only for selectable objects so that generating context menu on empty area doesn't deselect ()

    if (obj && obj.isSelectable && obj.isSelectable()) this.ProcessSelectionEventHelper(obj, event.ctrlKey);
  };
  /**
  ...
  dvt.EventManager.prototype.OnKeyDown = function (event) {
    ...
    var i;
    var increment; // if no current event manager is set to receive keyboard focus,
    ...
    if (this._currentKeyboardHandlerIdx < 0 || this._currentKeyboardHandlerIdx > this._keyboardHandlers.length) {
      // if we get a shift+tab, start from the back of the array
      if (keyCode == dvt.KeyboardEvent.TAB && event.shiftKey) i = this._keyboardHandlers.length - 1;else i = 0;
    } else {
      i = this._currentKeyboardHandlerIdx;
    } // if we get a shift+tab, start from the back of the array and move backward


    if (keyCode == dvt.KeyboardEvent.TAB && event.shiftKey) increment = -1;else increment = 1;

    for (i; i >= 0 && i < this._keyboardHandlers.length && !eventConsumed; i = i + increment) {
      var handler = this._keyboardHandlers[i];
      if (handler === this) eventConsumed = this.ProcessKeyboardEvent(event);else eventConsumed = handler.handleKeyboardEvent(event);
    ...
    if (!eventConsumed && (event.keyCode == dvt.KeyboardEvent.UP_ARROW || event.keyCode == dvt.KeyboardEvent.DOWN_ARROW || event.keyCode == dvt.KeyboardEvent.LEFT_ARROW || event.keyCode == dvt.KeyboardEvent.RIGHT_ARROW)) {
      dvt.EventManager.consumeEvent(event);
  ...
  dvt.EventManager.prototype.ProcessKeyboardEvent = function (event) {
    if (!this.KeyboardHandler) return false; // clear tooltip if one is shown; tooltip can be shown if mouse is used to make a selection
    // and then we use the keyboard to navigate from the mouse-selected item

    this.hideTooltip();
    var currentNavigable = this.getFocus();
    var nextNavigable = null; // When tabbing into/out of a DVT component or subcomponent, we need to ensure focus effect is consistently shown/hidden
    ...
    if (event.keyCode == dvt.KeyboardEvent.TAB && currentNavigable) {
      if (currentNavigable.isShowingKeyboardFocusEffect()) {
        ...
        currentNavigable.hideKeyboardFocusEffect();
        this.ProcessRolloverEvent(event, currentNavigable, false); // try to navigate to the next/previous subcomponent

        var nextComponent;
        if (event.shiftKey) nextComponent = this._context.previousKeyboardFocus();else nextComponent = this._context.nextKeyboardFocus();
    ...
    if (nextNavigable) {
      this.setFocus(nextNavigable); // the user has transferred focus via the keyboard, start showing keyboard focus effect
      ...
      if (this.KeyboardHandler.isSelectionEvent(event)) this.ProcessSelectionEventHelper(nextNavigable, event.shiftKey);else if (this.KeyboardHandler.isMultiSelectEvent(event)) this.ProcessSelectionEventHelper(nextNavigable, event.ctrlKey);
  ...
  dvt.EventManager.prototype.OnKeyUp = function (event) {
    ...
    if (this._handleExternalKeyboardFocus === true && event.keyCode == dvt.KeyboardEvent.TAB) {
      this._handleExternalKeyboardFocus = false; //  - firing a fake keydown event on keyup to communicate the tab to the newly focused element
      ...
      if (document.createEvent) {
        ...
        if (typeof keyboardEvent.initKeyboardEvent != 'undefined') {
          keyboardEvent.initKeyboardEvent(dvt.KeyboardEvent.KEYDOWN, true, true, window, ctrlKeyPressed, false, shiftKeyPressed, false, keycode, keycode);
        } else {
          keyboardEvent.initKeyEvent(dvt.KeyboardEvent.KEYDOWN, true, true, window, ctrlKeyPressed, false, false, shiftKeyPressed, keycode, keycode);
        }
        /*
  ...
  dvt.EventManager.prototype.OnMouseMove = function (event) {
    ...
    var displayable = objAndDisp.displayable; // Return if no object is found
  ...
  dvt.EventManager.prototype.OnMouseOver = function (event) {
    ...
    if (!obj) return; // Selection Support

    var selectionHandler = this.getSelectionHandler(obj);
    if (selectionHandler) selectionHandler.processMouseOver(obj); // Category Rollover Support

    this.ProcessRolloverEvent(event, obj, true); // Accessibility Support
  ...
  dvt.EventManager.prototype.OnMouseOut = function (event) {
    var obj = this.GetLogicalObject(event.target); // Return if no object is found
    ...
    var selectionHandler = this.getSelectionHandler(obj);
    if (selectionHandler) selectionHandler.processMouseOut(obj); // Category Rollover Support

    this.ProcessRolloverEvent(event, obj, false); // : If the mouse has moved off the component, hide the tooltip
  ...
  dvt.EventManager.prototype.OnMouseUp = function (event) {// no default behavior, subclasses can override
  };
  /**
   * @private
  ...
  dvt.EventManager.prototype._processTouchTooltip = function (event) {
    ...
    var tooltipTarget = tooltipInfoObj['tooltipTarget']; // If no hints and another event manager didn't already process the event for a tooltip, hide current tooltip
  ...
  dvt.EventManager.prototype.OnTouchStartBubble = function (event) {
    ...
    var isPopup = this.isInActionPopup(); // Action popups block other types of interactions
    ...
    this.TouchManager.postEventBubble(event); // Handle touch actions
  ...
  dvt.EventManager.prototype.OnComponentTouchHoverStart = function (event) {
    this.HandleTouchHoverStartInternal(event); // Handle touch actions if not already handled on touch start
  ...
  dvt.EventManager.prototype.OnComponentTouchHoverEnd = function (event) {
    this.HandleTouchHoverEndInternal(event); // Fire an event notifying callback object that touch hold release happened. Used by JET for context menus.
    ...
    if (this._callbackObj instanceof dvt.BaseComponent) {
      ...
      if (x <= stageCoord.x + this._callbackObj.getWidth() && x >= stageCoord.x && y <= stageCoord.y + this._callbackObj.getHeight() && y >= stageCoord.y) this._callback.call(this._callbackObj, dvt.EventFactory.newTouchHoldReleaseEvent(event.getNativeEvent()));
    }
  ...
  dvt.EventManager.prototype.OnComponentTouchClick = function (event) {
    ...
    if (done) return; // When touchResponse is touchStart, selection is handled on touchend listener.

    if (!this.isTouchResponseTouchStart()) this._processTouchSelection(dlo); // Prevent selection clearing if popup has just closed

    if (!this._popupJustClosed) this._processTouchSelectionClear(dlo);

    if (this.CustomTooltipManager.displayActionPopup()) {
      this.UpdateActionTooltipLaunchedFeedback(displayable, new dvt.Point(touch.pageX, touch.pageY));
  ...
  dvt.EventManager.prototype._processTouchSelection = function (dlo) {
    ...
    if (selectionHandler && dlo && dlo.isSelectable && dlo.isSelectable()) {
      ...
      bSelectionChanged = selectionHandler.processClick(dlo, true); // If the selection has changed, fire an event
  ...
  dvt.EventManager.prototype._processTouchSelectionClear = function (dlo) {
    ...
    if (selectionHandler && (!dlo || !dlo.isSelectable || !dlo.isSelectable())) {
      if (!this._prevActionClear) {
        ...
        var showClearDialog = false; // There are two different policies for the clear selection dialog
        ...
        if (showClearDialog) {
          ...
          bSelectionChanged = selectionHandler.processClick(null, false); // If the selection has changed, fire an event
  ...
  dvt.EventManager.prototype.OnActionTooltipClosed = function (event) {
    ...
    this.CustomTooltipManager.clearActionTooltip(); // end start
  ...
  dvt.EventManager.prototype.SetupTouchTooltip = function (targetObj) {
    ...
    var tooltipBorderColor = actionTooltip.getTooltipBorderColor(); // If border color wasn't overridden, get it here
  ...
  dvt.EventManager.prototype.HandleTouchDblClickInternal = function (event) {}; // Action popup shown for the given target object


  dvt.EventManager.prototype.UpdateActionPopupShownFeedback = function (targetObj) {}; // Action popup hidden for the given target object
  ...
  dvt.EventManager.prototype.addClearAllActionItem = function () {
    ...
    this.addActionTooltipMenuItem(dvt.EventManager.CLEAR_SELECTION_ACTION_TYPE, this.GetClearSelectionText(), this._actionTooltipClearListener, this); // Override color to always be gray
  ...
  dvt.EventManager.prototype._actionTooltipClearListener = function (evt) {
    ...
    var bSelectionChanged = this.getSelectionHandler(dlo).processClick(null, false); // If the selection has changed, fire an event
  ...
  dvt.EventManager.consumeEvent = function (event) {
    if (event) {
      event.preventDefault(); //: stop event propagation when the event is consumed
  ...
  dvt.EventManager.prototype.addComponentKeyboardHandlerAt = function (handler, index) {
    ...
    if (handler) {
      this._keyboardHandlers.splice(index, 0, handler); // maintain the pointer to the current keyboard handler
  ...
  dvt.EventManager.prototype.removeComponentKeyboardHandler = function (handler) {
    if (handler) {
      ...
      if (i > -1) {
        ...
        if (this._currentKeyboardHandlerIdx == i && i == 0) this._currentKeyboardHandlerIdx = 0; // maintain the pointer to the current keyboard handler
  ...
  dvt.EventManager.prototype.UpdateActiveElement = function (obj, displayable) {
    ...
    if (!displayable) {
      if (obj.getDisplayable) displayable = obj.getDisplayable();else if (obj.getDisplayables) displayable = obj.getDisplayables()[0];else displayable = obj;
    } // Don't continue if the displayable is not a valid displayable


    if (!(displayable instanceof dvt.Displayable)) return; // Find the displayable with the ARIA tags, by traversing ancestors if needed.  If the logical object has a deferred
    ...
    if (displayable == this.getCtx().getStage()) return; // Set the new active element
  ...
  dvt.EventManager.prototype.GetEventInfo = function (event, key) {
    if (!this._hasEventInfo(event) || key == null) return null; //get the value from event info
  ...
  dvt.EventManager.prototype.SetEventInfo = function (event, key, value) {
    ...
    if (!this.StoreInfoByEventType(key)) {
      ...
      if (!nativeEvt._eventInfo[typeName]) nativeEvt._eventInfo[typeName] = {}; //set the value in event info
  ...
  dvt.EventManager.prototype.ProcessObjectTooltip = function (event, relPosX, relPosY, obj, targetObj) {
    if (!this.GetEventInfo(event, dvt.EventManager._EVENT_INFO_TOOLTIP_DISPLAYED_KEY)) {
      if (this._processShowTooltip(targetObj, obj, relPosX, relPosY)) this.SetEventInfo(event, dvt.EventManager._EVENT_INFO_TOOLTIP_DISPLAYED_KEY, true);
    }
  };
  /**
  ...
  dvt.EventManager.prototype.ProcessRolloverEvent = function (event, obj, bOver) {// subclasses must override if hoverBehavior is supported
  };
  /**
  ...
  dvt.EventManager.prototype.isTouchResponseTouchStart = function () {
    ...
    if (touchResponse === dvt.EventManager.TOUCH_RESPONSE_TOUCH_START) return true;else if (touchResponse === dvt.EventManager.TOUCH_RESPONSE_TOUCH_HOLD) return false;else {
      ...
      while (root && root.nodeName !== 'HTML') {
        ...
        if (style.overflow !== 'hidden' && (root.scrollWidth > root.clientWidth + dvt.EventManager._TOUCH_RESPONSE_PADDING_CHECK && style['overflow-x'] !== 'hidden' || root.scrollHeight > root.clientHeight + dvt.EventManager._TOUCH_RESPONSE_PADDING_CHECK && style['overflow-y'] !== 'hidden') || window.frameElement && window.frameElement.nodeName == 'IFRAME') {
  ...
  dvt.EventManager.prototype.HandleTouchActionsStart = function (event) {
    var obj = this.GetLogicalObject(event.target);
    var touch = event instanceof dvt.ComponentTouchEvent ? event.touch : event.touches[0]; // Logic copied from dvt.TouchManager._onTouchHoldHover
    ...
    if (obj) this.ProcessRolloverEvent(event, obj, true); // Tooltips
  ...
  */
  ...
  dvt.EventManager.prototype.HandleTouchActionsMove = function (event) {
    ...
    if (!(event instanceof dvt.ComponentTouchEvent)) {
      ...
      event.preventDefault(); // The target for a touch event is always the touchstart target. dvt.TouchManager already handles updating the
      // target based on the current x/y coordinates, but we need to handle it as well if we are dealing with the browser
      // touch events.

      var targetObj = dvt.SvgDocumentUtils.elementFromTouch(touch);
      ...
      if (prevObj != obj) {
        ...
        if (prevObj) {
          var target;
          if (prevObj.getDisplayable) target = prevObj.getDisplayable();else if (prevObj.getDisplayables) target = prevObj.getDisplayables()[0];
  ...
  dvt.EventManager.prototype.HandleTouchActionsEnd = function (event, touch) {
    var targetObj;
    var obj; // Logic copied from dvt.TouchManager._onTouchHoldHover
    ...
    if (!(event instanceof dvt.ComponentTouchEvent)) {
      ...
      if (obj) {
        if (obj.getDisplayable) target = obj.getDisplayable();else if (obj.getDisplayables) target = obj.getDisplayables()[0];
      }
  ...
  */
  ...
  dvt.EventManager.prototype.HandleTouchActionsOver = function (event) {
    var obj = this.GetLogicalObject(event.target); // Category Rollover Support
  ...
  */
  ...
  dvt.EventManager.prototype.HandleTouchActionsOut = function (event, touch) {
    var obj = this.GetLogicalObject(event.target); // dvt.ComponentTouchEvent should use its stored touch, else at this point touch event has been passed in and removed from event

    if (event instanceof dvt.ComponentTouchEvent) touch = event.touch; // Category Rollover Support
  ...
  dvt.EventManager.prototype.getComponent = function () {
  ...
  dvt.EventManager.prototype.OnDndDragStart = function (event) {
    ...
    var dataTransfer = $event.dataTransfer; // Pass if the dataTransfer is already loaded, since the event is already handled by another event manager
    ...
    var dragSourceType = this.GetDragSourceType(event); // Cancel the event if drag can't be started
    ...
    this._context.getTooltipManager().hideTooltip(); // Provide data to the dataTransfer based on dataTypes


    var dragOptions = this._getDragOptions(event);

    var dataTypes = dragOptions['dataTypes'];
    var callback = dragOptions['dragStart']; // Cancel the both dataTypes and callback are undefined
    ...
    dragImage.style.fontSize = this._context.getDefaultFontSize(); // Disable mouse events on the drag image so it doesn't interfere with the drag events.

    dvt.SvgDocumentUtils.disableMouseEvents(dragImage); // The drag image has to be in the DOM tree when being set, so we add it and remove it immediately.
    ...
    dataTransfer.setDragImage(dragImage, dragOffset.x, dragOffset.y); // Store the drop offsets in the dataTransfer so we can position the item correctly if the drop happens on a DVT component
  ...
  dvt.EventManager.prototype._handleDropTargetEvent = function (event, eventType, showEffect) {
    ...
    if (!dropRejected && showEffect) this.ShowDropEffect(event);else if (dropRejected && showEffect) this.ShowRejectedDropEffect(event);else this.ClearDropEffect();
  };
  /**
  ...
  dvt.EventManager.prototype._handleDropTargetEventHelper = function (event, eventType) {
    ...
    var dropOptions = this._getDropOptions(event); // Handle user-defined callback function
    ...
    if (dataTypes != null) {
      ...
      if (!Array.isArray(dataTypes)) dataTypes = [dataTypes]; // If there's at least one matching dataType, accept the drop

      for (var i = 0; i < dataTypes.length; i++) {
        if ($event.dataTransfer.types && $event.dataTransfer.types.indexOf(dataTypes[i]) >= 0) return false;
      }
    }
  ...
  dvt.EventManager.prototype.DispatchElementEvent = function (eventType, eventDetail) {
    this.getCtx().getSvgDocument().dispatchEvent(new CustomEvent(eventType, {
      'detail': eventDetail
    }));
  };
  /**
  ...
  var DvtCustomTooltipManager = function DvtCustomTooltipManager(context, id) {
  ...
  DvtCustomTooltipManager.prototype.GetRichTooltipManager = function () {
    if (!this._RichTooltipManager) {
      var tooltipId = this._id + 'Tooltip'; // TODO: may want to change tooltip managers to have their own impl

      this._RichTooltipManager = new DvtHtmlRichTooltipManager(this._context, tooltipId);

      this._RichTooltipManager.InitializeTooltipElem(); // Attach listeners to action tooltip displayable
  ...
  DvtCustomTooltipManager.prototype.closeActionTooltip = function () {
    var closedEvent = dvt.EventFactory.newActionTooltipEvent('actionTooltipClosed', this._actionPopupObj);
  ...
  DvtCustomTooltipManager.prototype.displayActionPopup = function () {
    var actionTooltip = this.getActionTooltip(); // If no action tooltip initialized with actions (not just tooltips), don't show one
  ...
  DvtCustomTooltipManager.prototype.startActionPopupAtPosition = function (pageX, pageY, targetObj, alignment) {
    ...
    this.GetRichTooltipManager().showRichElementAtPosition(pageX, pageY, actionTooltip, true, false); // Clear the text

    var startEvent = dvt.EventFactory.newActionTooltipEvent('actionTooltipStarted', targetObj);
  ...
  var DvtCustomTooltipEventHandler = function DvtCustomTooltipEventHandler(context, customTooltipManager, callback, callbackObj) {
  ...
  var DvtCustomTooltip = function DvtCustomTooltip(context, id) {
  ...
  DvtCustomTooltip.prototype.Render = function () {
    var parent = this.getRootDisplayable(); //parent.setPixelHinting(true);
    ...
    var tooltipBorderWidth = 2; // Temp variables
    ...
    parent.setTranslate(tooltipBorderWidth / 2, tooltipBorderWidth / 2); // Add background panels
    ...
    parent.addChildAt(backgroundPanel, 0); // Add text items
  ...
  var DvtCustomTooltipItem = function DvtCustomTooltipItem(context, id, text, listener, listenerObj) {
  ...
  DvtCustomTooltipItem.prototype.FireActionTooltipItem = function (target) {
    if (this._listenerObj && this._listener) {
      ...
      this._menuItemTimer = new dvt.Timer(this._context, 1, this._handleMenuTimer, this); // store event on timer temporarily
  ...
  dvt.Obj.createSubclass(dvt.HtmlTooltipManager, dvt.Obj);
  /** @private @const */
  ...
  dvt.HtmlTooltipManager.prototype.Init = function (context, domElementId) {
    this._context = context; // if a dom element is specified, it will override the default tooltip div
  ...
  dvt.HtmlTooltipManager.prototype._showTextAtPosition = function (x, y, text, borderColor, useOffset, popupClass) {
    ...
    if (typeof text == 'string') {
      ...
      text = text.replace(/(<|&#60;)/g, '&lt;');
      text = text.replace(/(>|&#62;)/g, '&gt;'); // Support a subset of HTML tags, including bold, italic, and table tags.
      ...
      text = dvt.HtmlTooltipManager._restoreTag(text, 'td'); // Replace logical newlines sequences

      text = text.replace(/\n/g, '<br>');
      text = text.replace(/\\n/g, '<br>');
      text = text.replace(/&#92;n/g, '<br>');
      text = dvt.HtmlTooltipManager._restoreTag(text, 'br'); // Create the tooltip element

      tooltipDOM = document.createElement('span');
      tooltipDOM.style.visibility = 'inherit';
      tooltipDOM.style.width = null;
      tooltipDOM.style.height = null; // Set the text
    ...
    if (!borderColor) borderColor = dvt.HtmlTooltipManager._BORDER_COLOR;
    if (!this._isCustomBorderColor) outerElem.style.borderColor = borderColor;
    this._isCustomBorderColor = false; // Position the outer element
  ...
  dvt.HtmlTooltipManager.prototype.PostElement = function (tooltip, x, y, noEvents, useOffset) {
    ...
    tooltip.style['pointer-events'] = noEvents ? 'none' : 'auto'; // Clear the width and height, and reset the position to 0,0. This allows positionTip to get the accurate tooltip size

    tooltip.style.width = null;
    tooltip.style.height = null;
    tooltip.style.left = '0px';
    tooltip.style.top = '0px'; // Add offsets to the tip position as needed

    if (useOffset) {
      var offsets = this._getOffsets(tooltip);

      this.positionTip(x + offsets.x, y + offsets.y, x, y);
    } else this.positionTip(x, y, x, y); // Make the tooltip visible


    tooltip.style.visibility = 'visible'; // Invoke the tooltip attached callback if one is available.
  ...
  dvt.HtmlTooltipManager.prototype.hideTooltip = function () {
    ...
    if (tooltip) {
      ...
      if (!dvt.Agent.isRightToLeft(this._context)) tooltip.style.left = '0px';else tooltip.style.left = tooltip.style.right;
      tooltip.style.top = '0px'; // Use JQuery .remove() to cleanup DOM to prevent memory leaks
  ...
  dvt.HtmlTooltipManager.prototype.showTooltip = function (x, y, text, shape, bTrackMouse, borderColor) {
    ...
    this._x = x;
    this._y = y;
    this._text = text; //TODO: trackMouse is not working!
    ...
    if (dvt.Agent.isTouchDevice() || this._tooltipDisplayed && this._trackMouse) {
      ...
      this._showTimerId = window.setTimeout(this._handleShowTimer.bind(this), dvt.HtmlTooltipManager._SHOW_DELAY); //@HTMLUpdateOK
    }
  };
  /**
  ...
  dvt.HtmlTooltipManager.prototype.getTooltipBounds = function () {
    var tooltip = this.getTooltipElem(); // Fix for 17898759: The offset height can be incorrect if the tooltip div inherits from the HTML body.  Set to auto
  ...
  dvt.HtmlTooltipManager.prototype.positionTip = function (x, y) {
    ...
    var tooltipHeight = tooltipDimensions.h; // Calculate the bounds of the browser viewport, within which we'll position the tooltip

    var viewportBounds = new dvt.Rectangle(window.pageXOffset + dvt.HtmlTooltipManager._VIEWPORT_BUFFER, window.pageYOffset + dvt.HtmlTooltipManager._VIEWPORT_BUFFER, window.innerWidth - 2 * dvt.HtmlTooltipManager._VIEWPORT_BUFFER, window.innerHeight - 2 * dvt.HtmlTooltipManager._VIEWPORT_BUFFER);
  ...
  dvt.HtmlTooltipManager.prototype.getCustomTooltip = function (tooltipFunc, dataContext) {
    var tooltipElem = this.getTooltipElem(); // Clearing out the previous tooltip content

    while (tooltipElem.hasChildNodes()) {
      tooltipElem.removeChild(tooltipElem.firstChild);
    }

    dataContext['parentElement'] = tooltipElem; // reset class and border color each time so that we can tell if the user specifies custom ones ( + 21150376)

    tooltipElem.className = '';
    tooltipElem.style.borderColor = ''; // Fix renderer context before calling the tooltip function

    dataContext = this._context.fixRendererContext(dataContext);
    var tooltip = tooltipFunc(dataContext); // if custom class or border color are specified, set flags so we don't override them with the default values

    if (tooltipElem.className) this._isCustomClassName = true;
    if (tooltipElem.style.borderColor) this._isCustomBorderColor = true; // Set a flag for whether to cleanup tooltip elements when hideTooltip() is called by checking for the
    ...
    this._bCleanupChildren = dataContext['_templateCleanup'] != null; // If the tooltipElem has been populated by the app and a null returned, don't overwrite it.
  ...
  dvt.HtmlKeyboardListenerUtils.getListener = function (useCapture) {
    if (useCapture) return dvt.HtmlKeyboardListenerUtils._captureListener;else return dvt.HtmlKeyboardListenerUtils._bubbleListener;
  };
  /**
   * The event listener that is called by the implementation object's bubble phase listeners.
   * This function will wrap the event and delegate to the real event listeners.
  ...
  var DvtHtmlRichTooltipManager = function DvtHtmlRichTooltipManager(context, domElementId) {
    this.Init(context, domElementId);
  };
