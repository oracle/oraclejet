...
oj.JsonTreeDataSource = function (data) {
  var tree = new oj._JsonTreeNodeDataSource(); // that's the root node
  ...
  for (var prop in source) {
    if (prop === 'children' || depth === 0 && source instanceof Array) {
      ...
      for (var j = 0; j < children.length; j++) {
        ...
        var node = new oj._JsonTreeNodeDataSource();
...
oj.JsonTreeDataSource.prototype.move = function (nodeToMove, referenceNode, position, callbacks) {
  ...
  if (refNodeKey == null || refNodeKey === this.data.id) {
    if (pos !== 'inside') {
      Logger.log('Error: root can not be the reference node if position equals to ' + pos);
      return;
    }
  ...
  if (!this._searchTreeById(moveNode, refNodeKey)) {
    ...
    Logger.log('Error: the node to move contains the reference node as its sub-tree.');
  }
};
/**
