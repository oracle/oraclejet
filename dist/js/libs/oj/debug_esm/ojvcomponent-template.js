...
const _proxyUnwrap = (value) => value?.[PROXY_SYMBOL] ?? value;
...
class VTemplateEngine {
    ...
    _reorderProps(propertyNode) {
        const extraArgs = propertyNode.type == OBJECT_EXP ? [propertyNode] : undefined;
        const isCallExpression = propertyNode.type == CALL_EXP;
        ...
        const callback = (context, resolvedProps) => {
            const originalProps = isCallExpression
                ? this._cspEvaluator.evaluate(propertyNode, { $context: context, $h: h, BindDom })
                : resolvedProps;
    ...
    _keepIdentifiers(node, propName) {
        if (!isElementRegistered(node.tagName))
            return false;
        const propMeta = getPropertyMetadata(propName, getPropertiesForElementTag(node.tagName));
        if (!propMeta)
            return false;
        // Keep identifiers for non-primitive types only - object/array/other/any
        const types = ElementUtils.getSupportedTypes(propMeta.type);
    ...
    _populateIdentifierProps(engineContext, attrNodes, identifiersMap) {
        ...
        attrNodes.push({
            type: PROPERTY,
            key: { type: LITERAL, value: '__oj_private_identifier_to_prop' },
        ...
        attrNodes.push({
            type: PROPERTY,
            key: { type: LITERAL, value: '__oj_private_identifier_to_value' },
            value: this._createCallNodeWithContext(($context) => {
