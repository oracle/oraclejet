...
class RowDataGridProvider {
        ...
        if (options?.expandedObservable) {
            const expandedObservable = options?.expandedObservable.subscribe((value) => {
                ...
                completionPromise?.then(() => {
                    const oldExpandedKeySet = this.expandedState?.keys || new KeySetImpl([]);
    ...
    getFilterState(index, axis) {
        ...
        if (this.filterCriteria && filterAttribute) {
            if (DataProviderUtils.doesAttributeExistInFilterCriterion(filterAttribute, this.filterCriteria)) {
                return 'filtered';
            }
        }
        return 'filterable';
