...
class DvtTreeNode {
  ...
  getDatatip() {
    ...
    return Displayable.resolveShortDesc(this._shortDesc, () => DvtTreeNode.getShortDescContext(this));
  }
  ...
  animateUpdate(handler, oldNode) {
    ...
    if (!ArrayUtils.equals(startState, endState)) {
      ...
      nodePlayable = new CustomAnimation(this.getView().getCtx(), this, this.getView().__getAnimDur());
      nodePlayable.getAnimator().addProp(Animator.TYPE_NUMBER_ARRAY, this, this.GetAnimParams, this.SetAnimParams, endState);
...
class DvtSunburstNode extends DvtTreeNode {
    ...
    if (!(this.isRootNode() && this._createRootNodeContent())) {
      ...
      if (this._text != null) {
        ...
        if (this.hasPattern() || backgroundColor) {
          ...
          this._textBackground = new Rect(this.getView().getCtx(), txtDims.x, txtDims.y, txtDims.w, txtDims.h);
          if (backgroundColor)
  ...
  _createTextNode(container) {
    ...
    if (this._labelDisplay === 'auto') {
      ...
      // in those browsers.
      bRotated = Agent.browser === 'ie' || Agent.browser === 'edge' || Agent.os !== 'windows' || Agent.isEnvironmentTest();
    }
  ...
  getAriaLabel() {
    ...
    return Displayable.generateAriaLabel(this.getShortDesc(), states, () => DvtTreeNode.getShortDescContext(this));
  }
...
class DvtTreemapNode extends DvtTreeNode {
  ...
  showHoverEffect() {
    ...
    if (this._textStyle === this.TEXT_STYLE_HEADER) {
      ...
      stroke = new Stroke(nodeHeaderDefaults['hoverInnerColor'], this.GROUP_HOVER_INNER_OPACITY, this.GROUP_HOVER_INNER_WIDTH);
      ...
      if (this._shape.getCtx().getThemeBehavior() === 'redwood') {
        ...
        stroke = new Stroke(nodeDefaults['hoverColor'], this.NODE_HOVER_OPACITY, this.NODE_SELECTION_WIDTH);
      }
      //  - treemap visualization issues
      h = Math.max(h, 0);
      w = Math.max(w, 0);
  ...
  _getIsolateAnimation() {
    if (this._oldState) {
      ...
      var playable = new CustomAnimation(this.getView().getCtx(), this, this._ANIMATION_ISOLATE_DURATION);
      playable.getAnimator().addProp(Animator.TYPE_NUMBER_ARRAY, this, this.GetAnimParams, this.SetAnimParams, this.GetAnimParams());
  ...
  _createShapeNode() {
    ...
    if (this._textStyle === this.TEXT_STYLE_HEADER) {
      ...
      if (bNodeGaps) {
        ...
        shape = new Rect(context, rectsGaps._shape.x, rectsGaps._shape.y, rectsGaps._shape.w, rectsGaps._shape.h);
        ...
        if (bVisualEffects && this._width >= this.MIN_SIZE_FOR_BORDER && this._height >= this.MIN_SIZE_FOR_BORDER) {
          ...
          var topLeftColor = ColorUtils.interpolateColor(this.DEFAULT_NODE_TOP_BORDER_COLOR, fillColor, 1 - this.DEFAULT_NODE_BORDER_OPACITY);
          var bottomRightColor = ColorUtils.interpolateColor(this.DEFAULT_NODE_BOTTOM_BORDER_COLOR, fillColor, 1 - this.DEFAULT_NODE_BORDER_OPACITY);
  ...
  _createIsolateRestoreButton(container) {
    ...
    if (availIconWidth > this._ISOLATE_ICON_SIZE) {
      ...
      // Add a buffer to make the objects easier to interact with on touch devices
      if (Agent.isTouchDevice()) {
        var rect = new Rect(container.getCtx(),
  ...
  _createTextNode(container) {
    ...
    if (this._textStyle === this.TEXT_STYLE_NODE && availHeight > textHeight * 2 && computedTextStyle.getStyle(CSSStyle.WHITE_SPACE) !== 'nowrap' &&
    ...
    if (this._textStyle === this.TEXT_STYLE_NODE) {
      ...
      var chromeAdjustment = (Agent.browser === 'safari' || Agent.engine === 'blink') ? this._LINE_FUDGE_FACTOR : 0;
  ...
  ApplyHeaderStyle(shape, innerShape) {
    ...
    if (this._bHeaderUseNodeColor) {
      ...
      var borderColor = ColorUtils.interpolateColor(nodeHeaderDefaults['borderColor'], fillColor, 1 - this.DEFAULT_HEADER_WITH_NODE_COLOR_ALPHA);
  ...
  _getButtonHelper(resourceKey, callback) {
    ...
    var iconStyle = ToolkitUtils.getIconStyle(context, iconClass);
    // Create button and hook up click listener
    var button = new IconButton(context, 'borderless', {style: iconStyle, size: this._ISOLATE_ICON_SIZE});
...
class DvtTreeCategoryRolloverHandler extends CategoryRolloverHandler {
  ...
  _processHighlighting() {
    ...
    if (highlightMode === 'descendants') {
      ...
      CategoryRolloverHandler.highlight(options['highlightedCategories'], DvtTreeUtils.getAllNodes(tree.getRootNode()), options['highlightMatch'] == 'any');
    }
  }
}
...
class Sunburst extends DvtTreeView {
  ...
  Render(container, bounds) {
    ...
    if (this.__isRotationEnabled() && this.HasValidData()) {
      var buffer = (Agent.isTouchDevice()) ? this._ROTATION_SHAPE_RADIUS_TOUCH : this._ROTATION_SHAPE_RADIUS;
  ...
  expandCollapseNode(id) {
    ...
    this.dispatchEvent(new EventFactory.newExpandCollapseEvent(bDisclosed ? 'expand' : 'collapse', id, rootAndAncestors, this.getOptions()['_widgetConstructor'], expanded));
    this.__setNavigableIdToFocus(id);
  }
