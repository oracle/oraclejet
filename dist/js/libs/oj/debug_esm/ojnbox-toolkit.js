...
const DvtNBoxDataUtils = {
  ...
  isNodeHighlighted: (nbox, node) => {
    ...
    if (options['highlightedCategories'] && !options['__highlightedMap']) {
      options['__highlightedMap'] = ArrayUtils.createBooleanMap(
    ...
    else return ArrayUtils.hasAnyMapItem(options['__highlightedMap'], node['categories']);
  },
  ...
  buildAriaDesc: (nbox, object, datatip, selected, context) => {
    ...
    if (object.nboxType === 'categoryNode') {
      ...
      states.push(
        ResourceUtils.format(translations.labelAndValue, [translations.labelSize, nodeCount])
      );
...
const DvtNBoxStyleUtils = {
  ...
  getAnimDuration: (nbox) => {
    return (
      CSSStyle.getTimeMilliseconds(nbox.getOptions()['styleDefaults']['animationDuration']) /
      1000
    );
  },
  ...
  _getCellStyleOption: (nbox, cellIndex, styleKey, svgStyleKey) => {
    ...
    if (cellStyle) {
      if (!(cellStyle instanceof Object)) cellStyle = CSSStyle.cssStringToObject(cellStyle);
...
const DvtNBoxNodeRenderer = {
  ...
  _calculateSimpleNodeLayout: (nbox) => {
    ...
    if (icon) {
      var touchDeviceSize = Agent.isTouchDevice()
  ...
  _renderNodeBackground: (nbox, node, nodeContainer, nodeLayout) => {
    ...
    var selectionRect = new Rect(nbox.getCtx(), 0, 0, width, height);
  ...
  _renderNodeIndicator: (nbox, node, nodeContainer, nodeLayout) => {
    ...
    if (indicatorIcon) {
      ...
      if (indicatorIcon['source']) {
        indicatorMarker = new ImageMarker(
          nbox.getCtx(),
          indicatorIconX,
          indicatorIconY,
          indicatorIconWidth,
        ...
        indicatorMarker = new SimpleMarker(
          nbox.getCtx(),
          indicatorIcon['shape'],
          indicatorIconX,
          indicatorIconY,
      ...
      if (indicatorIconPattern != 'none')
        indicatorMarker.setFill(
          new PatternFill(indicatorIconPattern, indicatorIconColor, color)
        );
  ...
  _renderNodeIcon: (nbox, node, nodeContainer, nodeLayout) => {
    ...
    if (icon) {
      ...
      if (iconSource) {
        ...
        iconMarker = new SimpleMarker(nbox.getCtx(), null, iconX, iconY, iconWidth, iconHeight);
  ...
  animateUpdate: (animationHandler, oldNode, newNode) => {
    ...
    if (oldScrollContainer || newScrollContainer) {
      ...
      if (oldScrollContainer) {
        ...
        var oldScrollRect = new Rectangle(
      ...
      if (newScrollContainer) {
        ...
        var newScrollRect = new Rectangle(
    ...
    var movePlayable = new AnimMoveTo(
      newNode.getCtx(),
      newNode,
      new Point(newGlobalMatrix.getTx(), newGlobalMatrix.getTy()),
      animationHandler.getAnimationDuration()
    );
    ...
    if (!isNaN(newNodeIndex)) {
      ...
      if (!DvtNBoxDataUtils.isNodeHidden(newNBox, newNode)) {
        ...
        // Should it move to a different cell?
          ...
          if (overflow) {
            ...
            animationHandler.add(
              new AnimMoveTo(
                newNBox.getCtx(),
                oldNode,
                new Point(overflowMatrix.getTx(), overflowMatrix.getTy()),
                animationHandler.getAnimationDuration()
              ),
              DvtNBoxDataAnimationHandler.UPDATE
            );
            ...
            var cellCenterOffset = new Point(
              (nodeLayout['indicatorSectionWidth'] +
                nodeLayout['iconSectionWidth'] +
                nodeLayout['labelSectionWidth']) /
                2,
          ...
          var scaleAnim = new CustomAnimation(
            newNBox.getCtx(),
            newNode,
            animationHandler.getAnimationDuration()
          );
          scaleAnim
            .getAnimator()
            .addProp(Animator.TYPE_NUMBER, oldNode, oldNode.getScaleX, oldNode.setScaleX, 0.01);
          scaleAnim
            .getAnimator()
            .addProp(Animator.TYPE_NUMBER, oldNode, oldNode.getScaleY, oldNode.setScaleY, 0.01);
          Playable.appendOnEnd(scaleAnim, () => {
          ...
          animationHandler.add(
            new AnimMoveBy(
              newNBox.getCtx(),
              oldNode,
              new Point(oldDims.w / 2, oldDims.h / 2),
              animationHandler.getAnimationDuration()
            ),
            DvtNBoxDataAnimationHandler.INSERT
          );
          animationHandler.add(
            new AnimFadeOut(newNBox.getCtx(), oldNode, animationHandler.getAnimationDuration()),
            DvtNBoxDataAnimationHandler.INSERT
          );
  ...
  animateInsert: (animationHandler, newNode) => {
          ...
          if (group) {
            ...
            if (groupNode) {
              ...
              var movePlayable = new AnimMoveTo(
                newNBox.getCtx(),
                newNode,
                new Point(finalMatrix.getTx(), finalMatrix.getTy()),
                animationHandler.getAnimationDuration()
              );
              Playable.appendOnEnd(movePlayable, () => {
    ...
    var fadeAnim = new CustomAnimation(
      newNBox.getCtx(),
      newNode,
      animationHandler.getAnimationDuration()
    );
...
class DvtNBoxNode extends Container {
  ...
  HandleKeyboardEvent(event) {
    //use ENTER to drill down the cell and ESCAPE to drill up
    var drawerData = DvtNBoxDataUtils.getDrawer(this._nbox);
    if (drawerData && event.keyCode == KeyboardEvent.ESCAPE) {
      //drawer
      this.handleDoubleClick();
    } else {
      var maximizedRow = DvtNBoxDataUtils.getMaximizedRow(this._nbox);
...
const DvtNBoxCategoryNodeRenderer = {
  ...
  _renderNodeBackground: (nbox, node, nodeContainer, scale, gap) => {
    ...
    var selectionRect = new Rect(nbox.getCtx(), -side / 2, -side / 2, side, side);
    selectionRect.setFill(null);
    if (borderRadius) {
      selectionRect.setRx(borderRadius);
      selectionRect.setRy(borderRadius);
    ...
    var nodeRect = new Rect(nbox.getCtx(), -side / 2, -side / 2, side, side);
    if (borderRadius) {
      nodeRect.setRx(borderRadius);
      nodeRect.setRy(borderRadius);
    }
  ...
  _renderNodeIndicator: (nbox, node, nodeContainer, scale, gap) => {
    ...
    if (indicatorIcon) {
      var indicatorIconColor = indicatorIcon['color'] ? indicatorIcon['color'] : contrastColor;
      var indicatorMarker = new SimpleMarker(
        nbox.getCtx(),
        indicatorIcon['shape'],
      ...
      if (indicatorIcon['pattern'] && indicatorIcon['pattern'] != 'none') {
        indicatorMarker.setFill(
          new PatternFill(indicatorIcon['pattern'], indicatorIconColor, color)
        );
      } else {
  ...
  _renderNodeCount: (nbox, node, nodeContainer, scale, bIndicator, gap) => {
    ...
    var fontSize = TextUtils.getOptimalFontSize(
      label.getCtx(),
      label.getTextString(),
      label.getCSSStyle(),
      labelBounds
  ...
  animateDelete: (animationHandler, oldNode) => {
    ...
    var scalePlayable = new AnimScaleTo(
      newNBox.getCtx(),
      oldNode,
      new Point(0.01, 0.01),
      animationHandler.getAnimationDuration()
    ...
    var fadePlayable = new AnimFadeOut(
      newNBox.getCtx(),
      oldNode,
      animationHandler.getAnimationDuration()
    );
...
const DvtNBoxDrawerRenderer = {
  ...
  render: (nbox, data, drawerContainer, availSpace) => {
    ...
    var keyboardFocusEffect = new KeyboardFocusEffect(
      nbox.getCtx(),
      drawerContainer,
      new Rectangle(-1, -1, drawerBounds.w + 2, drawerBounds.h + 2)
    );
  ...
  _renderHeader: (nbox, data, drawerContainer) => {
    ...
    var indicatorIconColor = indicatorColor
      ? ColorUtils.getContrastingTextColor(indicatorColor)
      : contrastColor;
    ...
    if (drawerBounds.w - closeWidth - 2 * drawerButtonGap - drawerStartGap > countIndicatorWidth) {
      var countIndicatorPath = PathUtils.roundedRectangle(
        0,
        0,
        countIndicatorWidth,
      ...
      if (countIndicatorSectionWidth > 0) {
        if (indicatorColor) {
          var indicatorSectionPath = PathUtils.roundedRectangle(
            indicatorX,
            0,
  ...
  _renderBody: (nbox, data, drawerContainer) => {
    ...
    var bodyPath = PathUtils.roundedRectangle(
      0,
      0,
      drawerBounds.w,
      drawerBounds.h,
  ...
  getDrawerBounds: (nbox, data, availSpace) => {
    ...
    var gridGap = parseInt(options['__layout']['gridGap']);
    var cellLayout = DvtNBoxCellRenderer.getCellLayout(nbox);
    var drawerBounds = new Rectangle(
  ...
  animateInsert: (animationHandler, newDrawer) => {
    ...
    var fadePlayable = new AnimFadeIn(
      newNBox.getCtx(),
      newDrawer,
      animationHandler.getAnimationDuration()
    );
...
class DvtNBoxAutomation extends Automation {
  ...
  getDomElementForSubId(subId) {
    var nBox = this.getComponent();
    if (subId == Automation.TOOLTIP_SUBID) return this.GetTooltipElement(nBox);
...
class DvtNBoxCategoryNode extends Container {
  ...
  getNextNavigable(event) {
    ...
    if (event.keyCode == KeyboardEvent.SPACE && event.ctrlKey) {
      ...
      event.keyCode == KeyboardEvent.CLOSE_BRACKET &&
      DvtNBoxDataUtils.getGroupBehavior(this._nbox) == 'withinCell'
    ) {
      ...
      event.keyCode == KeyboardEvent.CLOSE_BRACKET ||
      event.keyCode == KeyboardEvent.OPEN_BRACKET
    ) {
...
class NBox extends BaseComponent {
  ...
  GetComponentDescription() {
    ...
    if (DvtNBoxDataUtils.hasValidData(this))
      return ResourceUtils.format(translations.componentName, [
