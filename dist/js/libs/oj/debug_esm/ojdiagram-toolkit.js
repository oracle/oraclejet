...
class DvtDiagramLink extends Container {
  ...
  getDatatip() {
    ...
    return Displayable.resolveShortDesc(this.getShortDesc(), () => DvtDiagramLink.getShortDescContext(this));
  }
  ...
  _movingToStart(direction) {
    ...
    return !((direction == KeyboardEvent.RIGHT_ARROW && linkDirectionL2R) ||
  ...
  animateUpdate(animationHandler, oldLink, bCleanUp) {
    ...
    if (this._getCustomRenderer('renderer')) {
      ...
      if (animateFrom && animateTo) {
        ...
        playable.getAnimator().addProp(Animator.TYPE_PATH, animateTo, pointsGetter, pointsSetter, PathUtils.createPathArray(newPointsCustom));
      }
      ...
      var playableExtra = new CustomAnimation(this.getCtx(), null, animationHandler.getAnimDur());
    ...
    if (bCleanUp) {
      var thisRef = this;
      Playable.appendOnEnd(playable, () => {thisRef.getParent().removeChild(thisRef);});
    }
  ...
  animateInsert(animationHandler) {
    this.setAlpha(0);
    animationHandler.add(new AnimFadeIn(this.getCtx(), this, animationHandler.getAnimDur()), DvtDiagramDataAnimationPhase.INSERT);
  }
  ...
  CreateConnector(points, connectorType, connectorPos) {
    ...
    var stroke = new Stroke(origStroke.getColor(), origStroke.getAlpha(), origStroke.getWidth());
...
class DvtDiagramNode extends Container {
  ...
  static _renderNodeBackground(diagram, nodeData, container) {
    ...
    if (!backgroundStyle.isEmpty()) {
      ...
      if (styleObj)
        backgroundProps.forEach((entry) => {delete styleObj[CSSStyle.cssStringToObjectProperty(entry)];});
  ...
  getDatatip() {
    ...
    // Custom Tooltip from ShortDesc
    return Displayable.resolveShortDesc(this.getShortDesc(), () => DvtDiagramNode.getShortDescContext(this));
  }
  ...
  _animateCustomUpdate(animationHandler, oldNode) {
    ...
    var scaleNewToOld = new AnimScaleTo(this.getCtx(), this, new Point(1, 1), animationHandler.getAnimDur());
    ...
    if (oldNodeContent) {
      ...
      var scaleOldToNew = new AnimScaleTo(this.getCtx(), oldNodeContent, new Point(scaleTo, scaleTo), animationHandler.getAnimDur());
      animationHandler.add(scaleOldToNew, DvtDiagramDataAnimationPhase.UPDATE);

      var fadeOut = new AnimFadeOut(this.getCtx(), oldNodeContent, animationHandler.getAnimDur());
      animationHandler.add(fadeOut, DvtDiagramDataAnimationPhase.UPDATE);
      var thisRef = this;
      Playable.appendOnEnd(fadeOut, () => {thisRef.getParent().removeChild(oldNodeContent);});
  ...
  static _renderContainer(diagram, nodeData, container) {
    ...
    //Parse out the CSS properties which are already applied on the DOM
    if (styleObj)
      containerProps.forEach((entry) => {delete styleObj[CSSStyle.cssStringToObjectProperty(entry)];});
    ...
    if (focusDisp && keyCode != KeyboardEvent.TAB && this._bPassOnEvent) {
      ...
      if (focusObj) {
        ...
        var animator = DvtDiagramStyleUtils.getAnimOnDataChange(this._component) != 'none' ?
                new Animator(this.getCtx(), this._component.getAnimDur()) : null;
...
class DvtDiagramKeyboardHandler extends PanZoomCanvasKeyboardHandler {
  ...
  processKeyDown(event) {
    ...
    } else if (keyCode == KeyboardEvent.F2 && currentNavigable) {
    ...
    } else if (keyCode == KeyboardEvent.ESCAPE && diagram.activeInnerElems && currentNavigable) {
...
class DvtDiagramOverview extends Overview {
  ...
  animateUpdate(animationHandler, oldDiagramOverview) {
    ...
    animationHandler.add(new AnimFadeIn(this.getCtx(), this.Content, animationHandler.getAnimDur()), DvtDiagramDataAnimationPhase.UPDATE);
    ...
    var fadeOutAnim = new AnimFadeOut(this.getCtx(), oldDiagramOverview.Content, animationHandler.getAnimDur());
    Playable.appendOnEnd(fadeOutAnim, removeFunc);
    animationHandler.add(fadeOutAnim, DvtDiagramDataAnimationPhase.UPDATE);

    //animate viewport
    var customContentAnim = new CustomAnimation(this.getCtx(), null, animationHandler.getAnimDur());
    ...
    customContentAnim.getAnimator().addProp(Animator.TYPE_NUMBER_ARRAY, this, this.GetAnimParams, this.SetAnimParams, newAnimationParams);
    ...
    if(this.activeInnerElems) {
      ...
      if (this.getElem().contains(activeElem)) {
        ...
        if (!this._oldActiveElemId) {
          ...
          while (!((CustomElementUtils.isElementRegistered(activeElem.tagName) && activeElem.id) || activeElem.tagName === 'OJ-DIAGRAM')) {
...
InitComponentInternal() {
  ...
  storeTouchEventContent(content, diagramObj) {
    ...
    if (content.namespaceURI === ToolkitUtils.SVG_NS) {
      ...
      content.forEach((node) => {ToolkitUtils.appendChildElem(this.getTouchEventPane().getElem(), node);});
    }
  }
