...
class TimelineOverview extends Overview {
    ...
    for (let i = Math.min(this._maxDurationY, this._MAXIMUM_STACK); i > 0; i--) {
      for (let j = 0; j < markers.length; j++) {
        ...
        if (i === node._durationLevel) {
          ...
          const x = OverviewUtils.getDatePosition(start, end, node.getTime(), this.Width);
          ...
          var displayable = new Rect(context, cx, cy, rectWidth, rectHeight, itemId);
  ...
  addVerticalMarker(node, sz) {
    ...
    // Do not antialias markers if specified or vertical
    if (
      (this.isVertical() || marker === SimpleMarker.RECTANGLE) &&
  ...
  addDurations(markers, start, end) {
    ...
    for (let i = Math.min(this._maxDurationY, this._MAXIMUM_STACK); i > 0; i--) {
      for (let j = 0; j < markers.length; j++) {
        ...
        if (i === node._durationLevel) {
          const x = OverviewUtils.getDatePosition(start, end, node.getTime(), this.Width);
          const x2 = OverviewUtils.getDatePosition(start, end, node.getEndTime(), this.Width);
          ...
          duration = new Rect(context, rectX, rectY, rectWidth, rectHeight, durationId);
          if (node._background) {
  ...
  calculateDurationY(item, durationMarkers, start, end) {
    ...
    const startPos = OverviewUtils.getDatePosition(start, end, startTime, canvasSize);
    ...
    for (var i = 0; i < index; i++) {
      ...
      if (currItem !== item) {
        ...
        const currStartPos = OverviewUtils.getDatePosition(start, end, currStartTime, canvasSize);
        let currEndPos = OverviewUtils.getDatePosition(start, end, currEndTime, canvasSize);
