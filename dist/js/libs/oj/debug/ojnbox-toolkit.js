...
define(['exports', 'ojs/ojdvt-toolkit', 'ojs/ojtranslation'], function (exports, dvt, Translations) { 'use strict';
  ...
  class DvtNBoxNode extends dvt.Container {
    ...
    _setDraggableStyleClass() {
      if (this._nbox.getOptions().dnd) {
        dvt.ToolkitUtils.addClassName(this.getElem(), 'oj-draggable');
      }
    }
  }
  ...
  class DvtNBoxKeyboardHandler extends dvt.KeyboardHandler {
    ...
    processKeyDown(event) {
      ...
      if (keyCode == dvt.KeyboardEvent.TAB) {
        ...
        (keyCode == dvt.KeyboardEvent.X && event.ctrlKey && transferOptions.cut === 'enable') ||
        (keyCode == dvt.KeyboardEvent.C && event.ctrlKey && transferOptions.copy === 'enable')
      ) {
        var navigables = [];
        var contexts = [];
        if (
        ...
        var event;
        if (keyCode == dvt.KeyboardEvent.X) {
          event = new dvt.EventFactory.newEvent('cutRequest');
          this._nbox.setActiveCut(navigables);
        } else if (keyCode == dvt.KeyboardEvent.C) {
          event = new dvt.EventFactory.newEvent('copyRequest');
        }
        if (event) {
        ...
        if (currentNavigable && currentNavigable.getData().row) {
          ...
          var event = new dvt.EventFactory.newEvent('pasteRequest');
          event['target'] = target;
          this._nbox.setActiveCut(null);
          this._nbox.dispatchEvent(event);
        }
        return null;
      } else if (keyCode == dvt.KeyboardEvent.ESCAPE && this._nbox.getActiveCut()) {
        this._nbox.setActiveCut(null);
        return null;
      }
  ...
  const DvtNBoxCellRenderer = {
    ...
    render: (nbox, cellData, cellContainer, availSpace) => {
      ...
      var keyboardEffect = new dvt.KeyboardFocusEffect(
        nbox.getCtx(),
        cellContainer,
  ...
  class DvtNBoxEventManager extends dvt.EventManager {
    ...
    ProcessKeyboardEvent(event) {
      ...
      if (
        keyCode == dvt.KeyboardEvent.ENTER ||
        (keyCode == dvt.KeyboardEvent.ESCAPE && !this._nbox.getActiveCut())
      ) {
        //drill up and down
    ...
    ClearDragSourceEffect() {
      ...
      if (nodes && nodes.length > 0) {
        for (var i = 0; i < nodes.length; i++) {
          dvt.ToolkitUtils.setAttrNullNS(nodes[i].getElem(), 'opacity', '1');
        }
      }
