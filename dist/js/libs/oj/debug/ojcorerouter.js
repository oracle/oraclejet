...
(function () {
  ...
  function CoreRouter(routes, options, parentRouter) {
    ...
    this.beforeStateChange = new Observable.BehaviorSubject({ accept: function () {} });

    this.currentState = new Observable.BehaviorSubject({ complete: function () {} });

    // If parent router is history-skipping, so are all of its descendants
    this._noHistory = (parentRouter && parentRouter._noHistory) || options.history === 'skip';
