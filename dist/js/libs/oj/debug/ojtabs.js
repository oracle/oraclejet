...
(function () {
  ...
  oj.__registerWidget('oj.ojTabs', $.oj.baseComponent, {
    ...
    _NotifyContextMenuGesture: function (menu, event, eventType) {
      ...
      if (this._isInATab(event.target) && !(this._touchProxy && this._touchProxy.touchMoved())) {
        ...
        if (this._menu.$elemRemove) {
          ...
          if (
            (isDisabled || this._getCloseIcons(tab).length === 0) &&
    ...
    _panelKeydown: function (event) {
      ...
      if (
        $(event.target).closest(_OJ_TABS_SEL).attr('id') !==
    ...
    _processTabs: function () {
      ...
      this.element.children(_OJ_TABS_FACET).each(function () {
        var facet = $(this);
      ...
      else if (trailingBtn && trailingBtn.length > 0) {
      ...
      this.tablist.children('li').each(function () {
        ...
        anchor.children().addClass('oj-tabs-title').removeAttr(_ARIA_HIDDEN);
        ...
        panel
          .attr(_ARIA_LABELLEDBY, anchorId) // @HTMLUpdateOK
    ...
    _toggle: function (event, eventData) {
      ...
      var fromTab = eventData.fromTab.closest('li').removeClass(_OJ_SEL);
    ...
    _destroy: function () {
      ...
      if (verticalEdge) {
        this.element.removeClass(
          'oj-tabs oj-component oj-tabs-vertical oj-tabs-start oj-tabs-end oj-helper-clearfix'
        );
      } else {
        this.element.removeClass(
          'oj-tabs oj-component oj-tabs-horizontal oj-tabs-top oj-tabs-bottom oj-first-child-selected'
        );
      }
      ...
      this.tabs.each(function (index) {
        ...
        div = tab.children('div').removeClass(_OJ_TABS_CONTENT);
        ...
        header.removeClass('oj-tabs-title').removeAttr(_ARIA_HIDDEN);
    ...
    _wrapLi: function (header, contentId) {
      ...
      if (header.prop('tagName').toLowerCase() === 'li') {
        ...
        if (div.length === 1 && div.hasClass(_OJ_TABS_CONTENT)) {
          ...
          div = tab
            .wrapInner("<div><a href='#'></a></div>") // @HTMLUpdateOK
            .children();
    ...
    addTab: function (newTab) {
      ...
      if (newTab.tab && newTab.content) {
        ...
        content = newTab;
        tab = this._wrapLi($(newTab).find(_FIRST_CHILD), this._getUniqueId(content));
      }
      ...
      if (navRoot.hasClass('oj-tabs-nav')) {
        ...
        tabbar = navRoot.length
          ? navRoot.find('.oj-tabs-nav')
    ...
    _setupEdge: function (_edge) {
      ...
      if (this._initialRender) {
        ...
        this.option('edge', edge, { _context: { internalSet: true }, changed: true });
      }
    ...
    getNodeBySubId: function (locator) {
      ...
      if (
        subId !== _CONV &&
        (typeof index !== 'number' || index < 0 || index >= this.panels.length)
      ) {
        return null;
      }
    ...
    _setOjDisabledOnTab: function (disTabs) {
      ...
      children.removeClass(_OJ_DISABLED).removeAttr(_ARIA_DISABLED);
