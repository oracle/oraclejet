 ...
 */
"use strict";
define(['ojs/ojcore', 'ojs/ojcomponentcore'], 
       /*
        * @param {Object} oj 
        */
       function(oj)
...
function ojValidationGroup(context) {
    ...
    if (document.body.contains(element))
    {
      ...
      for (var i=0; i < mutationsLength && !refreshNeeded; i++)
      {
        ...
        if (!refreshNeeded && addedNodes.length > 0)
        {
          ...
          ignore = _isNodeInTrackedComponent(target);
  ...
  self.updateDOM = function() 
  {
    ...
    // property.
    var busyContext = oj.Context.getContext(_ojDiv).getBusyContext();
   ...
   */
   self.showMessages = function ()
   {
     var component;
      ...
      {
        component = _trackedComponents[i];
  ...
  { 
    ...
    else
    {
      ...
      oj.Logger.info("focusOn's parameter value is not '@firstInvalidShown' or empty, so it's a no-op.");
    }
  };
   ...
   * @private
   */
  function _handleValidChanged(event)
  {
    context.props["valid"] = _consolidateValid();
  }
  ...
  function _consolidateValid()
  {
    ...
    for(var i=0; i < length && mostInvalid !== "invalidShown"; i++)
    {
      ...
      {
        componentValidState = component["valid"];
  ...
  function _isComponentNotDestroyed(elem) 
  {
    ...
    var bridge;
    var bridgeWidgetElem;
    var widgetElem;
    
    bridge = oj.BaseCustomElementBridge.getInstance(elem);
    bridgeWidgetElem = bridge._WIDGET_ELEM;
    ...
    if (bridgeWidgetElem !== undefined)
      widgetElem = oj.Components.__GetWidgetConstructor(bridgeWidgetElem);

    return (bridgeWidgetElem === undefined || widgetElem) ? true : false;
