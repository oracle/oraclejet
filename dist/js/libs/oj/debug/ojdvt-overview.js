  ...
  const DvtOverviewRenderer = {
    ...
    _createGrippyImage: (overview, width, height, isVertical) => {
      ...
      if (!isVertical) {
        ...
        grippy = dvt.OutputText.createIcon(ctx, { style: iconStyle, size: overview._horHandleSize, pos: { x: posX, y: 0 } });
        grippy.setId('hgrpy');
      }
      else {
        var posY = height / 2;
        var iconStyleY = dvt.ToolkitUtils.getIconStyle(ctx, overview._vertHandleBackgroundClass);
        grippy = dvt.OutputText.createIcon(ctx, { style: iconStyleY, size: overview._vertHandleSize, pos: { x: 0, y: posY } });
        grippy.setId('vgrpy');
      }
    ...
    _addFilledTimeRange: (overview, elem, width, height) => {
      ...
      if (rangeStart != null && rangeEnd != null) {
        ...
        var displayable;
        if (overview.isVertical())
          displayable = new dvt.Rect(overview.getCtx(), 0, rangeStart_pos, width - overview.getTimeAxisWidth(), rangeWidth, 'ftr');
        else {
          var overviewAbove = overview.isOverviewAbove() ? overview.getTimeAxisHeight() : 0;
          displayable = new dvt.Rect(overview.getCtx(), rangeStart_pos, overviewAbove, rangeWidth, height - overview.getTimeAxisHeight(), 'ftr');
        }
