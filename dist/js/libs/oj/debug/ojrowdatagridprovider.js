 ...
 * @ignore
 */
define(['exports', 'ojs/ojdatagridprovider', 'ojs/ojeventtarget', 'ojs/ojdataprovider', 'ojs/ojkeyset'], function (exports, ojdatagridprovider, ojeventtarget, ojdataprovider, ojkeyset) { 'use strict';
    ...
    class RowDataGridProvider {
        ...
        async fetchByOffset(parameters) {
            ...
            if (!this.supportsFetchByOffsetFilteredRowCount) {
                let totalSize = -1;
                const sameFetchParameters = ojdataprovider.DataProviderUtils.isSameFetchParameters(this.currentFetchParameters, fetchResult.fetchParameters, ['sortCriteria', 'filterCriterion']);
        ...
        updateKeyCache(fetchResult, rowOffset) {
            ...
            if (!ojdataprovider.DataProviderUtils.isSameFetchParameters(this.currentFetchParameters, fetchParameters, [
                'sortCriteria',
                'filterCriterion'
            ])) {
                this.keyCache.reset();
        ...
        _handleUnderlyingMutation(event) {
            ...
            if (!needsRefresh && detail.add && detail.add.keys.size > 0) {
                const finalKeys = ojdataprovider.DataProviderUtils.getAddEventKeysResult(this.keyCache.getKeys(), detail.add, allLoaded);
                needsRefresh =
