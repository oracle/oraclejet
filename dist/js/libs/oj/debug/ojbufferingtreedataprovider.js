...
define(['ojs/ojcore-base', 'ojs/ojdataprovider', 'ojs/ojeventtarget', 'ojs/ojmap', 'ojs/ojset', 'ojs/ojbufferingutils', 'ojs/ojbufferingdataproviderevents', 'ojs/ojmutablearraytreedataprovider', 'ojs/ojlogger'], function (oj, ojdataprovider, ojeventtarget, ojMap, ojSet, ojbufferingutils, ojbufferingdataproviderevents, ojmutablearraytreedataprovider, Logger) { 'use strict';
    ...
    class BufferingTreeDataProvider {
        constructor(_dataProvider, _options, _rootBufferingDataProvider, _parentKey) {
            ...
            this.TreeAsyncIterator = class {
                ...
                next() {
                    return ojbufferingutils.BufferingDataProviderUtils.fetchNext(this._params, this._baseIterator, this, this._parent.lastSortCriteria, this._parent.getTreeEditBuffer(), this._parent);
                }
            };
        ...
        getTreeEditBuffer() {
            return new ojbufferingutils.TreeEditBuffer(this.editBuffer, this.parentKey, this);
        }
        getChildDataProvider(parentKey) {
            ...
            if (!childDp) {
                ...
                childDp = new ojmutablearraytreedataprovider.MutableArrayTreeDataProvider([], '@value', { useKeyPaths: 'on' });
            }
        ...
        hasBufferedChildren(keyPath) {
            const treeEditBuffer = new ojbufferingutils.TreeEditBuffer(this.editBuffer, keyPath, this);
        ...
        containsKeys(params) {
            return ojbufferingutils.BufferingDataProviderUtils.containsKeys(params, this.getTreeEditBuffer(), this.dataProvider);
        }
        fetchByKeys(params) {
            return ojbufferingutils.BufferingDataProviderUtils.fetchByKeys(params, this.getTreeEditBuffer(), this.dataProvider);
        }
        removeItem(item) {
            const keyPath = this.convertKeyToKeyPath(item.metadata.key);
            const parentKey = this.getParentKey(keyPath);
            const mutationEvent = ojbufferingutils.BufferingDataProviderUtils.removeItem(item, this.getIteratorByParentKey(parentKey), this.getKeyCacheByParentKey(parentKey), this.getTreeEditBuffer());
        ...
        updateItem(item) {
            const mutationEvent = ojbufferingutils.BufferingDataProviderUtils.updateItem(item, this.getTreeEditBuffer());
            this._populateParentKeyAndDispatch(mutationEvent);
            this.dispatchSubmittableChangeEvent(this.editBuffer);
        }
        ...
        addItem(item, addDetail) {
            ...
            if (addDetail?.nullParentKey != null && item.metadata.key == null) {
                const generatedKey = ojbufferingutils.BufferingDataProviderUtils.generateKey(item.data, this.customKeyGenerator);
                ...
                Logger.error('item.metadata.key is null and addDetail.nullParentKey was not provided');
            }
            ...
            if (addDetail?.addBeforeKey != null || addDetail?.addAfterKey != null) {
                ...
                if (!oj.KeyUtils.equals(parentKey, addDetailParentKey)) {
                    Logger.error(`${addDetailAttribute} is not a descendant of item's parentKey`);
                    delete addDetails[addDetailAttribute];
                }
            }
            const parentIterator = this.getIteratorByParentKey(parentKey);
            const mutationEvent = ojbufferingutils.BufferingDataProviderUtils.addItem(addItem, this.getTreeEditBuffer(), parentIterator, this.getKeyCacheByParentKey(parentKey), addDetails);
            if (isNewParent) {
                this.dispatchEvent(new ojdataprovider.DataProviderRefreshEvent({ keys: new ojSet([parentKey]) }));
            }
            else {
                this._populateParentKeyAndDispatch(mutationEvent);
            }
        ...
        fetchByOffset(params) {
            ...
            if (!keyCache) {
                keyCache = new ojdataprovider.KeyCache();
            ...
            return ojbufferingutils.BufferingDataProviderUtils.fetchByOffset(params, this.getTreeEditBuffer(), keyCache, lastFetchParams, this.dataProvider, ignoreAfterTransaction);
        }
        fetchFirst(params) {
        ...
        getTotalSize() {
            return ojbufferingutils.BufferingDataProviderUtils.getTotalSize(this.dataProvider, this.getTreeEditBuffer());
        }
        isEmpty() {
            return ojbufferingutils.BufferingDataProviderUtils.isEmpty(this.getTreeEditBuffer(), this.dataProvider);
        }
        getSubmittableItems() {
        ...
        resetUnsubmittedItem(key) {
            ...
            this.dispatchSubmittableChangeEvent(this.editBuffer);
            ojbufferingutils.BufferingDataProviderUtils.resetUnsubmittedItem(editItem, this.editBuffer, this.dataProvider, this.getIteratorByParentKey(this.parentKey), keySet).then((mutationEvent) => {
                this._populateParentKeyAndDispatch(mutationEvent);
            });
        }
        ...
        _addEventListeners() {
            this.dataProvider.addEventListener(BufferingTreeDataProvider._REFRESH, async (event) => {
                this.dataBeforeUpdated = new Map();
                const refreshEvent = await ojbufferingutils.BufferingDataProviderUtils.handleRefreshEvent(event, this.editBuffer, this.dataProvider);
                this.dispatchEvent(refreshEvent);
            });
            this.dataProvider.addEventListener(BufferingTreeDataProvider._MUTATE, (event) => {
                this.dataBeforeUpdated = new Map();
                const mutationEvent = ojbufferingutils.BufferingDataProviderUtils.handleMutateEvent(event, this.editBuffer, this.getIteratorByParentKey(this.parentKey), this.generatedKeyMap, this.lastSortCriteria);
                if (event.detail.update) {
                    ...
                    event.detail.update.keys.forEach((key) => {
                        const treeEditBuffer = new ojbufferingutils.TreeEditBuffer(this.editBuffer, key, this);
                        ...
                        childAdds.forEach((child) => {
                            if (child.operation === 'add') {
                                ...
                                ojbufferingutils.BufferingDataProviderUtils.cleanUpSubmittedItem(keyPath, treeEditBuffer);
                            }
                        });
                    });
                }
    ...
    ojeventtarget.EventTargetMixin.applyMixin(BufferingTreeDataProvider);
    oj._registerLegacyNamespaceProp('BufferingTreeDataProvider', BufferingTreeDataProvider);

    return BufferingTreeDataProvider;
