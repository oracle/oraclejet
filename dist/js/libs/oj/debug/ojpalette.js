/**
 * @license
 * Copyright (c) 2014, 2023, Oracle and/or its affiliates.
 * Licensed under The Universal Permissive License (UPL), Version 1.0
 * as shown at https://oss.oracle.com/licenses/upl/
 * @ignore
 */
define(['exports', 'ojs/ojpaletteutils'], function (exports, ojpaletteutils) { 'use strict';

    /**
     * Adapted from D3.js -- d3-scale-chromatic
     * Contains utility function to get colors from certain color palettes
     * @ojmodulecontainer ojpalette
     * @ojtsmodule
     * @ojhidden
     * @since 11.0.0
     */

    /**
     * Returns an array of colors from the corresponding color palette and range values.
     * @ojexports
     * @param {string} palette the color Palette where the values are from. Options are 'viridis', 'magma', 'inferno', and 'plasma'.
     * @param {number=} bins the number of color values to be returned, all the colors will be returned if set to 0 or null. Color values may be repeated if the number of bins exceeds the number of colors defined in the specified palette
     * @param {Object=} options Additional configuration options for the colors to be returned.
     * @return {Array<string>} An array containing CSS color strings from the corresponding color palette
     * @ojsignature [{target: "Type", value: "'viridis'|'magma'|'inferno'|'plasma'", jsdocOverride: true, for: "palette"},
     *               {target: "Type", value: "Options", jsdocOverride: true, for: "options"}]
     * @memberof ojpalette
     * @method
     * @name getColorValuesFromPalette
     *
     */
    /**
     * @ojexports
     * @typedef {Object} Options
     * @memberof ojpalette
     * @property {Array<number>=} range the range of the values to draw the color values from. By default the range is [0,1]. An error will be thrown if the values are outside [0,1] or if the start value is higher than the end value.
     * @ojsignature {target: "Type", value: "[number,number]", jsdocOverride: true, for: "range"}
     */

    const colorData = {
        viridis: '',
        magma: '',
        inferno: '',
        plasma: ''
    };
    const colorMapCache = {};
    const getColorMap = (palette) => {
        if (!colorMapCache[palette]) {
            const colorString = colorData[palette];
            const length = colorString.length;
            const colors = [];
            for (let i = 0; i < length / 6; i++) {
                colors.push(colorString.substr(i * 6, 6));
            }
            colorMapCache[palette] = colors;
        }
        return colorMapCache[palette];
    };
    const validateArgs = (bins, range) => {
        if (bins == 1 || bins < 0) {
            throw new Error('Bins size too small, must return at least 2 color values.');
        }
        if (range[0] < 0 || range[1] > 1) {
            throw new Error('Range is outside of [0,1], please constrain the range to [0,1]');
        }
        if (range[0] > range[1]) {
            throw new Error('Start range is greater than the end range');
        }
    };
    function getColorValuesFromPalette(palette, bins, options) {
        const colors = getColorMap(palette);
        const newColors = [];
        const range = options && options.range ? options.range : [0, 1];
        const numBins = bins ? bins : colors.length;
        validateArgs(numBins, range);
        const [start, end] = range;
        const space = (end - start) / (numBins - 1);
        for (let i = 0; i < numBins; i++) {
            const hexColor = ojpaletteutils.getColorValue(colors, start + i * space);
            newColors.push(`rgb(${parseInt(hexColor.substr(0, 2), 16)},${parseInt(hexColor.substr(2, 2), 16)},${parseInt(hexColor.substr(4, 2), 16)})`);
        }
        return newColors;
    }

    exports.getColorValuesFromPalette = getColorValuesFromPalette;

    Object.defineProperty(exports, '__esModule', { value: true });

});
