   ...
   */
    ...
    CreateAnimOnDataChange(oldObjs, newObjs, animationType, animationDuration) {
      ...
      for (var i = 0; i < oldObjs.length; i++) {
        ...
        animation.getAnimator().addProp(dvt.Animator.TYPE_NUMBER_ARRAY, newObj, newObj.getAnimParams, newObj.setAnimParams, endState);
  ...
  const DvtStatusMeterGaugeRenderer = {
    ...
    _renderShape: (gauge, container, bounds) => {
      ...
      if (gauge.getCtx().getThemeBehavior() === 'redwood' || options['visualEffects'] === 'none') {
        ...
        var arColors = [dvt.ColorUtils.adjustHSL(color, 0, -0.09, 0.04), dvt.ColorUtils.adjustHSL(color, 0, -0.04, -0.05)];
        var arAlphas = [1, 1];
        var arStops = [0, 1];
    ...
    _renderPlotAreaVisualEffects: (gauge, container, shape, color, gradientAngle) => {
      ...
      if ((gauge.getCtx().getThemeBehavior() !== 'redwood') && options['visualEffects'] !== 'none') {
        // Gradient
        var arColors = [dvt.ColorUtils.adjustHSL(color, 0, -0.04, -0.05), dvt.ColorUtils.adjustHSL(color, 0, -0.09, 0.04)];
    ...
    _renderMetricLabelOutsidePlotArea: (gauge, container, bounds) => {
      ...
      if (isVert && options['metricLabel']['rendered'] == 'on') {
        ...
        size = metricLabelStyle.getStyle('font-size') || dvt.TextUtils.getOptimalFontSize(maxMetricLabel.getCtx(), maxMetricLabel.getTextString(), maxMetricLabel.getCSSStyle(), computedMetricLabelBounds);
  ...
  class StatusMeterGauge extends DvtGauge {
    ...
    CreateAnimOnDisplay(objs, animationType, animationDuration) {
      ...
      for (var i = 0; i < objs.length; i++) {
        ...
        animation.getAnimator().addProp(dvt.Animator.TYPE_NUMBER_ARRAY, obj, obj.getAnimParams, obj.setAnimParams, endState);
